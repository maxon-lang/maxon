...
/// <summary>
/// One decoded DebugStream entry.
///
/// <see cref="Position"/> is the entry's ABSOLUTE byte offset in the ring's write sequence — not a
/// position within the 2 MB buffer, which wraps. It is what makes an entry comparable against the
/// watermark the debug agent records at a stop (<see cref="RuntimeEmitter.DbgOffStopDsMark"/>), and
/// so what lets `maxon debug` answer "which of these happened before the target parked".
///
/// <see cref="Text"/> is the fully decoded line body — the same text `maxon monitor` prints, because
/// it comes from the same formatter. What each FACE adds around it differs (the monitor prefixes a
/// timestamp and indents by depth; the debugger renders an ordered slice), which is why the timestamp
/// and the depth travel as data here rather than baked into the text.
/// </summary>
public readonly record struct DsEvent(
  long Position, byte EventType, string Family, uint TimestampMs, int Depth, string Text);

/// <summary>
/// The DebugStream WIRE DECODER — the reader side of the ring protocol, and the ONE place the binary
/// event format is understood.
///
/// It is shared by both consumers deliberately. `maxon monitor` prints a live timeline; `maxon debug`
/// renders the slice of trace activity between two stops. Those are different QUESTIONS about the same
/// bytes, and a second decoder beside this one is the two-parsers-one-format defect this project has
/// already paid for once (the error-code registry). So the segment header, the schema handshake, the
/// commit protocol, the interned name tables and every event formatter live here, and each face adds
/// only its own presentation.
///
/// THE COMMIT PROTOCOL (see the ring protocol note on <see cref="RuntimeEmitter"/>'s DebugStream
/// partial). An entry appears below `write_cursor` the moment it is RESERVED; its payload is written
/// afterwards, outside the ring lock. So "below write_cursor" does not mean "readable", and this
/// decoder must not treat it as if it did — it decodes only entries carrying
/// <see cref="RuntimeEmitter.DsEntryFlagCommitted"/>, and stops at the first one that does not.
/// </summary>
public sealed class DebugStreamDecoder : IDisposable {

  // The event families a consumer can select. They are CONTIGUOUS code ranges (see the event table in
  // RuntimeEmitter), so membership is a range test rather than a list that has to be kept in step with
  // that table — and the same derivation names an event's family for a consumer that wants to group by
  // it, so "which family is this" is answered once.
  public const string FamilyMm = "mm";
  public const string FamilySched = "sched";
  public const string FamilyLog = "log";

  /// The selectable families, in the order a usage line should list them.
  public static readonly IReadOnlyList<string> Families = [FamilyMm, FamilySched, FamilyLog];

  /// The family <paramref name="eventType"/> belongs to. Never called for padding or the depth markers,
  /// which are structural rather than events (see <see cref="Drain"/>).
  public static string FamilyOf(byte eventType) {
    if (eventType < RuntimeEmitter.DsEvSchedSpawn) return FamilyMm;
    return eventType >= RuntimeEmitter.DsEvLogPhaseBegin && eventType <= RuntimeEmitter.DsEvLogText
      ? FamilyLog
      : FamilySched;
  }

  /// The stem of the segment name / backing-file name. Carries the consumer's pid and a random suffix
  /// so that concurrent spec-test workers cannot collide on it.
  private const string SharedSegmentPrefix = "maxon_ds_";

  /// How a <see cref="Drain"/> ended. The four are DISTINCT because they demand different next moves,
  /// and conflating the last two is what once let a consumer spin forever on an entry nobody would ever
  /// commit.
  public enum DrainStatus {
    /// Entries were decoded (or an abandoned one was stepped over). Call again.
    Decoded,
    /// Nothing is decodable right now and the producer is still alive: either the ring is empty, or its
    /// head entry is reserved and its producer is mid-payload. Waiting is the only correct move.
    Idle,
    /// Nothing is decodable and the producer is GONE with the ring fully drained. There will be no more.
    Finished,
    /// The producer does not speak this schema. Nothing was decoded and nothing ever will be; see
    /// <see cref="SchemaMismatchMessage"/>.
    SchemaMismatch,
  }

  private readonly SharedMapping _mapping;
  private readonly MemoryMappedViewAccessor _accessor;
  private readonly string? _familyFilter;
  private readonly string[] _tagNames;
  private readonly string[] _logNames;
  private readonly byte[] _localBuf;
  private readonly long _bufferSize;
  private readonly long _bufferMask;
  private int _depth;

  /// The value the target reads from <see cref="RuntimeEmitter.DsActivationEnvVar"/>: this segment's
  /// name (a Win32 section name on Windows, a MAP_SHARED file path elsewhere).
  public string SegmentName => _mapping.SegmentName;

  /// How far the decoder has consumed the ring, as an absolute byte position. Every entry it has
  /// emitted started below this; nothing at or above it has been seen yet. It is what a consumer
  /// compares a stop's watermark against to know whether its slice is complete.
  public long ReadCursor { get; private set; }

  /// Why the producer's schema was refused, or empty when it was not. Set with
  /// <see cref="DrainStatus.SchemaMismatch"/>.
  public string SchemaMismatchMessage { get; private set; } = "";

  /// Entries whose producer DIED between __ds_reserve and __ds_commit. Their payload will never be
  /// written, so they are unreadable — but they are counted and reported, never silently dropped from
  /// the trace, because a missing event is exactly the lie the commit protocol prevents.
  public long AbandonedEntries { get; private set; }
  ...
  public long TotalEvents => _accessor.ReadInt64(RuntimeEmitter.DsOffTotalEvents);
  public long DroppedEvents => _accessor.ReadInt64(RuntimeEmitter.DsOffDroppedEvents);
  public long PeakUsed => _accessor.ReadInt64(RuntimeEmitter.DsOffPeakUsed);
  public long BufferSize => _bufferSize;

  private DebugStreamDecoder(SharedMapping mapping, MemoryMappedViewAccessor accessor,
      string? familyFilter, string[] tagNames, string[] logNames) {
    _mapping = mapping;
    _accessor = accessor;
    _familyFilter = familyFilter;
    _tagNames = tagNames;
    _logNames = logNames;
    _bufferSize = RuntimeEmitter.DsDefaultBufferSize;
    _bufferMask = _bufferSize - 1;
    _localBuf = new byte[_bufferSize];
  }

  /// <summary>
  /// Create the ring segment, seed its header, and read <paramref name="exePath"/>'s interned name
  /// tables. The caller hands <see cref="SegmentName"/> to the target through
  /// <see cref="RuntimeEmitter.DsActivationEnvVar"/>.
  ///
  /// <paramref name="familyFilter"/> selects one of <see cref="Families"/>, or null for all of them.
  /// It is applied BEFORE an event is formatted, so a filtered run does not pay to build strings
  /// nobody asked for. An unrecognised value is refused here rather than in the decode loop, where the
  /// only available behaviours would be to throw or to silently show everything.
  /// </summary>
  public static DebugStreamDecoder Create(string exePath, string? familyFilter = null) {
    if (familyFilter != null && !Families.Contains(familyFilter))
      throw new ArgumentException(
        $"unknown DebugStream event family '{familyFilter}' (expected one of {string.Join(", ", Families)})",
        nameof(familyFilter));

    // The producer MAPS exactly this many bytes (EmitDebugStreamInit), so it is not the consumer's
    ...
    var mapping = SharedMapping.Create(totalSize, SharedSegmentPrefix);
  ...
  private static void SeedHeader(MemoryMappedViewAccessor accessor) {
    ...
    // Seed the producer's announcement slot to "nobody has spoken". Only the producer writes it, and a
    // producer too old to know it exists leaves it exactly here — which is what makes one detectable
    // rather than merely broken. See CheckProducerSchema.
    accessor.Write(RuntimeEmitter.DsOffProducerVersion, RuntimeEmitter.DsProducerVersionUnset);
  }

  /// <summary>
  /// Decode the committed prefix of the ring, handing every event to <paramref name="sink"/> in ring
  /// order, and release the consumed span back to the producer.
  ///
  /// <paramref name="producerExited"/> must be sampled BEFORE this call reads the cursors. If the
  /// producer is already gone by that sample, then every store it will ever make is in the ring, so the
  /// cursors read here are FINAL. The other order would let a last event land after the check and be lost.
  /// </summary>
  public DrainStatus Drain(bool producerExited, Action<DsEvent> sink) {
    long writeCursor = _accessor.ReadInt64(RuntimeEmitter.DsOffWriteCursor);

    // Pairs with the producer's RELEASE store of write_cursor in __ds_reserve. Everything it wrote
    // below this cursor — every entry HEADER, and the schema version it announced before the first of
    // them — is therefore visible to the checks below. Without this, the scan could read a previous
    // generation's bytes at a ring offset the cursor claims is live, and those bytes carry a stale
    // commit bit and a stale entry_size.
    Thread.MemoryBarrier();

    // Before a single entry is decoded: does the target even speak our schema? A mismatch here is not a
    // trace with a gap in it, it is a trace that is quietly and entirely fictional.
    if (CheckProducerSchema(_accessor, writeCursor) is { } mismatch) {
      SchemaMismatchMessage = mismatch;
      return DrainStatus.SchemaMismatch;
    }

    long committedEnd = ScanCommittedPrefix(_accessor, ReadCursor, writeCursor, _bufferMask);

    if (committedEnd == ReadCursor) {
      // Either the ring is empty, or its head entry is reserved-but-not-yet-committed and its producer
      // is mid-payload. Decoding it now would decode whatever bytes the ring last held at that offset,
      // and advancing past it would lose it.
      if (!producerExited) return DrainStatus.Idle;
      if (ReadCursor >= writeCursor) return DrainStatus.Finished;

      // The producer died between reserving the head entry and committing it — a crash, a panic, a
      // watchdog kill. Nothing will ever fill that payload, so waiting for it is a HANG. Its HEADER is
      // intact (reserve wrote it before releasing write_cursor), so step over it and keep draining the
      // entries behind it, which may well be complete.
      AbandonedEntries++;
      ReadCursor += EntrySizeOf(ReadEntryHeader(_accessor, ReadCursor, _bufferMask), ReadCursor);
      PublishReadCursor(_accessor, ReadCursor);
      return DrainStatus.Decoded;
    }

    // Orders every header load in the scan above ahead of every payload load in the copy below. That is
    // what makes the commit bit mean anything: __ds_commit released the bit AFTER the payload stores, so
    // a reader that has seen the bit and does not reorder past it is guaranteed the payload.
    Thread.MemoryBarrier();

    // Copy the committed prefix out of the ring into the private buffer, then immediately release that
    // span back to the producer.
    long pending = committedEnd - ReadCursor;
    long startPos = ReadCursor & _bufferMask;
    long firstChunk = Math.Min(pending, _bufferSize - startPos);
    _accessor.ReadArray(RuntimeEmitter.DsHeaderSize + startPos, _localBuf, 0, (int)firstChunk);
    if (firstChunk < pending) {
      // Wrap-around: copy the second chunk from the start of the ring buffer.
      _accessor.ReadArray(RuntimeEmitter.DsHeaderSize, _localBuf, (int)firstChunk, (int)(pending - firstChunk));
    }

    long spanStart = ReadCursor;
    ReadCursor = committedEnd;
    PublishReadCursor(_accessor, ReadCursor);

    // Every entry in the private copy is committed, so every payload is whole — the walk below can trust
    // what it reads.
    long localOffset = 0;
    while (localOffset < pending) {
      long header = BitConverter.ToInt64(_localBuf, (int)localOffset);
      byte eventType = (byte)(header & RuntimeEmitter.DsEntryTypeMask);
      long position = spanStart + localOffset;
      int entrySize = EntrySizeOf(header, position);
      uint timestampDelta =
        (uint)((header >> RuntimeEmitter.DsEntryTimestampShift) & RuntimeEmitter.DsEntryTimestampMask);

      // ONE advance for the whole walk. Padding, the depth markers and every filtered-out family all
      // step over the entry by exactly the same amount, and spelling that out per branch is six chances
      // to step by the wrong one.
      if (eventType == RuntimeEmitter.DsEvDepthInc) {
        _depth++;
      } else if (eventType == RuntimeEmitter.DsEvDepthDec) {
        if (_depth > 0) _depth--;
      } else if (eventType != RuntimeEmitter.DsEvPadding && PassesFilter(eventType)) {
        string family = FamilyOf(eventType);
        if (FormatEventFromBuffer(eventType, _localBuf, (int)localOffset, _tagNames, _logNames) is { } line)
          sink(new DsEvent(position, eventType, family, timestampDelta, _depth, line));
      }
  ...
  /// <summary>
  /// Does this event belong to the family the consumer asked for? A null filter means "all of them".
  /// The filter is validated at <see cref="Create"/>, so an unrecognised one cannot reach here.
  /// </summary>
  private bool PassesFilter(byte eventType) => _familyFilter == null || FamilyOf(eventType) == _familyFilter;
  ...
  /// `write_cursor` is RELEASE-stored after that announcement — so a consumer that can see any entry at
  /// all can see the version that produced it. There is no window in which a current producer looks
  /// like an old one.
  ///
  ...
  ///     `--debugstream` never opens the segment, and monitoring one is not an error; it simply has no
  ///     events. Proceed, and print nothing.
  ///   * anything else — a foreign schema. REFUSE, and say so.
  ///
  /// The last case is the one this exists for, because its silent form is so convincing: a v1 producer
  /// never sets the commit bit, so a decoder waits for a commit that will never come, lets the ring fill
  /// until the producer is dropping ~98% of its events, and then steps over every entry as "abandoned
  /// (producer died mid-entry)". Measured: 0 events decoded, 283221 dropped, 5290 abandoned — and the
  /// producer had exited cleanly. Every number in that summary is a fiction, and it is the trace an old
  /// binary produces TODAY. An instrument that lies is worse than no instrument; this is the check that
  /// makes it merely refuse.
  /// </summary>
  private static string? CheckProducerSchema(MemoryMappedViewAccessor accessor, long writeCursor) {
    ...
    return $"[debugstream] SCHEMA MISMATCH — refusing to decode.\n"
      + $"[debugstream]   this consumer speaks v{RuntimeEmitter.DsVersion}; the target speaks {speaks}.\n"
  ...
  /// <summary>
  /// How far below <paramref name="writeCursor"/> the ring is actually READABLE: the end of the run of
  /// entries, starting at <paramref name="readCursor"/>, that carry the COMMIT BIT.
  ///
  /// An entry appears below write_cursor as soon as it is RESERVED, and its payload is written after the
  /// ring lock is released — so "present" and "readable" are two different states, and the commit bit is
  /// the difference. The scan stops at the first entry without it and never looks past: entries are
  /// consumed in ring order, so an uncommitted head briefly blocks the committed entries behind it
  /// rather than being skipped over and lost.
  /// </summary>
  ...
  /// <summary>
  /// The packed 8-byte header of the entry at <paramref name="cursor"/>. An entry never straddles the
  /// end of the ring — __ds_reserve emits a padding entry rather than let one wrap — so this single
  /// read is always contiguous.
  /// </summary>
  ...
  /// <summary>
  /// Total bytes of an entry, header included. Zero is not a legal entry size — __ds_reserve never
  /// reserves one, and every event family has at least a header — so a zero here means the ring is
  /// corrupt AND that a walk keyed off it would spin on this offset forever. Say so.
  /// </summary>
  ...
  ///
  /// The fence is not decoration. The instant this store lands, the producer may overwrite the bytes it
  /// frees — so every load of the copy we just took must be complete before it.
  /// </summary>
  ...
  /// dispatch did not exist — the parser was PE-only, and on a Mach-O binary it read `e_lfanew`@0x3C as
  /// 0, believed the file had 256 sections and a 920-byte optional header, found no `.symtab` in the
  ...
  private static (string[] TagNames, string[] LogNames) ReadNameTablesFromExecutable(string exePath) {
    ...
    // A failure here is LOUD. An executable whose name tables cannot be located is not an executable
    // with no names in it: it is a consumer that will print `tag=1` for every event and a golden that
    // will fail without ever saying why. That silence is what hid this bug.
  ...
  /// meaning belongs to the emitting program's own category/level enums, which the decoder
  /// deliberately does not know. Anything it DOES name — a phase, an event — is an interned index
  /// into MXDS_STRS, so it costs the emitting program nothing to be readable.
  /// </summary>
