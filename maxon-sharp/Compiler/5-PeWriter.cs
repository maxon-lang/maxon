...
public class PeWriter {
  ...
  public static void Write(string path, byte[] code, byte[]? rdata = null, byte[]? data = null, byte[]? ucddata = null, IReadOnlyList<ImportEntry>? imports = null, byte[]? symdata = null) {
    ...
    // The COFF symbol table is deliberately NOT emitted (see docs/DEBUGGER_DESIGN.md): its
    // name->codeOffset payload now lives in the `.mxdbg` sidecar, and nothing in-tree reads it (there
    // is no linker; external-debugger compatibility is out of scope). PointerToSymbolTable and
    // NumberOfSymbols in the COFF header stay 0. The runtime `__symtable` (`.symtab` section, read by
    // the panic backtrace) is a DIFFERENT table and is still emitted.
