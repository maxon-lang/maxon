...
public class CodeGenerator {
	public static byte[] Generate(LirModule module) {
		var encoder = new X86Encoder();
		var regAlloc = new RegisterAllocator();

		// For now, just generate code for main function
		var mainFunc = module.Functions.Find(f => f.Name == "main") ?? throw new Exception("No main function found");
		var allocation = RegisterAllocator.Allocate(mainFunc);
	...
	private static void GenerateInstruction(X86Encoder encoder, LirInstr instr, Dictionary<int, Reg> allocation) {
		switch (instr) {
			case LirMov mov:
