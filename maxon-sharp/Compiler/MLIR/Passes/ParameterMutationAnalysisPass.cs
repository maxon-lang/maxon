...
public static class ParameterMutationAnalysisPass {
  public static void Run(IrModule<MaxonOp> module) {
    ...
    // First pass: local analysis per function. Each iteration writes only to
    // its own `f.ReassignedParams` / `f.MutatedParams`, so this loop is
    // embarrassingly parallel. The second pass below is a fixpoint with
    // cross-function writes and stays sequential.
    ParallelFunctions.Run(module, f => AnalyzeLocalMutation(f, module));

    // Second pass uses the unchanged sequential fixpoint below.
    SequentialFixpoint(module, funcLookup);
  }

  private static void AnalyzeLocalMutation(IrFunction<MaxonOp> f, IrModule<MaxonOp> module) {
  ...
  private static void SequentialFixpoint(IrModule<MaxonOp> module, Dictionary<string, IrFunction<MaxonOp>> funcLookup) {
