...
For the full specification see [LANGUAGE_REFERENCE.md](LANGUAGE_REFERENCE.md) and [BNF_SYNTAX.md](BNF_SYNTAX.md);
for the standard library's API see [STDLIB_REFERENCE.md](STDLIB_REFERENCE.md).
...
The full rules are in the language reference's [Testing](LANGUAGE_REFERENCE.md#testing) section.
...
`.bootstrap/maxon` seeds the first build; afterwards `run build`, run from the root by the compiler
inside this checkout, runs the root `maxon.maxtasks` task that builds it through `maxon-bin/maxon.maxproj`.
...
./maxon-bin/.maxon/maxon.exe build                                # build a target of the .maxproj file in the current directory
...
```bash
./maxon-bin/.maxon/maxon.exe build foo.maxon --log=trace          # all logging
./maxon-bin/.maxon/maxon.exe build foo.maxon --log=parser:debug   # category-specific
./maxon-bin/.maxon/maxon.exe build foo.maxon --log=codegen:trace
./maxon-bin/.maxon/maxon.exe build foo.maxon --metrics=phases.tsv # per-phase time + memory
```
