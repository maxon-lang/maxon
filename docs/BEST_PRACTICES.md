...
let active = users.filter(function(u) gives u.isActive)
```

The parameter types can be left out because `sort` and `filter` declare their callbacks with function types.
...
### Name the Skipped Cases with an `or`-Chain and `break`

When matching an enum or union where you only care about a few cases, you cannot use `default` (it must be `default throws` or `default panic`). Instead, name the remaining cases in an `or`-chain — one alternative per line — with `break` to skip them silently.
...
This is preferable to `default throws` or `default panic` because the unhandled cases are not errors — they are simply not relevant.

**Naming them is the point.** Add a case to `Instruction` and this match stops compiling (E2026), naming the case you have not decided about. A `default` would swallow it, and so would a range: a range covered the cases whose declaration position fell in its span, so a case inserted inside that span was absorbed silently. That is why a case range is refused (**E3146**) and the cases are spelled instead.
