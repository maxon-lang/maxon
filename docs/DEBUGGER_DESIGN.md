...
| P4 (sliced) | The largest rung, split into five sequential slices P4a–P4e, each its own wave. | step + navigate a value tree by path; `break … if`; break one GT while others run |
| P4a | **Value inspection.** Agent `read-mem` command (control segment v3→**v4**: reads target memory at the parked stop into a segment buffer); driver `ReadMemory` + a value layer that resolves the stopped function's named locals via the P2 loclist (`fp + signed slot`) and renders type-aware **value trees** from the sidecar type/field tables (int/ranged/bool/float, `String` as text+length, struct as named-field subtree, enum/union by case, Array/managed as pointer/length), with lazy path navigation (`print a.b.c`). REPL `print`(p)/`locals` + batch JSON, through shared renderers. Activates the P2a/P2b tables (unread at runtime until now). | `--batch` over a sample: `locals` + `print <struct>` render a named-field tree; `print a.b.c` navigates by path; a String prints as text; byte-identical; dark-when-unset; spec 3103/0; empty fragment diff |
| P4b | **Source-line stepping.** `step`/`next`/`finish`/`until <line>` — agent line-granular stepping over the sidecar line table, built on the P3b single-step primitive. | step lands on the next statement boundary; `next` steps over a call; `finish` returns |
| P4c | **REPL ergonomics.** Fuzzy `break Account.withdraw` (qualified/fuzzy function → offset via the func table), tab-completion of funcs/files/locals, "did you mean" on a typo, persistent history + reverse-search. Driver/REPL only. | fuzzy function break resolves; typo suggests; completion lists sidecar names |
| P4d | **Green threads + conditional breakpoints.** `threads`/`gts` list + `gt <id>` switch, per-GT park/resume (closes the P3b stop-the-world residual), `break … if <cond>` (in-proc condition eval by the agent), `--this-gt`/`--stop-others`. | `break … if`; break one GT while others run |
