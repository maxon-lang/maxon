...
| P3b ◑ | **Breakpoints + execution control** (the risky slice). INT3/`BRK` patching into `.text` (W^X page flips; macOS JIT-entitlement), the control command/response mailbox in the segment, park/continue, stop events. ⚠ Contract note from the P3a review: `__dbg_shutdown` must guard the trap handler on `__dbg_base != 0` (or unregister it), because P3b's handler READS the segment — else a post-shutdown fault dereferences an unmapped segment. Also resolves the arm64 `BRK #0` re-trap-loop residual (advance PC / chain to default). | breakpoint hits, parks the GT, continues |
| P3c ◑ | **The `MaxonDebugger` driver + rich REPL core** — spawn/attach over the segment, symbolize via `.mxdbg`, `break`/`run`/`continue`/`backtrace` with the auto source-context window. | source window + GT-aware backtrace render on stop |
