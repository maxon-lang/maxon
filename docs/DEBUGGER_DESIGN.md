...
| P4d | **Green threads + conditional breakpoints + bp-aware stepping.** `threads`/`gts` list + `gt <id>` switch, per-GT park/resume (closes the P3b stop-the-world residual), `break … if <cond>` (in-proc condition eval by the agent), `--this-gt`/`--stop-others`. Also: **`finish`/`next` stop early at a user breakpoint hit during their run** (the P4b-resid #1 refinement — honor "breakpoints win" using the user-bp registry). | `break … if`; break one GT while others run; `finish` stops at an intervening breakpoint |
