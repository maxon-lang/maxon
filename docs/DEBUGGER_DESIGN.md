...
⚠ **AMENDED by P4d-2b (2026-07-25) — the absolute form of this claim is no longer true, and the honest form is better.** Per-GT hold required **one** hook on a scheduler path: a `load __dbg_base; jump-if-zero` at `__gt_dequeue`'s result, the single place the scheduler decides what runs next. **The distinction this invariant is really making survives, but it is a distinction of DEGREE and must be stated as one:** DebugStream instruments *every trace site* plus the alloc/free/refcount paths; the agent has ONE hook, on a path that already performs a queue operation. **Measured, which is why it is still "~zero":** the dark path is **17 instructions** (disassembled from a real binary — two comments had claimed "one load and one not-taken branch"), and its cost is **+0.046% against a NULL CONTROL of +0.070%** (the same binary in both arms, 300 order-randomized pairs) — i.e. **smaller than the measuring harness's own bias**. ⇒ **The rule this invariant now carries: no per-line instrumentation, and any scheduler-path hook must be measured against a null control before it is called free.**
    ...
    metadata only) → codegen                 ├─ dormant debug agent (always emitted)
      ...
      └─ foo.exe.mxdbg  (sidecar, -g only)   │    · in-proc trap handler (VEH/SIGTRAP),
           ...
           · locals (loclists) + types       │    · reads mem/regs; simple in-proc eval
                                              ...
                                              └──────────────┬─────────────────────
                                                   TWO shared segments, TWO env vars:
                                                   MAXON_DEBUG      → control + stops
                                                   MAXON_DEBUGSTREAM → the trace ring
                                                   (one create/open/map helper, not
                                                    one segment — see below)
                                                              │
...
⚠ **"Extends" means a shared MECHANISM, not a shared SEGMENT** — stated here because the sentence above
read the other way and cost P4e a survey to settle. There are **two segments with two activation
variables**: the agent's 4 KB control page (`MAXON_DEBUG`, `__dbg_env_name`) and DebugStream's 2 MB ring
(`MAXON_DEBUGSTREAM`, `__ds_env_name`). What they share is `EmitOpenActivatedSegment` /
`EmitSharedSegmentShutdown` — how a named segment is opened and torn down — and the versioned-handshake
discipline. A driver that wants both must create, set and map **both**; the trace ring is not inside the
control page and never was.
...
### MCP — `maxon mcp`, a subcommand of the compiler itself

⚠ **RULED 2026-07-25 (user), overriding this section's previous content, which said to add tools under
`maxon-dev-mcp/mcp/`.** That was wrong twice over:

- **`maxon-dev-mcp` is for MAXON DEVELOPMENT** — building the compilers, running their spec suites,
  scale-testing. Its tools take `compiler: csharp|shv2` and a `repoRoot`, because their subject is *this
  repository*. A debugger's subject is **the user's own program**. It is a product feature, not dev tooling.
- **It contradicted this document's own architecture.** The line above states: *"One `MaxonDebugger` engine
  is the single brain; every surface (CLI, MCP, DAP, TUI) is a thin front-end over it."* A separate
  Maxon-written server process shelling out to `maxon debug` is not a front-end over the engine — it is a
  front-end over the CLI, with the engine two processes away.

**The surface is `maxon mcp`**, a peer subcommand of `lsp-server` / `debug` / `build`, hosting a tool
registry that is **extensible by FAMILY**. Debug is the first family; **refactoring and building follow**
(user, 2026-07-25). Refactoring has a ready reuse path — `Lsp/RenameHandler`, `PrepareRenameHandler` and
`CodeActionHandler` already exist and are front-ends over the same compiler, so a refactor family is another
surface over one brain rather than a second implementation.

⚠ **The old text justified its batch shape with "MCP is stateless per call." That is FALSE as stated** — an
MCP server is a long-lived process, and the dev-MCP is stateless only because it re-spawns per call. Inside
`maxon.exe`, which already owns the engine, the server **holds a LIVE `MaxonDebugger` session across calls**:
`debug_start` → `debug_locals` / `debug_step` / `debug_eval` / `debug_continue` → `debug_stop`. **That is the
...
second stop-renderer or a second command grammar would be the two-parsers-one-format defect this project has
...
| P4 (sliced) ◑ | The largest rung, split into sequential slices P4a–P4e, each its own wave. **ALL FIVE MECHANISMS NOW LAND** (2026-07-25): P4a value inspection ✅, P4b source stepping ✅, P4c REPL ergonomics ✅, P4d green threads + conditional breakpoints ✅ (itself sliced into P4d-1 breakpoint semantics, P4d-2a visibility, P4d-2b control, and P4d-GT-STACK-A, the OS fault reserve that made a breakpoint on a green-thread stack work at all), P4e DebugStream correlation ✅. **◑ NOT ✅, and the reason is coverage, not missing mechanism** — the enumerated `-resid` rows below are honest limits that no rung has closed: 2- and 4-byte operand widths remain UNREACHABLE (P4a-resid #1, a ranged local collapses to base `i64` before `DebugInfoBuilder` sees it, so that code is emit-only and untested), multi-processor green-thread enumeration is unverified (P4d-2a-resid #1), and the P4e goldens cover the `mm` family on x64 only. ⭐ **Every single slice's optimizer or independent review found a reachable defect the 3103-test suite was green over** — five slices, five finds, the last of them a `next` that silently DELETED its own trace activity. | step + navigate a value tree by path; `break … if`; break one GT while others run |
...
| P4d ✅ | **Green threads + conditional breakpoints + bp-aware stepping.** Delivered in four slices: **P4d-1** (breakpoint semantics), **P4d-2a** (green-thread visibility), **P4d-2b** (green-thread control — which CLOSED the P3b stop-the-world residual), and **P4d-GT-STACK slice A** (the OS fault reserve, without which a breakpoint on a green-thread stack killed the debuggee outright). ⚠ **Slice B** (the g0 cleanup — retiring the per-P 64 KB system-stack region and the TIB repointing it forces on every Win32 call) is a measurable simplification that **does NOT block this phase**; it has its own row. | `break … if`; break one GT while others run; `finish` stops at an intervening breakpoint |
...
| P4e ✅ | **DebugStream correlation.** A stop now shows the trace activity that led to it, and the NEXT stop shows only what happened since. ⭐ **The correlation key is a RING WATERMARK the agent records, not a timestamp**: control segment v9→**v10** adds `DbgOffStopDsMark`, the DebugStream `write_cursor` at the instant `__dbg_publish_stop` publishes, written before the StopSeq release-bump so a driver that sees the sequence sees the mark. A stop PARKS the thread, so any clock reading taken around one describes the debugger rather than the program; a byte position partitions the trace at exactly the instant the stop was taken, and the driver renders `[previous stop's mark, this one's)`. ⭐ **"Not available" is TWO negative sentinels, never 0** — 0 is a legitimate watermark ("nothing traced yet") and rendering it as "no events" is a wrong answer wearing an empty one's costume. `DbgStopDsMarkNoStream` (−1) is a COMPILE-TIME constant the emitter stores when the binary has no hooks (it must be: `__ds_base` is emitted only under `--debugstream`, so referencing it unconditionally would not degrade, it would fail to resolve — and the same branch that makes the agent emittable is what tells the driver to name the flag); `DbgStopDsMarkDetached` (−2) is the runtime case — **PROBED, not assumed**: the driver always creates the ring, so that arm is unreachable in an ordinary session, and it was reached by temporarily withholding the variable and confirmed to produce its own distinct sentence rather than the rebuild-the-binary one. Gated by `DbgTraceMinVersion`, which matters for the v3/v4 reason: a v9 agent leaves the word zeroed and that zero is believable. ⭐ **The decoder is FACTORED OUT, not copied** — `DebugStreamDecoder` now owns the segment header, the schema handshake, the commit protocol, the PE/Mach-O interned-name tables and every event formatter, and `maxon monitor` and `maxon debug` both consume it; a second parser for one wire format is the defect this project already paid for once. The monitor keeps only what a LIVE TIMELINE needs and a stop-slice does not: the `+SSSS.mmm` prefix and the depth indent. ⭐ **The ring is drained inside the stop-wait poll loop**, not at each stop, because the ring is 2 MB and a producer that fills it starts DROPPING — the one liveness sample the loop already takes is reused, so the pump adds no syscall. Losses are reported, never swallowed: the producer's drops, the driver's own window cap, and an entry still being written below the watermark. Also fixed here: **P4d-1-resid #5** — the "kill, wait, THEN join the pipes" pairing was hand-rolled in three places and is now `TargetStdio.EndAndJoin`, whose grace period is the only thing that differed; and the monitor read its name tables AFTER spawning, so a parse failure orphaned the target. `--debugstream` binaries now init the ring BEFORE the agent, so a `--debugstream` binary does not report "no ring" at its own entry stop (two equal-size call sites swapped: no `.text` offset moves). | `trace` golden over `DebugSamples/trace.maxon` (built `--debugstream`): stop 1 shows exactly `label(1)`'s 4 mm events, stop 2 shows only the 12 from the three calls since — and not one of the first stop's; plus the honest refusal on a plainly-built binary, naming the flag. 29/29 golden checks; 3103/0; byte-identical exe (plain AND `--debugstream`); empty fragment diff; 25/25 byte-identical over repeated runs |
| **P4e-perf** | **OPTIMIZER — one commit, one file** (the debuggee's symdata section was located and materialized TWICE — two opens, two PE/Mach-O walks — because the magic was a parameter; now one pass per session). **The headline is DECOMPOSED, because the aggregate misleads:** `--debugstream` under the debugger is **1.80×**, but that is debugger-itself **+99.8 ms** (pre-dates this rung) + hooks-going-live **+109.0 ms = 1.41×** (this rung's real cost for that binary class) + dark hooks **1.26×** (what `--debugstream` already cost). ⚠ **1.80× is the SATURATED figure** — a tight alloc loop emits ~36M events/s against a ~1.6M/s drain, so most events take the cheap DROP path (`traceDropped=859,453` over 2M iterations); **unsaturated the cost is ≈116 ns/event**, with saturation beginning between 8k and 32k iterations. Session footprint **+14.7 ms, +0.56 MB working set, +2.20 MB peak commit** — and this **CORRECTS the implementer's "pages commit on touch"**: true of the 2 MB *mapping* (only a ~128-byte header is written) but NOT of the managed `byte[]`, which is charged at allocation and is essentially the whole +2.20 MB. Decode verified linear (`ScanCommittedPrefix` never rescans, `TraceSlice` O(window) capped at 4096, name tables parsed once per session). **⭐ INVARIANT 2 VERIFIED BY MEASUREMENT — the first rung held to the rule P4d-2b amended it with:** exactly one emitted site, `.text` **+14 bytes plain / +34 `--debugstream`**, and **CONSTANT at both 250 and 4000 functions** — a 16× range. Constant growth is precisely what distinguishes one emitted site from per-function instrumentation. **Deliberately NOT changed, with reasons:** per-event formatting in `Drain` (deferring it forks the shared decoder this rung exists to unify, and draining faster makes the producer drop LESS and write MORE — a fidelity trade, not a speed one); `PumpTrace`'s unbounded drain (bounding it per poll advances `PollBackoff` into 1–4 ms sleeps, raising drops — it changes what the window CONTAINS, so it is outside a perf pass). An apparent 11 ms win was RETRACTED as machine drift rather than published. | `.text` growth constant across 16× code size; no golden moved |
| **P4e-review** ⭐ | **INDEPENDENT REVIEW — three reachable wrong answers, all fixed here, all from the same root: a WINDOW BOUNDARY WAS BEING PLACED AT A STOP NOBODY SEES.** (1) ⭐⭐ **A `step`/`next`/`finish` DELETED its own trace activity.** `AdvanceTraceWindow` ran from `WaitForStop`, i.e. at EVERY stop the agent publishes — and one source-line step is hundreds of them, every one carrying its own watermark and none ever rendered. Each rotated the window and pruned the queue below the new lower bound, so hundreds of windows were opened, closed and discarded unread. **Measured: `break trace.maxon:32; run; next; trace` reported `trace: []` for a line that demonstrably allocated and freed a heap String, and those four events then appeared in NO later window either** — `next` then `continue` to the second breakpoint showed 8 events where the un-stepped control showed 12. Silent: not counted as dropped, not counted as not-retained, no warning. Cured by splitting "where the target is now" (`ReadStopDsMark`, every stop) from "what a window covers" (`OpenTraceWindow`, keyed on the agent stop sequence and called from `TraceSlice`) — **DERIVED, not enumerated**: the only thing that calls `TraceSlice` is a face rendering a stop, so an internal step stop cannot rotate the window and there is no list of step operations to keep in step with the step operations. `trace` twice at one stop still answers twice the same, and the committed golden is byte-identical. (2) **The `trace` command printed "(no trace events since the previous stop)" over a total loss.** `RenderTraceText` returned from its empty-window arm BEFORE the loss line, while the JSON face wrote `traceDropped`/`traceNotRetained` at that same stop — two faces, one question, two answers. **Measured under a concurrent flood: 95,810,410 dropped and 56,122,099 not retained, rendered as "no trace events".** The loss text is now read once and rendered on every arm, and an emptied window says "nothing from this window survived to be shown" rather than "no events". (3) **A stop whose whole window was lost carried NO trace panel at all** — the suppression predicate was `Events.Count > 0`, written TWICE (once per face). Single-sourced as `StopCarriesTrace`, which now also admits a window that is empty BECAUSE it was lost: "nothing happened" and "everything that happened was thrown away" are different answers and only the second owes a warning. Also fixed: `TargetStdio.Kill` swallows the `AggregateException` that `Process.Kill(entireProcessTree: true)` raises for an unreachable descendant — the single-process kill it replaced could never throw one, and it runs from `Dispose`, where it would MASK the exception that ended the session. **VERIFIED BY PROBING, not by reading:** `maxon monitor` transcripts diffed base-vs-branch over the same binary for mm (exact), sched/dbg and log (address-normalized) plus every refusal path — identical; the Mach-O name-table parser confirmed REACHABLE and correct against an independently derived oracle (cross-compiled `arm64-macos`, both blobs found in the one `__TEXT,__const` read); both negative sentinels reached and distinct (`Detached` by temporarily withholding the variable) and a LEGITIMATE watermark of 0 shown to render as `[]` where the sentinels render `null` + reason; a stop taken during a ~150M-event/5.6 s concurrent flood reported 4/4 with a 5 s stop timeout (`PumpTrace`'s unbounded drain delays a stop but cannot lose it — the sequence check precedes the deadline check). | 29/29 goldens; 3103/0; byte-identical exe plain AND `--debugstream`; empty fragment diff; 25/25 on both `trace` blocks |
| P4e-resid ◑ | **P4e residuals** (accepted; none is a wrong answer, and each degrades by SAYING so): (1) **the golden covers the `mm` family only.** `sched`/`dbg` events carry a raw green-thread ADDRESS in their payload and the `log` family (the `__DebugStream` builtin) carries one too, so none can appear in an exact-diff transcript. They decode through the SAME shared formatter — `maxon monitor`'s own output is where they are exercised — so what is untested here is the transcript, not the decode. Fixing it would mean the driver mapping a gt handle to its stable `threads` id inside a formatter the monitor also uses, which is a bigger idea than this rung. (2) **every debug session now creates the 2 MB + 64 KB ring and a 2 MB decode buffer**, whether or not the debuggee has hooks, because whether it has them is a property of how it was BUILT and the driver cannot know before spawning. The pages are only committed as they are touched and an un-hooked binary never opens the segment at all, but the managed buffer is real. (3) **a `--debugstream` binary debugged under `maxon debug` now has its trace hooks ACTIVE**, where before `MAXON_DEBUGSTREAM` was unset and they were dark — that is the feature, and it is also a real slowdown for that one class of binary. (4) **the loss counters attribute to the window they were OBSERVED in**, not the window whose events were lost: the catch-up pump can add a post-watermark discard to the window being rendered. It over-reports rather than under-reports, which is the safe direction for a loss figure. (5) **`traceIncomplete` is not covered by a golden.** It fires only when ANOTHER thread reserved a ring entry below the watermark and has not committed it — reachable under `--stop-others` or several processors, and deliberately bounded (`TraceCatchUpTimeout`) rather than waited on forever, since a producer that is itself parked would never commit. (6) **`trace` has no family filter**, where `maxon monitor` has `--filter=`; the decoder takes one and the debug face simply does not offer it yet. (7) the agent-side store is **x64-verified only**; it is emitted through the platform-neutral builder, so arm64 is emit-verified like the rest of the agent. | — |
| P5 | **`maxon mcp` — the compiler's own MCP server, hosting a LIVE debug session.** ⚠ **Re-scoped by user ruling 2026-07-25**: NOT tools in `maxon-dev-mcp` (that server is for developing the compilers; a debugger's subject is the user's program) and NOT batch-per-call (an MCP server is long-lived; the dev-MCP is stateless only because it re-spawns). A tool registry **extensible by family** — debug first, **refactoring and building to follow** — over one held `MaxonDebugger`. Reuses `MaxonDebugRepl`'s existing renderers by giving `WriteEvent` a sink; adds no second renderer and no second command grammar. See the MCP section for the full ruling. | agent sets bp, gets stop+locals — then decides its NEXT call from what it saw |
...
| P9 | shv2 port (sidecar + agent-in-Maxon + loclists; wasm mechanism). **⚠ PREREQ, found while doing P4d (2026-07-25): shv2's green-thread stacks carry NO OS FAULT RESERVE** — it copied Go's `_StackMin` (2048) and the `928` but not `_StackSystem`, the identical omission P4d-GT-STACK-A had to clear for the bootstrap. There it is WORSE: a CPU fault inside `async` loses its panic ENTIRELY (measured — empty stderr, `0xC0000005`), where the bootstrap's survived by page-granularity accident. **An shv2 agent's first breakpoint on a green thread hits this wall.** Fix it on shv2's ladder BEFORE porting the agent, not during — it is filed there. Secondary: shv2's system stack is ONE GLOBAL region, not per-M, and its safety rests on prose (*"single-M runs one thing at a time"*) that shv2's own multi-M rung B1b falsifies by definition. | shv2 native binary debugs identically |
