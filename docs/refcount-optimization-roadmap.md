...
`mm_incref` / `mm_decref` traffic in emitted code. It is a forward-looking
companion to [MEMORY_MANAGEMENT.md](MEMORY_MANAGEMENT.md); the scoreboard at
[specs/optimizer-refcount.md](../specs/optimizer-refcount.md) is its benchmark.
...
lands so rarely it doesn't show up. Either case is worth investigating before declaring the item
done.
...
are fully eliminated. No leaks; the spec suite green.
...
`String`, `ArrayIterator`, and `__Tuple_…` tags). No leaks; the spec
suite green.
...
check treats that srcVar-decref-in-prefix as a kill and bails. Unlocking those
...
**`EnumDummy`: 12,987 allocations, 100% peak rc = 1. ELIMINATED by #8.** The
`try_call`-on-assoc-enum lowering allocated a dummy, increffed it to rc=1,
`select`ed between real-and-dummy, decreffed the non-selected and freed it —
on every successful call, without the dummy ever being read. It was believed
to be a structural cost of the select-based lowering; it was not a cost of
anything. Scope-end cleanup already emitted a null-GUARDED decref, so storing
the null a `try_call` returns on its error path was always safe. The lowering
now stores it. See item #8 below.
...
firing or the trace was regenerated from a stale cached build. Delete the
compiler's `.maxon/cache` and re-capture.
...
the emit site, so in theory no such incref should be emitted. But edge cases
leak through
...
> `ParameterRetentionAnalysisPass`.
...
### 8. Rethink the `try_call` assoc-enum `EnumDummy` lowering — **DONE**

**Pattern.** `try_call` on a function returning an associated-value enum
allocated a dummy enum, increffed it to rc=1, used `std.select_i64` to pick
between (real-result, dummy), decreffed the non-selected via
`mm_decref_if_nonnull`, and eventually freed whichever wasn't transferred
onward. Every successful `try_call` on an assoc-enum return paid it: the
dummy was allocated, increffed, decreffed and freed **without ever being
read**. By the time the compiler scaling ladder could measure it, it was
~20% of every allocation the compiler made.

**Resolution — no branch needed; no dummy needed.** This item assumed the
cost was structural ("the select-based lowering NEEDS the dummy to be a real
rc=1 allocation") and proposed a branch-based rewrite to escape it. Both
halves were wrong. The premise was that scope cleanup would fault or leak on
a null slot — but scope-end cleanup already emits a null-GUARDED decref
(`EmitDecrefValueIfNonnull`) and every managed slot is zeroed on function
entry, so a null slot was *already* the well-defined "nothing to release"
case. And no branch was needed either, because every `try` form already
branches on the error flag immediately, so the loads and increfs that would
fault on null are all dominated by the success edge; the error edge only ever
null-guard-decrefs the slot.

So the fix was to delete the dummy and store the null. A `try_call` returns
null on error, and null is what ABSENT means:

```
%r = std.try_call @f()          ; null on the error path
memref.store %r, __callret_N    ; that's the whole lowering
```

Landed in `MaxonToStandardConversion.Calls.cs`. Rung 5 of the compiler scaling
ladder: **19,261,978 → 14,518,054 allocations (−24.6%)**, bytes −158 MB.
Allocations and frees fell by exactly the same amount at every rung, which is
the signature of alloc/free round-trips disappearing rather than a leak.

**What it was hiding.** On the ERROR path the dummy was stored into the
callret slot of a routed try-block call — a slot that was never decreffed —
so the dummy *leaked*, once per failing call. Removing it halved the leak and
made the other half visible: `VarRegistry.KeysSince` excluded
`__try_block_result_*` temps from scope-end cleanup even though they receive
the callee's transferred reference and therefore own it. That exclusion was
correct only for struct returns, which were separately handed a `CallReturn`
`__call_tmp_` that turned the downstream alias into a move; an
associated-value union got neither and leaked a reference per routed call.
Both special cases are gone — see `specs/try-block.md`
(`try-block.routed-union-result-is-released`).

The leak had gone unseen because the spec runner discarded the batched
binary's process exit code, which is the only channel `mm_leak_check` has.
That is fixed too (`TestRunner.RunSpecBatch`).
...
Already partially done by `StackPromotionAnalysisPass`.
...
**Safety.** Spelled out in the `ClassifyAliasingOp` docstring.
...
> Landed 2026-04-21 in `RefcountOptimizationPass`.
...
The spec suite stayed green; three previously-passing `list/memory.remove*` and
...
3. **Self-compile, then the suite.** Compiling the compiler allocates
   hundreds of thousands of small structs, so a refcount bug that leaks even
   one allocation per compile shows up as tens of thousands of leaked
   allocations. This is the most sensitive leak detector available — run it
   on every refcount optimization change:
