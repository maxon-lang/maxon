...
same flags and prints the same bytes — pinned by ten fixture projects under
`tests/test-fixtures/`, whose `expected.txt` files are generated by THIS compiler's
`maxon test` and compared against shv2's to the byte, and by the tests in
`tests/test-command/`, which are run under BOTH compilers so neither can be the only
