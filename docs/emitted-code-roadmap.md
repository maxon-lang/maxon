# The emitted-code roadmap — what the codegen does not do yet

**Opened 2026-08-28.** Ranked plan for the **quality of the machine code the compiler emits** — not for the
...
`EC1`'s original ×1.78 CPU deficit is dead.

⇒ **The live deficits are the allocation column** — which still grows with program size, the
signature of a per-element cost — **and the absolute instruction quality** measured directly below,
which is a deficit against good codegen.
...
Post-change baseline, all eight rows in:
...
compiler built from the tree before the first row. **Both binaries built in one
...
**The compiler compiling itself**, which is the one program here that is not a benchmark: the
self-compile takes **77 s**, the fixpoint holds byte-identically (9,033,004 B), and **the whole suite
passes under stage-2** (6,834 / 0).
...
## The optimizer inventory — what the pipeline has and what it lacks

Pipeline: `maxon-bin/Compiler/IR/PassPipeline.maxon:395-413`.
...
| **Constant folding (`const ⊕ const`)** | ✅ `FoldConstants` (EC12) |
...
| Monomorphization | ⛔ **ruled against** (user, EC1) |
| Refcount pair elimination | ⛔ different model — static ownership |
| Register allocator | ✅ linear SSA-chordal + Hall condition |
| **Coalescing-driven operand commute** | ❌ |
| **Instruction scheduling** | ❌ |
| Jump-table dispatch | ◑ dense arms only |
| Short-jump (rel8) relaxation | ❌ always rel32 |
| Target-tier peephole | ❌ **no peephole pass at all** |
| **Unsigned bounds-check idiom** (`cmp`+`jae`) | ✅ `BoundsCompareOperandType` (A2) — one compare, all three lanes |

### Three passes deliberately absent, each with its reason

- **`Mem2Reg`** — the Std tier has **no memref slot for a local**: its only memory ops are
  `loadIndirect`/`storeIndirect` on real addresses (`StdDialect.maxon:884,910`), and locals are
  parser-minted `ValueId`s that are already SSA. The pass would have nothing to promote. *(Confirm before
  building anything that assumes otherwise.)* ⇒ Store-forwarding and dead-store elimination are likewise
  mostly moot for **locals** — but **not** for the `loadIndirect` traffic on struct fields and array
  headers, which is what `EC14` is about.
- **Monomorphization** — ⚖ user ruling 2026-08-25 (`EC1`): keep ONE shared generic body. The answer to
  the generic-`Array` cost is inlining the primitives' fast paths, which `EC1` did.
- **A dominator-based refcount-pair canceller** — ownership is decided statically in the parser, so the
  equivalent work is **not emitting the pair**, which is the open row `EC2`.
...
**MEASURED** on `examples/nbody.maxon`: the compiler emits **10,389 ops with 1,016 `jmp`s, of which 410 (40%)
jump to the physically next block** — 3.9% of the whole instruction stream, every one a taken branch
— and **112 blocks whose only op is a `jmp`**. Attribution of the 410 by block-name family:
...
⇒ **56% of it is debris the compiler's own two inline passes create**, so it grows with every future
inlining win rather than shrinking.

⛔ **THIS ROW'S STATED RISK WAS FALSE.** It read *"the compiler already depends on a fallthrough contract on
x64 (`StdCondBrOp` emits one `jcc` to Else)"*, citing
`[[project_condbr_fallthrough_is_an_x64_only_contract]]`. **MEASURED 2026-08-28: it does not.**
...
block ordering (a later pass may elide a fall-through)."* Block order is semantically free at the
Target tier — which is what makes this rung a DELETION rather than a reordering, and is the whole of
why it is low risk.
...
`specs/branch-cleanup.md` pins the behaviour — the arms an inverted, threaded or dropped branch
must still select, NaN included.

#### `EC12` · `foldConstants` — evaluate `const ⊕ const`

The compiler's own design named this pass and **deferred it** (`FoldConstOperands.maxon:5-7`,
...
[[green-case-proved-nothing-sabotage]] shape.
...
✅ **LANDED 2026-08-28** as `maxon-bin/Compiler/IR/Std/FoldConstants.maxon`, scheduled in
...
ever diverge. It is sound because every integer `binOp`/`cmp` in the compiler computes at the MACHINE WORD on
...
Register pressure is this compiler's known sore point (E5001 is a *diagnostic* the compiler raises when it
cannot allocate), so CSE pays twice: fewer ops, and a narrower live set for the chordal allocator.

⚠ CSE must key on `isPure` (`StdOpMeta`): a `div` is not pure (it traps) and a `call` is not
(effects). ⚠ And it must run **after** `inlineLeaves`, which is what makes identical expressions
co-resident in one block at all.

✅ **LANDED 2026-08-28** as `maxon-bin/Compiler/IR/Std/CommonSubexpressionElimination.maxon`, scheduled
...
- **DOMINATOR-SCOPED**, not intra-block. `StdDominatorTree.maxon` is new and is what `EC14` needs too:
  ...
  8 hits in `cse3.maxon` and ZERO in nbody and fannkuch** — the compiler's own two inliners fragment every
...
- **A CALL IS A BARRIER TO REUSE**, and that rule is the row's second finding. Without it,
  ...
  register`, **for ONE reused expression**. The compiler REFUSES rather than spills, so a CSE that lengthens a
...
would drag across a call; the three pure ADDRESS ops (`globalAddr`,
...
callee-saved registers and that the compiler REFUSES rather than spills).
⭐ **The safety fact this row needs is already proven by its own frontend**: the loop element is a
BORROW of the subject and the borrow is *lexical*, so a call that takes the subject mutably while the
loop is live is refused by the borrow checker (`Parser.maxon:37261-37560` — the ⚖ 2026-08-26 ruling
...
`specs/borrow-liveness.md`). ⇒ **the array header cannot change under the loop, and the compiler
...
`maxon-bin/Compiler/IR/Std/StdDominatorTree.maxon` (successors, reverse postorder, immediate
...
✅ **LANDED 2026-08-28** as `maxon-bin/Compiler/IR/Std/LoopInvariantCodeMotion.maxon`, scheduled in
`buildLoweringPasses` between `commonSubexpressionElimination` and `promoteStackRecords` — the
classical order (`canonicalize → cse → licm → dce`), and for a reason of this compiler's: CSE has already
...
- **PURE COMPUTATION** needs no argument beyond `StdOpMeta.isPure` ("can be duplicated, reordered, or
  ...
  are processed innermost-first and each loop's hoists are applied before the next is analysed.
  `div`/`mod` never reach it (they trap, and the
...
so neither ever reached Rule 1. **In this compiler's loop shapes Rule 2a admits essentially only the loop
...
measured with the new columns, not carried forward. ⚠ **`mgd-call` moved by ONE and that
...
`InlineLeaves` each declared a file-level `let InlineContLabel` / `InlineSlowLabel`, and the compiler resolved
...
The x64 dialect had exactly `loadRegBaseDisp(base, disp)` and `leaRegRegReg(base, index)` at
...
**Scope, and the three conditions that set it** (`specs/scaled-index-addressing.md` is the pin):
...
- **The chain must be in ONE BLOCK.** Cross-block is SOUND (the add's def dominates the memory op, so
  ...
  value's, and the compiler REFUSES rather than spills — EC13 measured exactly that as an `E5001`. **Measured
...
⭐⭐ **BUT THE ROW'S OWN EXHIBIT WAS ALIVE, EXACTLY AS WRITTEN — `Array.clear` REALLY IS
...
**MEASURED — A REAL CONTROL, both binaries built in ONE session and the SAME source compiled by both, so
...
**Cases added** to `specs/array-declared-record.md`, 2, both sabotage-verified against the committed
...
That function's header states the design outright and gives its reason: this compiler's release is STRUCTURAL, so
*"an unconditional acquire is unconditionally balanced and needs no analysis at all"*, where a
whole-function dataflow pass would be the alternative. ⇒ **The remaining EC2 shape is a PAYLOAD-BINDING rule,
...
its compiled binary is BYTE-IDENTICAL under both builds.
...
in both binaries and only the emitted code differs), each self-compiling `maxon-bin`, interleaved
...
`fannkuch-redux`'s binary is **BYTE-IDENTICAL** under both builds — it holds none of these accessors,
...
⭐ **WHAT IT COSTS THE COMPILER, ATTRIBUTED — AND IT IS NEGATIVE.** `--metrics` on both builds
self-compiling `maxon-bin` (the allocation columns are exact and depend only on the LOGIC, so they read
...
✅ **LANDED 2026-08-29** as `maxon-bin/Compiler/IR/Std/StrengthReduceDivision.maxon`, scheduled in
...
⭐⭐ **`specs/strength-reduction.md` USES THE HARDWARE `idiv` AS ITS ORACLE.** Each case divides
...
**Cases added** to the new `specs/strength-reduction.md`, 7, every one of them a DIFFERENTIAL
...
`commuteForCoalescing` swaps a commutative op's operands so the register coalescer can eliminate the
copy. The allocator already coalesces (biased colouring, `RegisterAllocator.maxon:1823-1853`); the row
was filed to make it succeed more often. Its SUBJECT, though, is the wider question — **why are 17% of nbody's emitted ops copies, and which of them need
...
in the compiler's custom convention — `rcx, rdx, rax, r9, rsi, rdi` and `xmm0–5` — is CALLER-saved, so a value
...
in the compiler's own source is 46% copies for exactly that reason:
...
⭐ **ASKING LIVENESS IS THE WHOLE OF THE DESIGN, AND THE CLASSICAL RULE WOULD HAVE MISSED THE ONE
SHAPE THAT PAYS.** The textbook form classifies each operand by its DEFINING OP —
fixed-register-bound, immediate-foldable, or flexible — and swaps only to move a fixed-bound operand
off the left; it never asks liveness. The shape that actually costs a copy here is `EC1`'s inlined
bounds guard, `bitOr(setcc(i < 0), setcc(i >= len))`, whose operands are BOTH "flexible", so that rule
answers *no swap*. This one asks the question that decides the copy instead. The immediate-foldable
half has no analogue here either: `foldConstOperands` has already turned a constant operand into an
`imm` FORM (`andRegImm32`), which is not a two-operand op at all.
...
existence, 0 becomes 1 per iteration, and the compiler REFUSES rather than spills, so the price is an `E5001`
...
`let swap = false`, both binaries built in one session and the SAME source compiled by both:
...
NUMBERS inside `panic at StdToX64Conversion.maxon:1736` — exactly the 12 lines of comment added
...
FOR TWO WEEKS.** `ManagedMemoryRuntime.emitIsNegative` read *"the `StdCmpPred` enum has NO unsigned
...
It is nonetheless the answer to whether a general `or(cmp,cmp) → cond_br` two-jump fusion is worth
adding here: **no**. A source census of the
...
one at all: it short-circuits into blocks. ⚠ And this compiler's own two-sided RANGE CHECK is a CASCADE here,
not an `or` (`InsertRangeChecks.maxon`), so the shape such a fusion exists for does not occur in this compiler
at all. **The one thing worth taking is the `emitSliceRangeViolated`-shaped case, and even
that is 6 instructions in a cold runtime body.**

**TIMED A/B**, two binaries from one tree differing only in the two guard emitters, built in one
session, runs interleaved, both printing identical output:
...
discriminating program had to be built — `specs/union-managed-payload.md` ·
...
`measured=62055`. Without it: **`measured=1085102592571149903`, exit
...
⭐ **The producer header's *"a dataflow pass this compiler does not have"* is STALE IN LETTER AND UNCHANGED IN
SUBSTANCE.** The compiler now has `StdDominatorTree` (`EC13`), `IR/NaturalLoops.maxon` (`EC14`) and
...
tier exists. But that sentence was never the safety argument. The safety argument is that the compiler's release is
...
arbitrary callee releases the caller's box — a whole-function analysis (CFG, RPO, dominators,
...
source, so its producers are only the passes); `EC21` (interval match dispatch — its premise cites a
jump-table pass that does not exist; re-measure before quoting it).
...
- ✅ **CLOSED 2026-08-30 — `f(obj.managedField)` WHERE THE CALLEE FREES THAT FIELD.** The compiler handed a managed
  ...
  **The fix is a RETAIN, not an E3070**, and the narrowing is the whole of its cost.
  ...
  entire compiler + stdlib.** Counted off the two stage-2 IRs, same source, both binaries built in one
  ...
  the compiler stopped self-compiling; `borrow-liveness:a-field-handed-to-a-callee-that-cannot-reach-it` is the
  committed guard on that. **Eight cases** in `specs/borrow-liveness.md`, seven of them RED on the pre-change binary; the eighth
  ...
  owned and `self` read twice after: `n=0 count=1 len=30`, exit 0) — its callees are
...
The classical form is a register-pressure-aware bottom-up list scheduler with a per-target latency
table. This compiler has no equivalent and **no MIR tier to put one in** (3 tiers by design), so it
would sit between SSA destruction and register allocation. ⚠ A scheduler is easy to make expensive:
weigh it against the compile time it costs before building it.
...
The compiler emits `jmp`/`jcc` at rel32 unconditionally (`X64Backend.maxon:157,1125,1149`).
**Code size only** — nbody is 64,296 bytes of code with 1,016 jumps, so the whole
...
this compiler has little to collect — the real producers are inlining, identity folds and proven-redundant
...
- **Compiler-speed work.** `scale-test` and `docs/optimization-log.md` are a different axis; a row
  here is judged by a timed A/B on a program that exercises its shape and by `--emit-ir`, not by the
  ladder.
...
results interleaved. A before/after across a source change is not an A/B — the delta then holds the new
code as well as its effect (see `EC19`'s false reading). Beside it: `scripts/fixpoint.sh` for the
self-compile fixpoint, and `maxon-bin/.maxon/maxon spec-test` for the suite.

⛔⛔ **AND THE COMPILER COULD NOT COMPILE ITSELF, WHICH MEANS THIS WORKSTREAM'S ONE INSTRUMENT COULD NOT
RUN AT ALL.** Found 2026-08-29 while re-measuring `EC17`; three defects, all
landed by rows in this document, none of them visible to the spec suite:
...
../../maxon-bin/.maxon/maxon build nbody.maxon --emit-ir     # writes nbody.ir + nbody.exe
```

Then count `x64.jmp <t>` lines whose next non-blank line is `<t>:`.
⚠ The suite runs every test binary with cwd `temp/`, so that directory is shared with anything else you
park there — stage a binary you must keep somewhere else.

The probes are reproduced in full below so they survive `temp/` being scratch. Maxon needs
...
(`EC12`). The compiler emits `imul`/`lea` three times; the ideal is one `mov rax,97`:
...
and every guard it pays is dead. Measured: **4 of the loop's 11 instructions are the dead guard**,
and the `__il_slow` arm re-issues the CALL, defeating `EC5`'s inlining on this shape:
...
`idiv`-by-constant sites that a program actually pays are in the **float formatting stdlib**
...
nothing — this one formats floats deliberately. 300,000 numbers, **3,586 / 3,554 / 3,559 ms**,
carrying 8 of the corpus's 22 `idiv`:
...
through, so the three copies survive folding and only CSE can remove them. Before `EC13` the compiler emitted
...
**`probe.maxon`** — one function per classical opportunity (`EC12`, `EC18`). The compiler emits
`imul` for `x * 8` and `cqo`+`idiv` for `/ 8` and `/ 10`; folds `x + 0`; does not fold
`2 + 3 * 4`; does not CSE `(a+b)*(a+b)`. It also inlines every one of them into `main`
and dead-function-eliminates the bodies:
