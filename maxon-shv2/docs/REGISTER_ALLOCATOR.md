...
2. **M5's calls halve the pool at a stroke, and the pressure count doesn't see it.** A value live across a call is forbidden every caller-saved register, so its effective pool is the **7** callee-saved, not 14. The splitter must treat a call as a pressure point *against the callee-saved subset* (a call is a one-op region across which idle values are split out — same machinery), and `E5001` must report **7**, not 14, or the message is actively misleading.
