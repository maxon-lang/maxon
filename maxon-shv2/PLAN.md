...
| **P1.7** ◑ | **`Array`** | = P1.6 ∘ P1.2 — the first real integration proof (managed elements → element-destroy through the descriptor). ⇒ unlocks **`b"…"` byte-string literals**. **SLICED A/B/C/D — ◑ ALL BUILDABLE SLICES CLOSED (1011→1162/0, +151).** ✅ **Slice 1** (scalar core, →1079/0, `94b4f667c`) + ✅ **Slice 2** (byte-string, →1100/0, `82e8f35a9`) + ✅ **Slice 3a** (CONCRETE managed elements — element-destroy via `funcAddr`; →1140/0, `15ace5d01`) + ✅ **Slice 4** (slice/append/clone trivial + a systemic negative-index OOB fix; →1162/0, `3c65ccde2`) CLOSED — see the closed boxes at top. ⏭ **Slice 3b (opaque-`T` managed arrays + the `funcAbs64InRdata` reloc) DEFERRED to its own GENERICS-DEPTH rung** (coordinator, 2026-07-22, on an implementer stop-and-report): the reloc's only consumer is a 3-mechanism generic-programming substrate no prior rung built — nested generic-instance typealias in a generic body, opaque-`T` `Array`-typed field, static-method descriptor threading — which are generics-DEPTH (not Array-crux) and are what **P1.7b `Set` / P2 `Map`/`Vector`/`List`** need, so they belong to a rung those depend on (see the "Future rungs" entry + the reloc design map). ⚠ **FOR-USER-REVIEW residuals (two thesis/scope calls the coordinator made autonomously):** ⭐ borrow-on-get `try…otherwise <owned>` bump-masked UAF (incref-on-get vs move-only — a THESIS decision); and this Slice-3b deferral. Other residuals: **⭐ Array slice COW (user ruling 2026-07-22 — `.slice` currently eager-copies O(n); COW wanted, "nearly free" — a Future rung, pairs with E3070/borrow-on-get);** module-level/top-level managed constants; nested-union/`.rawValue`-field-chain/enum-match/>6-param gaps; E3070 borrow-liveness; element_size 8-vs-1 |
...
- **⭐ Array slice COW (USER RULING 2026-07-22 — "COW makes slices nearly free"; number TBD — the parallel repo files #NN concurrently, so assign at sequencing to avoid a collision).** `.slice`/`.clone` currently **eager-copy** (correct — the `slice-cow-modify-*` specs test only independence — but **O(n) per slice**, which the user wants gone). COW: a slice SHARES the parent's buffer (**O(1)**) with `capacity=-1` (nested-slice sentinel) + `parent_ptr`→parent; a `maxon_cow_check` (detach-on-write) runs before every mutation (`set`/`remove`/`grow`/`shift`) and copies into a private owned buffer on the FIRST write of *either* slice or parent. shv2 already reserves `parent_ptr@32` (unused for owned arrays) + `capacity<0` sentinels (byte-strings use −2), so the record shape is ready. Reference to PORT (not reject, this time): bootstrap `LowerManagedMemSlice` + `EmitCowCheck` (`MaxonToStandardConversion.{Strings,ManagedMemory}.cs`), v1 `__managed_mem_slice` / `__managed_mem_cow_check` (`stdlib/Internals.maxon`).
  ⭐ **THE ONE DESIGN FORK a move-only compiler forces (the rung must settle it):** a shared view must keep the parent's buffer ALIVE while it references it. **(A) REFCOUNT the shared buffer** — a bounded, deliberate exception to "no runtime refcount", ONLY for COW-shared array buffers (view increfs, drop decrefs, free at 0). Handles a slice OUTLIVING its parent (`array-slice-managed-elements`'s "source-freed-before-access"). The bootstrap/v1 way. **(B) BORROW-LIVENESS (E3070)** — the slice is a compile-time-bounded borrow that provably can't outlive the parent (no refcount); COW-detach on write. Purer vs the thesis, but forbids slice-outlives-parent unless the detach already fired. ⇒ **pairs with the E3070 residual + the borrow-on-get thesis — one design thread.** The corpus's slice-outlives-source cases lean toward (A) or a hybrid.
