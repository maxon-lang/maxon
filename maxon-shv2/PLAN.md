...
- **⏭ >6 PARAMETERS: THE COMPILER PROMISES A MILESTONE THIS LADDER DOES NOT RECORD.** `function f(a,b,c,d,e,f,g)` is refused with *"Unsupported: more than 6 parameters (M5 passes arguments in registers only; **stack arguments are a later milestone**)"* — and **no such milestone exists anywhere in PLAN.md** (grep for "stack argument" returns nothing). The refusal is clean, so this is a gap in the plan, not a defect in the compiler; it needs a number before anything nontrivial can be written in Maxon.
