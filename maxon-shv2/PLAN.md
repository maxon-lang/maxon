...
| **P1.6** | **generics + layout descriptors** ⭐ | declarations + instantiation. ⇒ `own.drop`'s descriptor arm goes **LIVE** here. **SLICED A/B1/B2/C. ✅ A CLOSED (957→966/0)** generic TYPES over trivial args, front-end only, no descriptor. **✅ B1 CLOSED 2026-07-21 (→989/0)** — `sizeof(T)` via a LIVE layout descriptor READ + the dictionary-passing substrate (shared body + threaded 64-byte descriptor; funcptr slots still 0 so no `funcAbs64InRdata` reloc yet). **B2** (managed-T + drop-through-`destroyFunc@40`: `funcAbs64InRdata` across the 6 targets + WASM funcref-table, destroy-thunk synthesis, revise trivial-scalar classification, remove E2057) + **C** (per-instance ranged typealiases) remain |
