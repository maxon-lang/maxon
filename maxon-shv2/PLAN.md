...
| **P1.8** ◑ **SLICE A DONE** | `String` methods · `for-in` | real `String.equals` body (struct-`cmp` → `methodCall`); hardcoded `for-in` over Array/Range/String. ⚠ **`trim()` lands here and it is the thing that dragged `Set` in** — so P1.7a/P1.7b must precede it. ✅ **Slice A (`for` over ranges + Arrays) COMPLETE 2026-07-28 →1906/0** — see the ladder row. ⬜ B (`Character` + graphemes + `for c in s`), C (byte/ASCII String methods), D (R2/UCD + `CharacterSet` + `trim()` + `Map`). ✅ **E3070 borrow-liveness — the rung Slice A's BORROW ruling owed — LANDED 2026-07-28 (→2003/0)**, sequenced ahead of B/C/D by user ruling because every later slice adds borrow surface onto an unsound base (see Future rungs for the full entry). **B/C/D now stand on a base where mutate-while-borrowed is a compile error.** |
