...
| **P1.0d.5b** | **top-level `var` (GLOBALS)** ⚠ **another one the plan never listed** | **← NEXT.** The corpus found it, exactly as it found parens and block scoping: `specs/short-circuit-evaluation.md` is **0/12, and NINE of the twelve need a global.** **MEASURED: 99 of 276 `/specs` files use one.** ⭐ **Two of the three costs this row used to claim are RETIRED by measurement: initialization order is VACUOUS** (initializers are constant-only — verified: `var b = a + 1` over a **`var`** is `E2004`, while `var derived = BASE * 2` over a `let` works) **so there is NO `__module_init` to build**, and **a managed global is impossible** before P1.2 (no heap ⇒ scalars only). **What is left is STORAGE**: create the Std **memory band** (`StdOpCategory` declares `memory`/`system`; `StdOp` has only `arith`/`control`/`call`) as v1's `globalAddr` + `loadIndirect` + `storeIndirect`; a `.data` section (PeWriter has none); and the **already-declared** `RdataRelocKind.dataSectionRipRelDisp32` / `GlobalLabelClass.dataSection` (`PeWriter.maxon:236` panics *"not supported until its milestone"*). `GlobalDataTable` already names this milestone and says its `DataSectionEntry` **grows back there**. ⚠⚠ **READ `OPEN.md` #17 FIRST: the bootstrap ALIASES two file-private `var`s of one name into ONE `.data` slot — a silent wrong answer — because its resolver is file-scoped while its `.data` label is global-by-name. shv2 MUST key a private global per-FILE, not per-NAME, and "reject duplicates" is FORBIDDEN by `top-level-let.md`. The corpus has NO `var` twin of that case — write it.** A global is how most spec files *observe a side effect*, so expect it to gate a wide slice. **Not merely a parser rule** — it needs storage (rdata/data), initialization order, and, for a managed global, a lifetime. ⚠ It already cost real test coverage: with no globals, short-circuit `and`/`or` had **no way to prove the right operand is SKIPPED** — an eager `and` returns the *same answer* on every input, so no value test can see it. P1.0d.2 had to author a bespoke spec whose guarded operand **divides by zero**, so a clean exit *is* the proof. **Retire that spec when globals land and the real cases can run** |
