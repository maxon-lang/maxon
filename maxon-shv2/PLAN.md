...
- **#108 — ⭐⭐ A POISONING `__mm_free` IS THE GENERAL DETECTOR FOR THE BUMP-MASKED-UAF CLASS — PROMOTE TO A SCHEDULED RUNG (coordinator recommendation 2026-07-22).** `MmRuntime.buildMmFree` reclaims nothing (only decrements the leak counter), so freed memory reads back intact and **no use-after-free is visible to any test.** This began as a note; it is now **proven, and it has already paid for itself twice.** During the #43/#105/#107 fix batch a throwaway ~20–30-line patch to `buildMmFree` (fill the payload with `0x3F` using the header's size word) was used as an instrument and:
  - **confirmed [[#43]] is a REAL read-after-release** (release moved ahead of the blit → `abcabc??????…` under poison, intact without it);
  - **found a pre-existing array-slots UAF** the green suite hid (`try arr.get(i) otherwise <box>`, 4 tests read `0x3F3F3F3F`), which was then fixed under #107;
  - **distinguished a masked DOUBLE-FREE from a leak** (poison writes the payload, not the refcount, so the box read correctly right before both owners freed it — the exit code alone could not tell them apart).
  ⇒ The distinction "leak vs UAF vs double-free" is one **only the poison instrument can draw**, and the fix differs per case, so this is not optional tooling. The bootstrap already has `--mm-debug` (magic/canary/**poison**); shv2 should gain a real **`--mm-poison`** flag (not committed as default — it changes allocator behaviour). It is the general cure for the whole class still on the ladder — P1.2's **#40**, and P1.7's borrow-on-get residual — and it must land **BEFORE** the allocator gains a recycling free list, because after that these become silent wrong answers instead of loud ones. *(Every "release after read" ordering in `ArrayRuntime`/`StringRuntime`/the destructor cascades is currently unguarded by any gate; a spec claiming to pin such an ordering is claiming something it cannot see — the slice-B review proved this by moving `__arr_cow_detach`'s release ahead of its copy and staying 1197/0.)*
