...
- **#31 — a cross-register-class match/ternary EXPRESSION is refused (E2015) instead of promoting int→float.**
  The stated blocker ("float has no nameable type yet") is now FALSE — float F1–F3b landed — so the promotion
  has a consumer. Unify via `arithResultTag` and promote each non-float arm's give in its own exit block before
  its branch to merge. Shared match + ternary via `finalizeMatchMerge`.
- **#81 — value-merge (match + ternary) does not compare function SIGNATURES**, so `unary if c else binary`
  (and the `match … gives` twin) merge silently and run. Its own signature-agreement slice (near #26).
- **#80 — the shared merge diagnostics say "match" for a ternary** (the cross-class E2015 + the give-mismatch
  E3005 fire through `finalizeMatchMerge(matchTok:ifTok)`). Honest reuse, wrong noun; making it construct-aware
  regenerates match+ternary goldens.
...
- **#78 — a wasm function VALUE returning a non-i64-width type traps `call_indirect`** because the
  `callIndirect` StdOp carries no result type (StdToWasm hardcodes `(i64ⁿ)→i64`). Thread the result width to
  the wasm tier. Closure-independent.
...
| **P1.7** ◑ | **`Array`** | = P1.6 ∘ P1.2 — the first real integration proof (managed elements → element-destroy through the descriptor). ⇒ unlocks **`b"…"` byte-string literals**. **SLICED A/B/C/D — ◑ IN PROGRESS.** ✅ **Slice 1 (scalar/trivial-element core) CLOSED 2026-07-22 → 1079/0** (`94b4f667c`; see the closed box at top: synthesized-native `Array`, 48-byte record w/ runtime-polymorphic element_size, ~68 A-cases). Remaining: **Slice 2** byte-string `b"…"`; **Slice 3** managed elements (THE CRUX — `funcAbs64InRdata` reloc + descriptor `destroyFunc@40` + element-destroy walk); **Slice 4** slice/append/clone. Residuals: module-level arrays, value-position `try` on field-chain accessor (both named in the closed box) |
...
- **#68 — statement-position indirect calls.** `parseStatement`'s `callStmt` lacks the
  `scope.contains && varType==function` diversion `parsePrimary` has, so a discarded `f(21)` and a
  `function()` void value both misreport E3004; the spec-stable `typealias Callback = function()` is
  uncallable. Wire `callStmt` like `namedPrimary` + author statement-position/void-call specs.
