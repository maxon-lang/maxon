...
| **P1.7** | `Array` (1/2/3a/**3b**/4 + COW A/B) | ◑ | →1374 | ✅ **Slice 3b COMPLETE for move-IN** — MANAGED opaque-`T` arrays (`Container with String`, `e8b45b613`): `funcAbs64InRdata` reloc fills the descriptor `destroyFunc@40`; opaque-value OWNERSHIP = call-site consume + TRANSITIVE-feed forwarding (`add`→`store`→push) + MOVE-TRACKING (double-move→E3102, conditional-move→runtime descriptor-gated drop). x64-only (wasm/arm64 loudly refuse the reloc). Borrow-on-get UAF resolved (`936b22ab3`). ⏭ **3b-vi = element MOVE-OUT** (pop/get/slice/clone — return-value ownership for the type parameter) remains; + named residuals |
