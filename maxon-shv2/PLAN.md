...
- **⚠ A VALID PROGRAM PANICS THE X64 EMITTER — CROSS-REGISTER-FILE MOVE, and it has MORE THAN ONE manifestation point.** Filed 2026-07-25 by the split-cascade rung's review (found by a 40-program differential fuzz: **18 of 40** hit it) and **coordinator-confirmed on CURRENT `main`**. Repro needs **BOTH** floats live across a call **AND** a ranged-typealias parameter — reading the floats *before* the call compiles fine, a bare `int` parameter compiles fine, and the same source compiles clean for `arm64-macos`: `typealias Num = int(-1e9 to 1e9)`; `function probe(k Num)` with `let f0 = 1.5`, `let f2 = 3.625`, then `acc = work(acc)` and `acc = acc + trunc(f2) + trunc(f0)`. ⚠ **The panic SITE moved between bases** — the review saw `X64Backend.maxon:703` (*"a register-to-register move from rcx to xmm0 crosses register files"*) on `15321679e`, the coordinator saw `X64Backend.maxon:1367` (*"rax is in the gpr register file where the xmm file is required — this encoding would take its low 3 bits and silently name a register in the wrong file"*) on `f58908b59`. **Same class, two different guards catching it**, which means whoever fixes it must not assume one call site. **PRE-EXISTING and unrelated to the split rung** (identical on a rebuilt base; the verify build was silent on all 18, so the incremental split structures agree with a full rebuild on every one). Lands in `RegisterAllocator.maxon` / the X64 backend ⇒ **the same territory as the Wave-2 rung below**, and the two should probably be planned together. Minimal repro kept at `C:\Users\Eric\Dev\splitprobe\min\v3.maxon`.
