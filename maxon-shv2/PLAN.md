...
- **⛔⛔ THE WITNESS LABEL JOIN IS NON-INJECTIVE — A REACHABLE SILENT MISCOMPILE. ⭐ THE NEXT RUNG.**
  ...
  no generic-instance name to reach it.** `__witness_<conformer>_<interface>` joins with a bare `_`, and `_` is a
  legal type-name character, so `(conformer A_B, interface C)` and `(conformer A, interface B_C)` both label
  `__witness_A_B_C`. One table survives; **both dispatch sites read it.**
  **Coordinator-reproduced** (`scratchpad/wit/w.maxon`): two interfaces, two conformers, two `where T is …` holders.
  The right answer is **43** (`3` + `4×10`); shv2 builds **exit 0, no diagnostic, and returns 7** — the program's
  only `*10` never happens, because `HoldB with A` dispatched into `A_B.idc`. `--emit-ir` shows the cause with no
  inference required: `__witness_A_B_C` appears **twice as a reference and once as a definition**. Identical on the
  pre-rung base, so it is pre-existing and this rung neither caused nor worsened it.
  ⚠ **The bootstrap is NOT an oracle here** — it refuses the program outright (`E4006: Primitive type 'int' has no
  method named 'idb'`, a constrained-type-param gap), so this must be settled from the language's own semantics,
  which are unambiguous. Do not plan against an oracle diff that cannot be produced.
  **Why its own rung and not a ride-along:** it is minted in `LowerMaxonToStd.ensureWitnessTable`, outside the
  disjointness rung's entire compiler footprint (`ParseStaging`/`SignatureIndex`/`Project`/`MmRuntime`), and the fix
  is a design decision about the witness namespace's join — escape it, length-prefix it, or move it into the
  reserved space — plus regenerated goldens on four targets. ⭐ **It is the IDENTICAL thesis P1.7 just proved for the
  type namespace, which is the strongest argument for taking it next.** ⚠ Note the asymmetry that decides the
  design: P1.7's contest had a *declaration* to move out of the way, so a re-probe sufficed. Two witness tables have
  no such asymmetry — this needs an **injective** join, not a prefix.
