...
>
> ### ✅ **`P1.2` (String — THE CRUX) WAVE A IS CLOSED (2026-07-16) — string LITERALS + `print` + `==`. `specs-shv2` 508 → 523/0.**
> P1.2 is sliced into 4 waves along the heap-alloc / leak-gate boundary. **Wave A is the ownership-free slice:**
> string literals as **immortal `.rdata`** (a fused 48B record, `capacity=-2`, **NO `__mm_alloc`, no drop** —
> the leak gate is trivially green), `print` (→ `__print_string`, a `WriteFile` runtime fn — shv2's FIRST
> stdout output, ever), and `==`/`!=` (→ `__str_eq`, byte compare). **⇒ NEXT = WAVE B** (primitive
> interpolation → the first OWNED heap String → `__destruct_String` + drop-at-scope-exit — where the leak gate
> and shv2's static-ownership drop model get their real debut), then C (moves) and D (growth).
>
