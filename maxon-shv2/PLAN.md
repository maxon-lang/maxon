...
- **E3070 borrow-liveness** — ⬜ **its own rung, as it always was** (the COW design fork deliberately took (A) buffer-refcounting *over* (B) borrow-liveness; the true mutate-while-borrowed pass was never P1.7's to build).
...
| **P1.8** ◑ **SLICE A DONE** | `String` methods · `for-in` | real `String.equals` body (struct-`cmp` → `methodCall`); hardcoded `for-in` over Array/Range/String. ⚠ **`trim()` lands here and it is the thing that dragged `Set` in** — so P1.7a/P1.7b must precede it. ✅ **Slice A (`for` over ranges + Arrays) COMPLETE 2026-07-28 →1906/0** — see the ladder row. ⬜ B (`Character` + graphemes + `for c in s`), C (byte/ASCII String methods), D (R2/UCD + `CharacterSet` + `trim()` + `Map`). ⚠ **E3070 borrow-liveness is sequenced AHEAD of B/C/D by user ruling** (see Future rungs) |
