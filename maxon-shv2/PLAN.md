...
- **⭐ Array slice COW — an IMMEDIATE P1.7 RESIDUAL, BUILD NEXT (NOT a future defer; USER RULING 2026-07-22: "COW makes slices nearly free… worked in as early as possible, a residual if it can be implemented immediately"). ⇒ It CAN be implemented immediately via mechanism A (no deferred prerequisite), so it is a residual, not a Future rung.** (Number TBD — the parallel repo files #NN concurrently, so assign at sequencing to avoid a collision.) `.slice`/`.clone` currently **eager-copy** (correct — the `slice-cow-modify-*` specs test only independence — but **O(n) per slice**, which the user wants gone). COW: a slice SHARES the parent's buffer (**O(1)**) with `capacity=-1` (nested-slice sentinel) + `parent_ptr`→parent; a `maxon_cow_check` (detach-on-write) runs before every mutation (`set`/`remove`/`grow`/`shift`) and copies into a private owned buffer on the FIRST write of *either* slice or parent. shv2 already reserves `parent_ptr@32` (unused for owned arrays) + `capacity<0` sentinels (byte-strings use −2), so the record shape is ready. Reference to PORT (not reject, this time): bootstrap `LowerManagedMemSlice` + `EmitCowCheck` (`MaxonToStandardConversion.{Strings,ManagedMemory}.cs`), v1 `__managed_mem_slice` / `__managed_mem_cow_check` (`stdlib/Internals.maxon`).
