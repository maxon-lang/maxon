...
>
> ### ✅ **`P1.2` (String — THE CRUX) WAVES A + B ARE CLOSED (2026-07-16) — literals + `print` + `==` (A), then primitive INTERPOLATION → the first OWNED heap String + the drop model (B). `specs-shv2` 508 → 523 (A) → 564 (B).**
...
> - ⇒ **REMAINING owned-value follow-ups (both P1.2 → their own rungs): OPEN #40 (borrowed→owned in a nested block — masked UAF, the height-stack can't own SHALLOWER than the block; depth-model rework) and OPEN #42 (use-after-move enforced only at bare reads, NOT field access / method / field STORE — a moved struct read/written through its source is unchecked; needs an all-read-site guard + a field-store-revive ruling). Do #42 before a recycling `__mm_free`. ⇒ NEXT on the ladder = WAVE D (growth/`append`/detach + `__destruct_String`).**
>
