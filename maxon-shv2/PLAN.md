...
| **P1.7** | `Array` (slices 1/2/3a/4 + COW A) | ◑ | →1184 | slice 3b (opaque-`T` managed) DEFERRED to a generics-depth rung; ✅ COW **Slice A** (detach-on-write, 5 segfaults fixed) CLOSED; ⏭ COW **Slice B** (zero-copy slices) is the live residual; + named element/index residuals |
...
- **⭐ Array slice COW — SLICE A ✅ CLOSED 2026-07-22 (`12705a7d9` + review `66bd13c24`, 1162→1171, 1184/0 after rebase); SLICE B (zero-copy slices) IS THE REMAINING P1.7 RESIDUAL.** (Number TBD — the parallel repo files #NN concurrently, so assign at sequencing to avoid a collision.) *(User ruling 2026-07-22: "COW makes slices nearly free… worked in as early as possible, a residual if it can be implemented immediately".)* **SLICED A/B by the coordinator after the survey, because A fixes live crashes and is a strict prerequisite for B.**
  ...
  - ⏭ **SLICE B — ZERO-COPY SLICES (the remaining residual).** `.slice`/`.clone` still **eager-copy**: correct (the `slice-cow-modify-*` specs test only independence) but **O(n) per slice**, which the user wants gone. Design: a slice SHARES the parent's buffer (**O(1)**) with a `capacity=-1` view sentinel + `parent@32` → the shared allocation's BASE pointer (a view's `buffer@0` is INTERIOR, so the base is what `__mm_decref` needs); detach-on-write fires for *either* side. Slice A was built so B adds its trigger as **another arm of the SAME test, without moving the check or touching a call site**.
