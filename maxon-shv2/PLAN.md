...
- **#104 — the runtime ABORT exit-code space has no single home, and it has already collided.**
  `GtRuntime.ProcStoreOverflowExitCode = 70` and `ArrayRuntime.ElementSizeZeroExitCode = 70` are the same code for
  unrelated aborts, each picked against a different local list — the 4-digit *error* codes have `docs/error-codes.txt`
  and a check that fails the build on a duplicate, and the abort codes have nothing. Nothing is broken today (COW's 73
  collides with nothing), but the next abort added is a coin flip. Wants one registry + a check, the shape
  `error-codes` already proves. Related: `emitVoidAbort`'s `osExit`+dead-terminator fact is still open-coded twice more
  in `GtRuntime.maxon:1270-1274` and `:1556-1559` (cross-file, so slice A could not single-home it).
- **#105 — String and Array answer "is this buffer mine?" with OPPOSITE fields, and Slice B is about to add a third
  encoding.** String uses `parent@32` (`DetachedRootParent = -1`, read by `__str_decref`); Array uses the sign of
  `capacity@16` (`-2` rdata) while its own `parent@32` sits reserved-and-unwritten; and String's
  `RdataCapacitySentinel = -2` is *written and never read as a test*. Three encodings of one question across two
  runtimes. **Decide the single encoding BEFORE Slice B invents a fourth for "shared heap buffer"** — this is the
  file's own signature bug (ONE FACT WRITTEN TWICE) waiting at a seam that is about to be widened.
- **#107 — `try f().throwingMethod()` is wrongly REJECTED (E3055); the oracle accepts it.** `let s = try make().slice(1, endIndex: 3) otherwise return 9` ⇒ shv2 `error E3055: try requires a throwing function: 'make' does not throw`, bootstrap compiles and returns 2 (coordinator-verified both ways). The `try` handler binds the **FIRST** callee in the expression rather than the outermost/throwing one. **Array-independent in shape** — any `try f().throwingMethod()` hits it — and it lives in `Parser.maxon`, so it belongs to the `try`/errors subsystem (P1.4b), not the array runtime. shv2 rejecting a VALID program is the wrong direction of divergence. *(Found by the COW slice-B rung, which restructured its probe around it rather than touching the parser.)*
- **#108 — ⭐ NO UAF IN THIS COMPILER IS DETECTABLE BY ANY TEST, because `__mm_free` reclaims nothing.**
  `MmRuntime.buildMmFree` only decrements `__mm_alloc_count` (the leak gate's counter); the bytes are never
  reused or poisoned, so freed memory reads back intact. **Demonstrated, not theorized:** the slice-B review
  deliberately moved `__arr_cow_detach`'s buffer release *ahead* of its byte copy — a genuine
  read-after-release — and the suite **stayed 1197/0**. ⇒ Every "release after read" rule in `ArrayRuntime`,
  `StringRuntime` and the destructor cascades is currently **unguarded by anything**, and a spec claiming to
  pin one is claiming something it cannot see. **This is the GENERAL CURE for the bump-masked-UAF class already
  on this ladder** — P1.2's **#40** and **#43**, and P1.7's borrow-on-get residual, are all "correct only
  because the allocator never recycles". The bootstrap already has `--mm-debug` (magic/canary/**poison**);
  shv2 has no equivalent. A poisoning or debug-only free would turn that whole class from latent into loud.
  Its own rung; do it BEFORE the allocator gains a recycling free list, not after.
...
- **#102 — THREE array-over-a-`b"…"`-literal wrong answers, all found 2026-07-22 by the P1.7 COW slice-A rung while
  using the bootstrap as its oracle. shv2 is RIGHT in all three; the bootstrap is the one that errs.**
  (Number assumes #100/#101 from `2cbe4f616`; the parallel repo files #NN concurrently, so re-check at sequencing.)
  - **`insert` LOSES THE TAIL — a silent wrong answer, coordinator-verified.** `var a = b"hey"; a.insert(1, value: 88)`
    ⇒ bootstrap prints `104 88 0 0` (the `e`,`y` overwritten with ZEROS), shv2 prints the correct `104 88 101 121`;
    `count() == 4` in both, so nothing announces the loss. This is the worst of the three: it corrupts data with a
    green exit.
  - **negative `resize` has no coherent answer**: `[10,20,30].resize(-1)` PANICS (`Array.resize: setLength OOB —
    reserve ensured capacity for newLength`, exit 1) but `b"hey".resize(-2)` silently publishes `count() == -2`,
    because its own growth check is satisfied by the negative rdata sentinel so the assert is never reached. Both
    halves agree a negative length is invalid; only one acts. shv2 aborts (exit 73) uniformly.
  - **`resize(0)` on a literal panics** `__ManagedMemory: realloc size must be > 0`; shv2 handles it.
  ⇒ All three are the same root shape as the bootstrap's COW design: **its `capacity` sentinel is consulted by
  arithmetic that was written for non-negative capacities**, so a negative sentinel silently satisfies a `>=` test.
