...
| **P1.6** | generics + layout descriptors (A/B1/B2/C) | ◑ | →1951 | #94 (generic-instance union payload match-bind) → Future rungs. ⚠ **REOPENED ✅→◑ 2026-07-28 by the BARE-TYPE-ARGUMENT rung**, which found P1.6-A admits things that reach a COMPILER PANIC. Bare/float type arguments are now refused (E2061/E2062, below) — but **`X64Backend.maxon:715` is still reachable with a LEGAL type argument** (`Box with Integer` … `create(1.5)`, a float VALUE at a `T`-typed formal), so the mechanism is not done. Three residuals below; ✅ over a known reachable panic would be the expensive lie this plan exists to prevent |
