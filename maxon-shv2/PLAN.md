...
| **P1.7** ◑ | **`Array`** | = P1.6 ∘ P1.2 — the first real integration proof (managed elements → element-destroy through the descriptor). ⇒ unlocks **`b"…"` byte-string literals**. **SLICED A/B/C/D — ◑ IN PROGRESS.** ✅ **Slice 1** (scalar/trivial core, 1011→1079/0, `94b4f667c`) + ✅ **Slice 2** (byte-string `b"…"`, →1100/0, `82e8f35a9`) CLOSED — see the closed boxes at top. Remaining: **Slice 3** managed elements (THE CRUX — `funcAbs64InRdata` reloc + descriptor `destroyFunc@40` + per-slot element-destroy walk); **Slice 4** slice/append/clone. Residuals (named in the boxes): module-level/top-level managed constants (arrays + `b"…"`), value-position `try` on field-chain accessor, bare `ByteArray` annotations, element_size 8-vs-1 unification |
