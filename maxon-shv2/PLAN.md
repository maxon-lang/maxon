...
| **P1.6** | **generics + layout descriptors** ⭐ | declarations + instantiation. ⇒ `own.drop`'s descriptor arm goes **LIVE** here. **SLICED A/B/C. ✅ A CLOSED 2026-07-21 (957→966/0)** — generic TYPES over trivial args, front-end only, NO descriptor (shared bodies; trivial-arg instantiation ≡ monomorphic IR; managed args rejected E2057). **B** (descriptor + managed-T + dictionary-passing + drop-through-`destroyFunc@40` + `sizeof(T)`) + **C** (per-instance ranged typealiases) remain |
