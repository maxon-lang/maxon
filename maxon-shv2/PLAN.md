...
| **P1.7** | `Array` (slices 1/2/3a/4 + COW A/B) | ◑ | →1199 | ✅ COW **A+B** CLOSED — `.slice` is O(1) (469 ms → 15 ms), 5 segfaults fixed; slice 3b (opaque-`T` managed) DEFERRED to a generics-depth rung; borrow-on-get UAF = user thesis call; + named element/index residuals |
