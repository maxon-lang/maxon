...
| **P1.1b** | **enums + `match`** | *(was the enum/match half of `P1.1`.)* **SLICED into two waves 2026-07-16** (the corpus survey found it too big for one). **WAVE A ✅ CLOSED 2026-07-16** — `match` on SCALARS (int/bool/float), statement + expression, value/or/range/default, block-arg `gives` join; **399→462**; see the closed box at top. **⇒ WAVE B ← NEXT: payload-free `enum` + `union` + ENUM MATCH + ordinal RANGE ARMS.** **HEAP-FREE**: both references collapse a payload-free enum to an `int` typealias + tag constants; enum match rides Wave A's condBranch chain (**never bend `IrBlock.CondBranch`'s one-branch-per-block invariant**). Payload-free `union` = enum (identical path). ⭐ **RANGE ARMS USE ORDINAL ORDER** — declaration order, **NOT raw value** (user ruling, 2026-07-15): a range arm is the **OR of the cases it covers by declaration index**, kept as a two-compare range only where no *uncovered* case's tag falls between the covered extremes. ⚠ **v1 does NOT have this fix — its range code is the OPEN #21 bug; the bootstrap (`645a690cc`) is the ONLY correct reference.** shv2 stores cases in a parallel-column `EnumLayout` (index = declaration index, a `tags` column = the tag) so "one field two meanings" is unrepresentable. See OPEN.md #21 |
