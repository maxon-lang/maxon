...
| **P1.0d.4** | **floats (f64)** | **← NEXT, and it is the LAST slice of P1.0d.** **The deep one.** There is **no XMM/float register class in the allocator at all**, so this is a whole new register bank (+ SSE emitters, float rdata, `NumberParsing` already lexes the literals). 67 corpus cases. ⚠ **Two ready-made acceptance cases are already sitting `disabled-test:` behind it**: `top-level-let/basic-float-constant` and `static-variables`' two `data-section-f64-*` (whose `RequiredData` needs float parsing — `int.fromString` fails). ⚠ **The frame alignment is now load-bearing for the first time**: Win64 requires RSP 16-byte aligned **at** every call and `push rbp` inverted the parity (P1.0d.3) — **an SSE spill is exactly what turns a latent misalignment into a fault**, and nothing before floats could have exposed it |
