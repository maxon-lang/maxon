...
| **P1.5** | closures + `async` + escape → `shared` | ◑ | A1 →813 | A1 core landed (#67 arm64 open); async R3 substrate landed; residuals #64/#73/#74/#75/#79 (front-end; ✅ #65 implicit-self CLOSED 2026-07-23 `55cafda11`) + #88/#89/#93 (runtime; ✅ #87+#92 GT/scratch reclamation CLOSED 2026-07-23 B1c `b9bc8c98b`); B2c/C remain (B1c ✅) |
...
| R3 | GT scheduler + async substrate | ◑ | P1.5 — substrate landed; #88/#89/#93 open (✅ #87+#92 reclamation CLOSED 2026-07-23 B1c) |
...
- **#88 — a completed GT whose promise is never awaited leaks its struct (+ a never-run GT leaks its 2 KB seed
  + struct).** B1c reclaims at AWAIT, so the AWAITED case is now leak-free; what remains is the un-awaited tail:
  `let p = async f()` (never runs → leaks the 2 KB seed + struct) AND a GT completed out-of-order but whose
  own promise is dropped (runs, but never reclaimed → leaks the struct). Both are `__slab`-invisible (the GT
  leak-counter was removed by ruling, so this is silent). Needs linearity/drain — a "promise must be awaited"
