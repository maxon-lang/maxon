...
> - **OPEN #42 (use-after-move at field/method/store sites) ✅ FIXED (`51f543fdd`, 597→602)** — one shared `requireBindingLive` guard at the two funnels; sabotage-proven by the review. ⇒ **REMAINING owned-value follow-up: OPEN #40 (borrowed→owned in a nested block — masked UAF; the height-stack can't own SHALLOWER than the current block; a depth-model rework, its own rung). ⇒ NEXT on the ladder = WAVE D (growth/`append`/detach + `__destruct_String`).**
>
