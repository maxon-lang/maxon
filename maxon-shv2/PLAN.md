...
| **P1.2** | `String` + ownership + drops (waves A–D) | ◑ | →611 | ✅ #43 (self-append UAF) CLOSED with #105. **#40 RE-HOMED to its own rung (user ruling 2026-07-23)** — it is a reachable UAF in P1.2-depth-model × P1.4a-borrowed-aggregate-PARAMS, not a P1.2 String residual; the fix carries a thesis ruling (retain vs drop-flags). See Future rungs |
...
>
> ### ⇒ **REMAINING P1.2 owned-value follow-up:** **#40 — RE-HOMED to its own rung (user ruling 2026-07-23)**, because it is NOT a P1.2 String residual: it is a REACHABLE UAF in the interaction of P1.2's reassignment depth-model with P1.4a's borrowed managed-aggregate PARAMS (`var q = <borrowed struct/union param>`; `q` conditionally reassigned to owned in a nested block → the owned value drops at the inner block while `q` lives on). The naive "drop at declaration scope" fix ships an else-path double-free; the correct fix (retain-promotion vs drop-flags) each break a thesis invariant → a design ruling the new rung carries. **See the "#40 → conditional ownership" entry in Future rungs.** Poison-on-free (#108) is the standing detector. **P1.2 stays ◑** with #40 re-homed. **✅ OPEN #43 (`s.append(s)` self-append UAF) — CLOSED 2026-07-22** (main `0be8c7465`, with #105): `__str_append` released the old buffer BEFORE the blit read the aliased source; the release now rides a block arg past both copies. **Proven a REAL read-after-release** with a poisoning `__mm_free` (release-ahead-of-copy → `abcabc??????…`; fixed → `abcabc`), not merely inferred — see [[#108]].
>
