...
| **P1.7** | `Array` (1/2/3a/**3b**/4 + COW A/B) | ◑ | →1384 | ✅ **MANAGED opaque-`T` arrays — move-IN + element read/move-OUT COMPLETE** (`Container with String`): move-in `e8b45b613` (reloc `destroyFunc@40` + ownership: call-site consume + TRANSITIVE-feed forwarding + MOVE-TRACKING double-move→E3102 / conditional→runtime drop); **✅ 3b-vi-a `18b64f22c`** = get/first/last BORROW + pop/remove OWNED move-out (drop-in-body via `__drop_type_param`; **returning** an owned opaque element out of the method is cleanly E2015-rejected — a review-found leak). x64-only. 🔴 **BARE opaque-`T` FIELD reassignment (`self.saved = x`) CRASHES 0xC0000005 on main — a REACHABLE 3b-v defect** (`computeTypeDescriptorNeeds` doesn't reserve the descriptor for a bare-opaque-field drop-on-reassign → `__drop_type_param` with no descriptor); distinct from arrays, found by the 3b-vi-a review — **FIX NEXT** (reserve the descriptor, symmetric to the array move-out signal). ⏭ **3b-vi-b = slice/clone/append** (needs the deep-clone primitive P1.2 deferred). Borrow-on-get UAF resolved (`936b22ab3`) |
