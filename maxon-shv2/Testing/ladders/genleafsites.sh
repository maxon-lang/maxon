...

{
	...
	case "$MODE" in
	onefunc|spread)
		echo "function leafPure(a int, b int) returns int"
		echo "	return ((a + b) * 3) - 1"
		echo "end 'leafPure'"
		;;
	nosites)
		...
		echo "function leafSeed(a int) returns int"
		echo "	return a + 1"
		echo "end 'leafSeed'"
		echo "enum LeafErr"
		echo "	never"
		echo "end 'LeafErr'"
		echo "function leafPure(a int, b int) returns int throws LeafErr"
	...
	guardone|guardspread)
		...
		echo "function leafPure(mask int, r $GUARD_ALIAS) returns int"
	...
	onefunc|nosites|guardone)
		...
		# moves ONE axis.
		echo "function leafMany(a int) returns int"
		echo "	var acc = 0"
		for ((i = 0; i < N; i++)); do
			case "$MODE" in
