...
types|sites)
	...
	# `types` gives every site its own type; `sites` pins them all to type 0.
	{
		...
		for ((i = 0; i < N; i++)); do
			...
			echo "function tp$i() returns int"
			echo "	let t = $lit"
			echo "	return t.0"
			echo "end 'tp$i'"
		done
...
arity)
	...
	# statement so exactly one tuple element is live at a time.
	{
		...
		echo -n "function tpWide() returns int"
		echo ""
		echo -n "	let t = (1"
...
access)
	...
	# count is pinned and only the access count sweeps.
	{
		...
		echo "function tpRead() returns int"
		echo "	let t = (1, 2)"
		echo "	var acc = 0"
...
files|fileset)
	...
	for ((i = 0; i < N; i++)); do
		if [ "$MODE" = "files" ]; then ty=$(tuple_type "$i"); else ty=$(tuple_type 0); fi
		{
			...
			echo "function tf$i(t TP$i) returns int"
			echo "	let q = t.0"
			echo "	let xs = [q]"
			echo "	return xs.count() as int"
			echo "end 'tf$i'"
		} > "$OUT/b_f$i.maxon"
	done
...
nest)
	...
	# O(n^2) bytes over the ladder while the source text is O(n).
	{
		...
		echo "function tpNest() returns int"
		echo -n "	let t = (1"
		for ((i = 1; i < N; i++)); do echo -n ", (1"; done
		echo -n ", 1"
