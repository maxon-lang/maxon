...
# The cure was to answer with a `BlockRef` (an LadderInt) instead of a union — see `expandAt`'s header.
...

{
  ...
  echo "typealias LadderInt = int(i64.min to i64.max)"
	echo "function mpOpaque(a LadderInt) returns LadderInt"
	echo "	if a > 100 'big'"
	echo "		return a - 100"
	echo "	end 'big'"
	echo "	return a + 1"
	...
	onefunc)
		...
		echo "function mpMany(a LadderInt) returns LadderInt"
	...
	spread)
		echo "typealias MpByte = int(0 to u8.max)"
		for ((i = 0; i < N; i++)); do
			echo "function mpOne$i(a LadderInt) returns LadderInt"
			echo "	var mm = try __ManagedMemory.create($SLOTS, elementSize: 1) otherwise return 0"
			echo "	try mm.setLength($SLOTS) otherwise return 0"
			echo "	try mm.set($((i % SLOTS)), (mpOpaque(a + $i) and 255) as MpByte) otherwise return 0"
		...
		;;
	nosites)
		echo "function mpNone(a LadderInt) returns LadderInt"
		echo "	var acc = 0"
		for ((i = 0; i < N; i++)); do
