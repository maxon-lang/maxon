...
| `genrangesites.sh <n> <onefunc\|spread> <outdir>` | `n` field stores of a NON-CONSTANT value through a narrow ranged alias, all in ONE function (`onefunc`) or one per function (`spread`) | the ALIAS-NAMED range-check sites of P1.9 — `guardSiteAt`, the guard CHAIN and the split that carves it. ⛔ **`ScaleCorpus` declares narrow ranged aliases and reaches a guard with NONE of them**, which is the hard kind of blindness: 128 `ScaleXElem<N>` aliases are only ever generic TYPE ARGUMENTS (erased before any door sees them), every alias at a real door is `int(i64.min to i64.max)` and `rangeIsFull` discards it, and the one narrow alias at a real door sits at exactly ONE `as` cast that does not double. **MEASURED: the rung0 and rung5 binaries each contain the SAME single range-check panic blob** — a 32× corpus emits one guard, so every column of a default run has priced the RECORDING of vacuous sites and has never once priced the EMISSION of a guard. `spread` is the CONTROL: same site count, same guards, different per-function concentration, so a bend on `onefunc` alone is the sites×blocks product term. |
| `genretsites.sh <n> <onefunc\|spread> <outdir>` | `n` guarded `return` sites, each behind its own `if`, all in ONE function and all returning the SAME parameter (`onefunc`) or one per function (`spread`) | the ranged-`return` sites — `retBlockOf` and `splitBlockInPlace`. **The other half of `genrangesites.sh`'s axis and blind to it both ways**: no store or cast makes a function RETURN a narrow alias, and `ScaleCorpus` has not one non-full ranged RETURN type, so this whole path reads Δ0 at every rung of the standing instrument. ⭐⭐ **It is the ladder that found `findRetBlock`'s per-site scan, and it found it in the CPU COLUMN ALONE**: x2.21 x2.53 x3.20 per doubling at n = 50/100/200/400 — a RISING ratio — while allocations and bytes read a dead-flat x1.99 / x2.03 over the same span, because a scan allocates nothing. Read the CPU column FIRST here. Cured (A1h, corrected by its review) to x1.96 x1.94 x2.03. Every rung's binary must EXIT 7, so a miscompile shows as a wrong exit code and not only as a time. `spread` is the CONTROL, for `genrangesites.sh`'s reason. |
