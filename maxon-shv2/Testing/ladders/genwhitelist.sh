#!/usr/bin/env bash
# THE STDLIB-WHITELIST LADDER — the one question the SHARED corpus structurally cannot ask:
#
...
# authors were alert to exactly this class; this ladder asks whether a SECOND instance exists one
# phase downstream.
#
...
# shared corpus DOES exercise the cost — every rung loads the same 3 whitelisted modules — but it
# moves BOTH axes at once and can only ever move one of them (the corpus doubles; the module count is
# fixed at whatever the whitelist says that day). A product term and a sum term are indistinguishable
...
# `StdlibLoader`'s header states the mechanism outright: a whitelisted module "is registered into the
# query database (`fileChanged`) exactly like a user source, so it flows through the SAME tokenize ->
# signature-index -> parse -> merge spine with no query-spine change." A stdlib file is not a special
# kind of input; it is an input. So the question generalizes, with nothing lost, to *"does parse cost
# scale with the product of user code and total declarations in the program?"* — which is measurable
# with plain files and no compiler edit, at any module count, including counts no whitelist has.
#   ⇒ The confirmation that the generalization holds is a REAL whitelist A/B — rebuild shv2 with an
#     entry REMOVED and diff the two binaries over one corpus. Done the day this was written, and the
#     two routes agree to FOUR ALLOCATIONS OUT OF 1,650: the real `helpers/string/utf16.maxon` dropped
#     in as an ordinary USER file (this ladder's `real` mode) costs **+1,650** `phase:parse`
#     allocations, flat at corpus rung 0 AND rung 5; the same file reached through the WHITELIST costs
...
#
# Usage: genwhitelist.sh <n> <mode> <outdir>
...
#
#   e.g. MODULES=0  genwhitelist.sh 256 synth p0/     # the CONTROL: corpus alone
#        MODULES=16 genwhitelist.sh 256 synth p16/    # same corpus, 16 extra modules
#        MODULES=1  genwhitelist.sh 512 real  pr/     # the real utf16.maxon, once, over a 512-fn corpus
#
...
#
#   ⭐ **THE REAL WHITELIST ENTRY BEHAVES IDENTICALLY, MEASURED THREE-BINARY.** `head` (the slice),
#   `nowl` (the slice with ONLY the `helpers/string/utf16.maxon` line removed) and `base` (the parent
#   commit) were built by the same bootstrap and driven interleaved from equal-length paths. `head -
#   nowl` — the whitelist entry and nothing else — is **+1,654 `phase:parse` allocations at EVERY ONE
...
#
#   ⚠ **WHAT DID GROW IN THAT RUNG'S DELTA COLUMN WAS NOT THE WHITELIST**, and the three-binary split
...
synth | real) ;;
*)
	echo "genwhitelist.sh: unknown mode '$MODE' (want synth | real)" >&2
	exit 2
	;;
esac

if [ "$N" -lt 1 ]; then echo "genwhitelist.sh: <n> must be >= 1" >&2; exit 2; fi
...
# an extra module costs a program that does not use it — the same invariant `StdlibLoader`'s header
# states for the whitelist ("adding a module changes NOTHING for a program that does not use it").
emit_corpus() {
	...
	while [ "$f" -lt "$FILES" ]; do
		{
			echo "// whitelist ladder corpus file $f of $FILES — see maxon-shv2/Testing/ladders/genwhitelist.sh"
	...
	{
		echo "// whitelist ladder entry point — see maxon-shv2/Testing/ladders/genwhitelist.sh"
...
emit_synth_module() {
	M="$1"
	{
		echo "// whitelist ladder: synthetic stdlib-shaped module $M — never called by the corpus"
...
emit_real_module() {
	...
	if [ ! -f "$REAL_MODULE" ]; then
		echo "genwhitelist.sh: mode 'real' needs $REAL_MODULE" >&2
		exit 2
	fi
