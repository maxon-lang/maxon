...
### 50. 🔴 NEW 2026-07-18 (pre-existing, tripped by P1.4b Wave 1 probing) — **float-returning FUNCTIONS panic the x64 backend — a general gap, NOT errors-specific.**

> **⭐ THE FLOAT RUNG is "do BOTH" (user ruling 2026-07-19) — a sliced sequence. F1 (float NAMEABLE) LANDED 2026-07-19 (`fda8f73bb`, 756→761); #50/#57 (the float ABI) ARE F2, the next slice.** F1 makes `typealias F = float(low to high)` parse + resolve to `MaxonType.float`, unlocking the 5 float-LOCAL tests in `ranged-typealias.md` (float stays local via the no-op `as`-cast, `trunc`→int — no ABI). The float-BOUNDARY tests (`float-return-from-function`, `f32-function-param-return`, `return-float-range-check`, `implicit-type-conversion` float-param cases) stay disabled and are **F2's acceptance list — they now port BYTE-IDENTICAL** since float is nameable. F2 mechanism (verified sound this session): thread the existing `buildCalleeResultClasses` map into the lowering walk so caller-capture + callee-return pick XMM0/d0 vs R8/x0 by the callee's return class; 4 sites × 2 targets; route `lowerErrorReturn`'s primary write through `emitPrimaryReturnMove` (dedup + float); the `tryCall` resequencer can't form a cross-file cycle (value XMM vs flag GPR). #57 (float ARGS, separate-counter ABI) is F2's second half.
