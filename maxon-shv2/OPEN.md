...
### ~~44. 🔴 (P1.3 Slice 1 → P1.4) — a boxed-union crosses a call boundary in NEITHER direction: PARAM is E2015, RETURN is now E2015 too (was an exit-101 LEAK).~~ ✅ **RETURN half FIXED 2026-07-18 (P1.4a Wave 1, `354995f3f`)** — `mintOwnedCallResult` now ADOPTS a boxed-union call-result (a `named`+`isBoxed` LAYOUT lookup, `valueIsBoxedUnion`; the Slice-1 `rejectBoxedUnionReturnType` removed). A boxed union as a PARAM still borrows correctly (a union value carries `named`, matched to read the tag). What remains for P1.4a Wave 2: a struct-with-managed-FIELD and moving a managed payload OUT of a borrowed union param (the consume half). Original finding below.

**Found by the P1.3 Slice 1 independent review by probing.** A payload-bearing union is a heap box, and its call-result carries the `named` value-tag — **indistinguishable from a bare enum** (only the layout's `isBoxed` says otherwise). `valueIsOwned` (`Parser.maxon`) recognizes a STRUCT call-result via the `structRef` tag and drops it, but a boxed-union result reads as a bare enum, its `valueOwnsHeap` bit is set only at the `emitEnumBox` CONSTRUCT site (never on a value arriving from a callee), so **`let m = make()` where `make() returns <payload-union>` LEAKED the box (exit 101)** — the caller never dropped it.

**Interim (shipped in Slice 1):** reject a payload-bearing (`isBoxed`) union RETURN type at the return-type annotation with **E2015** (`requireReturnTypeLowerable`/`rejectBoxedUnionReturnType`, real-parse only, not the tolerant sweep), **symmetric with the union-PARAM E2015 already deferred** — so a boxed union is uniformly not-yet-passable across a call in EITHER direction rather than rejected one way and leaking the other. A payload-FREE union return (bare i64 tag) stays allowed (verified). Rather than ship a reachable leak — **"leaks are not ok"** ([[project-p12-string-ownership]], the OPEN #37 precedent: a return-ownership leak is FIXED-or-cleanly-GATED, never deferred as a live leak). Pinned by `specs-shv2` `error.union-return` (red-before-green: leaked/compiled without the reject).

**Full fix (P1.4, both directions together):** the caller ADOPTS a returned boxed union as owned via a `named`+`isBoxed` LAYOUT lookup in `valueIsOwned` (mirroring `valueIsUnion`, which already does the layout lookup on `named`), and union PARAMS become real `structRef`-style bindings — this IS the cross-call borrow-vs-consume ruling P1.1a/P1.2 both deferred to P1.4. Until then the two rejections keep the boundary sound. [[project-p12-wave-c-moves]]

### ~~🔴 (P1.3 Slice 2 review) — a TEMPORARY union scrutinee leaked (exit 101); an owned `gives` value double-freed (0xC0000005)~~ ✅ FIXED 2026-07-18 (`fa685a7a1`, 632→641)

**Both were REACHABLE, both PRE-EXISTING (Slice 1 / P1.2, shipped on main), both found by the Slice 2 independent review by probing (the suite was GREEN over both — no committed test matches a temp scrutinee or an owned `gives`).** They are the reason the "leaks are not ok" rule was codified into the rung skill the same day, and the reason the reviewer's instinct-to-defer became a blocker instead. (1) **`match U.case(x) {…}`** — a constructed union used directly as the scrutinee is an owned heap temp drained per-statement, so whichever arm parsed first and terminated drained the box into that arm alone; every other runtime path leaked it (arm-order-dependent; a managed union leaked its payload too). FIX: `parseMatchCommon` enrolls an owned-TEMP scrutinee as a match-scoped owned binding (guard `scrutOwned and pendingTempsContain(scrut)` so an already-bound scrutinee is never double-enrolled), dropped exactly once on ALL FOUR exits. (2) **`let out = match c { a gives "x{1}"; b gives s }`** — an owned give (an interpolation temp, or a moved-out managed binding) fed a NON-owned result phi, so the post-match drain decref'd both arms' temps (only one ran → crash) and `dropArmScopedOwned` freed a value the phi still pointed at (UAF). FIX: an owned give TRANSFERS to the phi (`transferGiveToPhi` removes it from pending + poisons a bare-local source; `finalizeMatchMerge` marks the phi owned via `trackOwnedTemp`; disagreeing arms `promoteBorrowedGive`). Sabotage-proved load-bearing; 10 review probes + the coordinator's combined case (temp managed scrutinee + moved-out give) all clean.

### 45. ✅ **FIXED 2026-07-19 (= #54 Slice C, `29844a2ff`..`7bda27e44`, 748→754)** — **a match-expression `gives` MIX of different types (register-class-compatible) had NO diagnostic and SEGFAULTED. `finalizeMatchMerge` checked register-class, not type identity.**

`let out = match c { first gives "text{1}"; second gives 5 }` — a `String` give and an `int` give feed one result phi. `finalizeMatchMerge` (`Parser.maxon:~5436`) only rejects a FLOAT/non-float register-class mismatch, not a type-identity mismatch, so this ill-typed match is ACCEPTED; the phi takes the first give's type (`String`) and `print(out)` dereferences the int `5` as a pointer → **exit 139 / segfault**. **PRE-EXISTING and NOT worsened by Slice 2** (the review proved it bit-for-bit identical at the pre-fix parent — the corrective fix changes neither the edge value nor the phi type; it only adds a would-be `__str_decref` unreachable behind the `print` crash). **Ill-typed by construction** (mixing `int`+`String` gives has no single result type) and **unreachable by well-typed code**, so it is not a merge blocker — but it wants a **give-type-consistency diagnostic** as its own rung (a new type-check surface + error code, arriving with the give-type unification the register-class check already defers to; could reject memory-safe scalar-mix gives, so it needs the full suite as its gate). The struct/int give-mix has the same shape.

**✅ FIXED 2026-07-19 as OPEN #54 Slice C** (completing the aggregate-identity trilogy: A = return/reassign/otherwise, B = call-arg, C = match-merge). A `checkGiveTypes` agreement loop in `finalizeMatchMerge` (expression matches only, placed BEFORE `resolveGivesOwnership`/the phi, AFTER the existing float-class `unsupported` check which is preserved) composes THREE cases against the first reaching arm — both-scalar → `typesAgree` (the symmetric mirror of `checkDeclaredType`, NOT a raw same-tag compare, so `ExitCode`+`int` and ranged-alias+`int` stay valid); one-aggregate-one-scalar → `aggregateNameOf(tag,nameId).isEmpty()` DISAGREEMENT; both-aggregate → `namedAggregatesConflict` — throwing **E3005** (`ParseError.giveTypeMismatch`, "match arms give incompatible types: '{got}' vs '{expected}'") at one throw site. Reuses the shared `typesAgree`/`aggregateNameOf`/`namedAggregatesConflict` (TypeRules + Slice A helpers), never copies them; `TypeRules` UNTOUCHED. **⭐ Coordinator verification caught a LIVE SEGFAULT the first cut missed** — a boxed-union give + int give still crashed (139) because `ValueTypeTag.named` is OVERLOADED (a ranged-int alias in the type-class lattice, but a boxed-union VALUE also carries `named`, so `typesAgree(named, integer)` wrongly agrees); closed LOCALLY via the aggregate-ness-disagreement arm (`aggregateNameOf` gates on `containsStruct`/`containsEnum`, distinguishing a real union from a named scalar). Now REJECTS: String+int (was 139), bool+int, struct+struct, union+union, boxed-union+int (both orders), struct+int, String+struct. STAYS valid: same-type, ranged-alias+int, ExitCode+int, statement matches. ⚠ **The independent reviewer HUNG (0-byte transcript, ~68min, no live process) — the coordinator did the full review itself** (duplication, over/under-reject probe matrix, float-unsupported preservation, statement-match, TypeRules-untouched). See #58 (the bootstrap ALSO crashes on this program) + #59 (the cross-cutting `named`-overload gap at return/call-arg).

### 58. 🔴 NEW 2026-07-19 (bootstrap bug, found by #54 Slice C oracle check) — **the C# BOOTSTRAP internal-errors (`E9001 Value cannot be null. (Parameter 'key')`) on a mixed-type `match … gives`, where shv2 now cleanly rejects E3005.**

`maxon build <file>` (the C# bootstrap) on `let out = match c { 0 gives "hello"; default gives 5 }` → **`error E9001: Value cannot be null. (Parameter 'key')`** — a C# null-ref internal error, NOT a clean type diagnostic. So BOTH compilers were broken on mixed-type match gives (shv2 accepted+segfaulted, now E3005 via #54 Slice C; the bootstrap internal-errors). The bootstrap's `finalizeMatchMerge`-analog has its own match-give-type bug (a null key lookup, likely the merge/phi type inference). A bootstrap fix (its own C# subsystem — NOT reachable from an shv2 rung); shv2's suite is unaffected (specs-shv2 tests shv2 only). File for when the bootstrap's match handling is next touched; the correct behavior is a clean type error mirroring shv2's E3005.

### 59. 🟡 MOSTLY FIXED 2026-07-19 (754→756) — **the `ValueTypeTag.named` OVERLOAD left an aggregate-vs-scalar hole at the coercion sites (Slice A/B), the twin of the match-merge segfault #54 Slice C closed locally.** ✅ RETURN / reassign / otherwise / field / payload FIXED via the extended SHARED rule; ⏭ the CALL-ARG site is DEFERRED to Slice B2 (its union-param name is erased — see below).

`ValueTypeTag.named` means a ranged-int alias (a NUMERIC scalar) in the TypeClass lattice (`TypeRules.maxon:90` "every alias shv2 can declare is an INT one"), but a boxed-union VALUE also carries `named` as its value tag — so a value-flow check that treats `named` as numeric (`typesAgree`/`checkDeclaredType`) cannot tell a real boxed union from a named scalar. CONFIRMED: `return <boxed-union> from a returns-Integer fn` COMPILES and **LEAKS (exit 101)** — Slice A's return check misses it (the tag check treats named~integer, and `namedAggregatesConflict` fires only when BOTH sides are named aggregates, so aggregate-vs-scalar falls through). The call-arg site (Slice B) has the same shape. #54 Slice C closed this LOCALLY at the match-give site (via the `aggregateNameOf` aggregate-ness-disagreement arm) but did NOT touch the shared rule, to keep Slice C's diff clean. **The proper cross-cutting fix: extend the SHARED rule (`namedAggregatesConflict` / the aggregateNameOf-emptiness check) to reject aggregate-vs-scalar, so return / reassign / otherwise / call-arg / field / payload ALL catch it uniformly** (a strict improvement — aggregate-vs-scalar is always ill-typed, no valid program relies on it). Its own rung; reuses the exact `aggregateNameOf` technique Slice C proved.

**✅ FIXED 2026-07-19 (754→756).** Extended `namedAggregatesConflict` (`TypeRules.maxon:243`) to `not (expected.isEmpty() and got.isEmpty()) and not got.equals(expected)` — it now fires on aggregate-vs-scalar (exactly one side named), not only two-different-aggregates. At the RETURN / REASSIGN / OTHERWISE sites (`Parser.maxon`) the aggregate-name check was **REORDERED to run AFTER the tag check** and **un-gated** (compute both names unconditionally), with the message routed through Slice C's `giveTypeName(tag, aggregateName)` helper so the scalar side prints its `typeTagName` (`'int'`) not an empty name. The reorder is the KEY to zero golden churn: a genuine class mismatch (struct-vs-int, bool-vs-int) is caught by the tag check FIRST and keeps its existing message; the name check only NEWLY catches the case the tag check WRONGLY AGREES on (a boxed union vs a scalar, `checkDeclaredType(named, integer)` agreeing). Slice C's now-redundant local `aggregatenessDisagrees` check was FOLDED into the extended shared rule. `field/payload` (`requireManagedValueMatches`) inherits the extended rule (its message now uses `giveTypeName` too). Confirmed: `return`/reassign boxed-union-where-scalar-declared → **E3005** (was leak 101); ranged-alias-vs-int STAYS valid; the boxed-union+int match-give still E3005 after the fold; **full suite 756/0 with ZERO drift on any existing golden** (the reorder worked). Specs: `error.return-union-as-scalar` (structs.md), `error.reassign-union-as-scalar` (assignment.md). ⏭ **The CALL-ARG site (`SemanticCheck.checkArgTypes`) is DEFERRED to Slice B2** — it uses `structRefName` (STRUCT-only) and the union PARAM name is erased before SemanticCheck, so catching a union arg vs a scalar param needs the pre-erasure param-name carrier Slice B2 supplies; its one-sided gate was left untouched, so the shared-rule extension does not change it. ⚠ PROCESS: implemented by the coordinator directly (agents were surfacing permission dialogs) — the code was via the stopped implementer's salvaged diff (verified line-by-line), the specs + gates + review by the coordinator. A long dead-end chasing "why isn't my new spec discovered" turned out to be **the spec-test runner resolving `specDir` relative to CWD** — every run had been reading MAIN's specs from a stale `cd`; [[project-open54-ab-closed]].

### 46. ⚠ NEW 2026-07-18 (bootstrap bug, tripped by the P1.3 match-flow fix) — **the BOOTSTRAP's whole-program mutating-parameter fixpoint does NOT CONVERGE once a function's call graph grows, adversarially demanding `let` then `var` on a binding that is already `let`.**

When the give-ownership helpers entered `finalizeMatchMerge`'s call graph, routing the merge edge through the shared `recordBranchEdge(block, …)` helper made the bootstrap (`maxon-sharp`) reject the edge loop's `arm` binding with **E3077** ("variable 'arm' is never reassigned; use 'let'") — pointing at a binding that is *already* `let`. A "use `let`" error against a `let` is the signature of a non-converging mutating-parameter fixpoint (verified by the review: re-routing through the helper fails to compile; the direct push compiles). **Worked around** in shv2 by a documented 2-line direct `arm.exitBlock.branchEdges.push(BranchEdge.create(merge.id, argIds))` (same `argIds.count() > 0` guard, **byte-identical IR** to `recordBranchEdge`, confirmed additions-only fragments). Residual hazard: if `recordBranchEdge` ever gains logic this site won't inherit it (the in-place comment flags it). The real bug is in the bootstrap's analysis; a note for when the bootstrap is next touched.

### 47. 🔴 NEW 2026-07-18 (P1.4a Wave 1 review) — **rejecting a String/non-numeric FIELD DEFAULT leaks 4 COMPILER allocations (the compiler exits 101, not 1, on its own error-unwind path). A "leaks are not ok" item — pre-existing, its own rung.**

Found by the P1.4a Wave 1 independent review by probing (NOT in the suite). A program with a non-numeric-literal field default — `type T { export var label as String = "x" }` — is correctly REJECTED with **E2015** (`rejectNonLiteralFieldDefault`, `Parser.maxon:2149`), but the shv2 compiler ITSELF leaks **4 allocations** on that error path, so the compile process exits **101 ("MM leak: 4 allocation(s) remain")** instead of a clean exit 1. **Confirmed PRE-EXISTING and identical on `main`** (the review built main in a throwaway worktree and reproduced the exact `exit 101 / 4 allocations`) — a compile-time leak in a DIFFERENT feature's error-unwind path (string-literal field defaults), NOT in borrow/adopt, so NOT a P1.4a blocker and it was correct not to smuggle the fix into that review. But it IS a reachable compiler leak (**"leaks are not ok"**, now codified) and wants its own rung: a red-before-green spec pinning that a rejection is ALSO leak-free (the diagnostic-payload String or the partially-built decl is not freed on the throw path — likely the same throw-of-a-heap-value error-unwind family as the bootstrap's `try…otherwise` leaks). **A rejection should exit 1, never 101.** Do adjacent to any parser error-unwind allocation cleanup.

### ~~🔴 (P1.4a Wave 2 review) — a CONDITIONAL MOVE-OUT leaks the not-moved path (exit 101), PRE-EXISTING in the core move model.~~ ✅ **FIXED 2026-07-18 (`c58b2c793`, the path-sensitive-drop corrective, 657→670)**

**The SECOND consecutive rung whose review found a pre-existing move-model leak** (Slice 2's temp-scrutinee/gives were the first). `VarInfo.movedFrom` was MONOTONIC + path-blind: a value moved on ONE `if`/`match` branch was drop-SKIPPED on EVERY exit path (`emitScopeDrops`), leaking on the not-moved path. Verified reachable on `main` (a union payload moved in one branch of `makeWrap` → exit 101; owned locals + Wave-2 struct-field consume are just more triggers). **User ruled "the real fix, no runtime flags."** FIX: compile-time **drop elaboration** — `captureMoveMark`/`restoreMoveMark` snapshot the move bit around a branch; `reconcileMovesAtMerge` settles each owned binding at every JOIN (`finalizeIfNoElse`/`finalizeIfElse`/`finalizeMatchMerge`): moved on some reaching edges + live on others ⇒ drop it on the LIVE edges, each edge's own value. Conditional moves are now ACCEPTED + correct (drop once per path); reads past a maybe-moved join stay conservative E3102. Review sabotage-proved all three guards. ⚠ **Remaining follow-up (deferred with agreement): the LOOP-EXIT RECONCILE.** Moving an OWNED binding declared OUTSIDE a loop from inside the loop body is currently REJECTED (E2015) — a genuine correctness boundary (the back edge re-moves → double-free; a `break` leaves it live on the normal exit). ACCEPTING it (drop-elaborating across the loop boundary — edge-split the normal-exit condBranch + retroactive break-block drops) is its own wave. A loop-body-LOCAL move already works.

### 48. ⚠ NEW 2026-07-18 (P1.4a Wave 2 — harness robustness, not compiler code) — **the spec runner's `.spec-tmp` staging can build against a STALE leftover `.maxon` from a racing worker (a false-alarm hang/failure).**

Tripped over by the P1.4a Wave 2 implementer (cost real diagnosis time) and re-hit by the reviewer (mitigated by a manual `rm -rf specs-shv2/.spec-tmp` before every build). `SpecTestRunner.maxon:417-466` (`purgeSourceFiles`) rebuilds a per-test directory's files but does NOT fully reset the overall `.spec-tmp` per run, so a stale `.maxon` left by a racing concurrent worker can silently join a later directory build and hang/miscompile an unrelated program (this session: a leftover made `comparison-operands` hang). Same class as OPEN #6 (the runner picking up gitignored leftovers). A harness-robustness fix (fully purge or namespace `.spec-tmp` per run/worker), gated by the full spec harness — its own follow-up, not any rung's compiler code.

### 49. ~~⚠ (P1.4b Wave 1) — a throwing fn with a MANAGED return type is GATED (E2015).~~ ✅ **RESOLVED 2026-07-18 (P1.4b Wave 2b, `d16a45f0f`)** — managed returns now WORK: the gate + `returnTypeOwnsHeapBox`/`payloadClassOwnsHeapBox` + the reject spec were removed, and the tryCall result is re-scoped to the ok edge (NULL on the error edge). Original finding below.

`function f() returns String throws E` (or any managed return: String / any struct box / boxed union) accept-and-**CRASHED** (segfault, exit 139) — a `tryCall`'s result register is NULL on the error edge, so a managed result has nothing valid to drop. Genuinely deferred to **Wave 2** (managed error values). Wave 1 turned the crash into a clean **E2015 reject** (`Parser.parseFunction`, guarded by `returnTypeOwnsHeapBox` reusing `SignatureIndex.classifyUnionPayload` — the same classifier as construct/extract/drop, so it fires on exactly the crashing set; scalars stay green), consistent with how the rung rejects every other unsupported error construct — NOT shipped as accept-and-crash ("leaks/crashes are not ok"). The check is in the PARSER because `resolveTypes` erases a union return to `integer` before SemanticCheck runs. Pinned by `error.managed-return-throws-unsupported` (red-first). **Wave 2 removes the guard + the spec + enables the disabled `[wave2-managed]` cases** (`error.otherwise-return-string`, `-managed-struct`, the assoc-value throws family).

### 50. ✅ FIXED 2026-07-19 (F2a, `f8c516e33`, 761→762) — **float-returning FUNCTIONS panic the x64 backend — a general gap, NOT errors-specific.**

> **✅ F2a (float RETURN) DONE 2026-07-19 — `float-return-from-function` GREEN (exit 3), byte-identical.** The FLOAT RUNG is "do BOTH" (user ruling): F1 (NAMEABLE, `fda8f73bb`, 756→761) → F2a (RETURN, this) → **F2b (float ARGS #57) is what remains.** F2a was END-TO-END, THREE blockers: (1) FRONT-END — a float-alias return type was mis-read as int (`checkDeclaredType`: `tagIsIntegral(named)=true` → false E3009); fixed by resolving a `named` declared type to its underlying primitive (float alias → float) via ONE shared `namedIsFloatAlias` + two typed doors (`floatResolvedTag` tag→tag, `resolveFloatAliasType` type→type) — an int alias/union/struct passes through untouched (a 6th stale "every alias is int" comment fixed at the return site). (2) BACKEND — XMM0/d0 return codegen: exported `buildCalleeResultClasses`/`calleeResultClass`, `buildValueClassColumns` takes the map as a param (built ONCE in each backend's `create`, threaded into the column build AND the new `callResultRegister`/`emitReturnValueMove`) so capture-file and coloring-file are one fact; `lowerErrorReturn` routes through `emitReturnValueMove`. (3) CALL RESULT (found in verification) — `resolveTypes` rewrites the Std return type but NOT the parser value tags, so a float-alias call result stayed `named` and `trunc` wouldn't fire; `resolveFloatAliasType` at `mintOwnedCallResult` tags it `float`. Optimizer: +2 allocs/+40 B flat once-per-compile (linear). Review CLEAN (probed non-float-alias pass-through: `returns Count`→8, union→9 no-leak; aggregate check preserved). ⏭ **#57 (float ARGS, separate-counter ABI) = F2b, reuses `namedIsFloatAlias`/`floatResolvedTag` for the float-alias PARAM type.** See **#60** for the throwing-THROW-path gap deferred here.

`function f() returns float` (regular, non-throwing) already panics: `StdToX64Conversion.maxon:1783: float return values (XMM0) are not supported until M3 — the 'return 42' slice is integer-only`. A deliberate deferral from M1 that never landed; `lowerCall`/`lowerReturn` capture/write the result via **R8 unconditionally** with no float-return-register (xmm0) path. A throwing fn returning a float inherits this but crashes DEEPER (`X64Backend.maxon:603` cross-register-file `r8→xmm0`) because the `tryCall` path lacks the explicit `1783` guard. Out of P1.4b's scope (float-return codegen ≠ errors). **The fix (a future float-return rung): wire the float return register through `lowerCall`/`lowerReturn`/`lowerTryCall`/`lowerErrorReturn` + arm64 (d0) together**; until then, add the `1783` guard to the `tryCall` path so the error is the clean "not supported" one, not the confusing cross-file panic. No spec exercises float returns.

### 51. ⚠ NEW 2026-07-18 (pre-existing, surfaced by P1.4b Wave 1 review) — **`as` casts + ranged-int narrowing are NOT enforced ANYWHERE (returns/assignments/casts all pass a value through unchanged).**

Wave 1 added `as` support (the error specs need `0 as Integer`), but `parsePostfixCast`/`evalConstPostfix` return the value **unchanged** — no retag, no range check. The review confirmed this adds **no new** hole: bare `return 300` and `return x` (Integer→ExitCode, no cast) ALSO compile and truncate to exit 44, so shv2 does not enforce ranged narrowing on returns/assignments at all yet, and the inert cast can't even produce a wrong-typed value (tag unchanged). A pre-existing type-checking gap (ranged narrowing is P1.9-adjacent), not a P1.4b regression — file, don't block. Also noted: the sibling `maxonOpIdxToString` segfault (a different subsystem) remains a documented accept-and-crash deferral — the same "turn latent crash into clean reject" sweep applies to it.

### 52. 🟡 PARTIALLY FIXED — **a union-payload INTERNER MISMATCH: the parser resolved a signatures union-layout's payload id against `self.artifact.typeNames`.** ✅ **SINGLE-FILE slice FIXED 2026-07-19** (`6ae9b56cd`, 745→748: the two re-intern doors). ⏭ The BACKWARD-ordering cross-file case (`classifyUnionPayload:582` panic) still needs a declaration-order union-layout PRESCAN — its own rung.

`SignatureIndex.classifyUnionPayload:582` panics — "a payload field's named type id is not in the interner it was resolved against" — when a boxed (payload-bearing) union is DECLARED in a later-sorting file than the one that USES it. This is why `error.cross-file-throws-caught-later-file` stays disabled. **NOT a Wave-2a regression:** it reproduces with a plain `return` (no throw, no error handling) — the cross-file signature/interner path, unchanged by the errors diff (the `return` variant even misclassifies the payload's type as the enclosing union). Needs the cross-file signature machinery (seed every file's union layouts into a shared interner before any body is parsed) + its own goldens. **Its own rung** (the cross-file `throws`-prescan / union-layout-prescan). ⚠ Related coverage note: `error.file-private-union-caught-cross-file` (FORWARD file ordering) **passes now** (probed exit 42) but is left disabled with a stale `[wave2-managed]` reason — a future cross-file rung should enable it + refresh the reason (premature to assert cross-file boxed-union as supported while the sibling ordering panics).

**⚠ REFINED 2026-07-19 by the OPEN #56 review — this is the SAME interner-mismatch CLASS as #56, and it is NOT cross-file-only.** #56 fixed the RUNTIME struct destructor walk (a `signatures` layout id resolved against `project.typeNames`); the PARSER's union-payload paths have the identical shape with a different interner pair — a `signatures` union layout's payload id resolved against **`self.artifact.typeNames`**: construct (`parseEnumLiteral` layout from `self.signatures.enumOf` at `Parser.maxon:9099` → `requirePayloadLowerable`/`requireScalarPayloadArg` classify `field.declaredType` against `self.artifact.typeNames` at `~9303`), match-binding (`declarePayloadBindings`, `~7283`), and scope-exit drop (`decrefCalleeFor` via `enumMatchFor`, `~5120`/`~7133`). Demonstrated live: `union UHold; holds(inner BoxA)` — a VALID single-file program — compiles fine WITHOUT a ranged alias (exit 4) but is WRONGLY rejected **E3005** ("cannot assign 'struct' … of type 'int'") the moment a `typealias Integer = int(...)` is declared, because BoxA's id is misread as the alias (in the unlucky arrangement, a silent wrong classification instead). So #52 is triggerable **single-file by a ranged alias**, not only by cross-file ordering. The struct side has a re-intern door (`fieldTypeOf(..., into:)` via `adoptType`); **the union side has no equivalent, and adding that union analog is this rung's core work** (then classify payloads against the re-interned type, as #56's fix resolves against the id's home interner).

**✅ SINGLE-FILE SLICE FIXED 2026-07-19** (`6ae9b56cd`, optimize `e7dd8c9ff`, 745→748). TWO doors added to `ProgramSignatures`: **Door A** = exported the existing private `adoptType` (the id-crossing primitive) and routed the 3 value-producing sites through `self.signatures.adoptType(field.declaredType, into: self.artifact.typeNames)` — classify at construct+load (`Parser.maxon:9309`) + match-bind (`:7286`), AND the load-value-type MINT (`:9372`, the "second manifestation" — else the loaded value carries a signatures id `enumMatchFor` later misresolves); this mirrors the struct sites' inline `fieldTypeOf(..., into:)` convention (review APPROVED the inline over a named door: a fetch-door doesn't fit sites holding the `PayloadField`, a naming-only wrapper would be thin, and `adoptType` panics loudly on a wrong-layout id so the foot-gun is defused). **Door B** = `unionBoxDropCallee(layout) = unionNameDropCallee(self, self.typeNames, layout)` — `decrefCalleeFor`'s union branch (`:5123`) routes through it, resolving the payload drop-callee against the layout's OWN (private) interner. Door B fixes a memory-SAFETY hole, not just a wrong reject: the mis-resolved drop returned `MmDecrefName` instead of `destructorSymbol` → a payload LEAK (or wrong-`__destruct` wild-free). Corrected the stale `enumTypes` comment (`SignatureIndex.maxon:857`, "carries no ids/no remap" — false; `foldFile:975` remaps payloads into `self.typeNames`). Specs: `union-struct-payload-ranged-alias`(+`-match`) authored (a valid union-struct-payload program + a ranged alias — was wrong E3005, now exit 4/6 leak-free); `error.file-private-union-caught-cross-file` (FORWARD ordering) ENABLED (14/0). Three consecutive CLEAN reviews now (A/B, #56, #52). ⏭ **REMAINING = the BACKWARD-ordering cross-file case** (`error.cross-file-throws-caught-later-file`, still disabled with an updated reason): the union is declared in a LATER-sorting file, so its layout isn't seeded when the earlier body parses — needs a declaration-order **union-layout PRESCAN** (seed every file's union layouts into a shared interner before any body), which the doors alone don't provide. Also unblocks part of **#54 Slice B2** (union call-arg). See [[project-open56-interner-mismatch]] (the same recurring interner-mismatch family).

### 53. 🔴 NEW 2026-07-18 (pre-existing, blocks byte-identical /specs ports; found by P1.4b Wave 2a) — **shv2 does not parse LABELED union-construction arguments (`U.case(a, label: b)` → E2004).**

`LexErr.pair(7, column: 13)` fails `E2004 "Undefined variable 'column'"` — shv2's union construction only accepts POSITIONAL args; it does not slot a labeled arg against the case's payload field names. Orthogonal to managed error VALUES (a union-*construction* gap), but it blocks the byte-identical port of any `/specs` union test that uses labels (Wave 2a disabled `throw-transfers-ownership.propagate-throw-otherwise-no-binding-decrefs` for this and authored a POSITIONAL twin `error.otherwise-no-binding-boxed-decref` to keep coverage). Needs a parser rung: slot labeled args against the case's payload field names, with unknown/duplicate-label diagnostics (mirror the struct-literal / call-arg label handling).

### 54. 🔴 (pre-existing since P1.1, CONFIRMED by P1.4b Wave 2b review + coordinator probes) — **shv2 does NOT name-check aggregate TYPE IDENTITY → a mismatched struct/union is dropped with the WRONG destructor = a wild free / SEGFAULT. A type-SOUNDNESS hole.** ✅ **Slices A + B CLOSED 2026-07-18** (A: return / reassign / `otherwise`-value; B: consuming call-arg for STRUCTS). ⏭ **Slice B2** (union call-arg — the param name is erased before SemanticCheck) + **Slice C** (value-position `match … gives` merge = **OPEN #45**) remain.

Every struct shares the `structRef` tag and every boxed union the `named` tag, and the type checks at return / `otherwise`-value / consuming-arg positions are **TAG-level only** — so a value of the WRONG aggregate type passes the check and is later dropped with the DECLARED type's destructor. **CONFIRMED reachable with NO error handling:** `return BoxB.create(9)` from a `function bad() returns BoxA` (BoxA has a String field, BoxB is scalar-only) COMPILES and **SEGFAULTS (exit 139)** — `__destruct_BoxA` reads BoxB's scalar as a String pointer and frees it. Also reachable at a managed `otherwise` fallback (`let s = try makeA() otherwise BoxB.create(...)` → 139; Wave 2b's guard catches tag-mismatch + borrowed, NOT a same-tag NAME mismatch). A `var` reassignment (`var a = BoxA…; a = BoxB…`) is OK (exit 0). **NOT a Wave-2b regression** — pre-existing since P1.1 structs; Wave 2b's gate removal merely made the `otherwise` instance newly reachable. **A dedicated corrective rung: a NAME-level type-identity check applied UNIFORMLY wherever a managed value flows (return, `otherwise`-value, consuming arg, assignment), with cross-site spec coverage. ⭐ USER (2026-07-18): v1 (`maxon-selfhosted`) already fixed this — PORT v1's type-identity check, do NOT re-derive** (the project's "find the v1 file that does it and read it first"). **HIGH priority** (a reachable segfault on valid syntax). Also fold in the sibling `maxonOpIdxToString` accept-and-crash noted at #51.

**✅ Slice A (`16c729b07`, optimize `406987b5d`, 736→741) — return / reassign / `otherwise`-value.** Ported v1's exact-interned-name identity check (`maxon-selfhosted/SemanticCheck.maxon:2452-2545,2687` — `argTypesCompatible`/`namedStructOrEnumName`, structs never subtype). shv2 already name-checked struct-FIELD / union-PAYLOAD sites via `requireManagedValueMatches`; Slice A factored **ONE** shared pair — `aggregateNameOf(tag, nameId)` (the interned aggregate name, or empty for scalar / String / a ranged-int alias — gated on `containsStruct`/`containsEnum`, real aggregate registration, NOT merely "has a name id") + `namedAggregatesConflict(expected, got)` (fires only when BOTH are named aggregates AND the names differ) — and applied it **additively** (alongside the existing tag check) at `parseReturnStatement` (E3005), `parseAssignment` local-reassign (E3005), `parseUnboundOtherwiseForm` (E3059); `requireManagedValueMatches` now routes through the same pair (one comparison, not five). Both `return BoxB`-from-`returns BoxA` (139) and the managed `otherwise`-BoxB (139) now compile-REJECT cleanly; the latent reassign hole rejects too. 5 reject specs (`error.return-wrong-struct/-union`, `-reassign-wrong-struct`, `-otherwise-wrong-struct`, `-retype-struct-to-other-struct-errors`). Independent review: **the ONE-FACT-WRITTEN-TWICE signature bug is ABSENT** — first P1.4-era rung with a clean review. Scale delta 0 (the check reads dense scalar columns, materializes no `MaxonType`; confirmed via the OPEN #22 immune A/B, not the logged baseline). ⏭ **Slice B** = the consuming call-arg site — needs `valueNameIds` published on `IrFunction` + `SemanticCheck.checkArgTypes` (erasure means the union NAME is gone by SemanticCheck for unions — structs keep `structRef(id)`; the call-arg is the one site the parser can't cover alone). ⏭ **Slice C** = value-position `match … gives` arm agreement (scalar + aggregate) = the already-filed **OPEN #45**.

**✅ Slice B (`aa2ac2509`, optimize `2a50ea55c`, 741→743) — the consuming CALL-ARG site, STRUCTS.** The confirmed RED (`WrapA.create(b)` where `b` is a `BoxB` but the param consumes a `BoxA` → `__destruct_BoxA` frees BoxB's scalar as a String pointer → **139**) now compile-REJECTs **E3005** (`argument type mismatch for 'inner': expected 'BoxA', got 'BoxB'`); ownership-independent (a borrowed wrong-struct arg rejects too). This site MUST live in SemanticCheck (the callee may be in another file), so: **published `valueNameIds` on `IrFunction`** (the parallel per-value interned-NAME column, mirror of `valueTags` — 1 real create site `Parser.maxon:3276`, empty in `createFromStd`, by-ref in `shallowCopy`/`relocate`, remapped project-global in `ParseStaging.remapArtifact` via a new `TypeNameInterner.remapValueNameId` that COMPOSES the single-home tag→name functions, not a second copy) + a name branch in `SemanticCheck.checkArgTypes` (arg name from `valueNameIds`, param name from the surviving `structRef(id)`). The conflict RULE `namedAggregatesConflict` MOVED from `Parser.maxon` to `IR/Maxon/TypeRules.maxon` — ONE function, now called by both the 4 parser coercion sites AND the call-arg site. Independent review CLEAN again (2nd P1.4-era rung with no signature bug); covers ALL call shapes (`call`/`tryCall`, method calls lower to these — all probed E3005). Scale: a small LINEAR memory bump (the retained name column, ~x2.0 ratios, immune A/B), NOT superlinear. ⏭ **Slice B2 = the UNION call-arg** — a union PARAM is erased to `integer` at `TypeResolution.maxon:192-194` and kept nowhere that survives to SemanticCheck (the arg side is ready — a union arg keeps tag `named` + its published `valueNameIds`), so it needs a NEW pre-erasure param-name carrier (the `funcParamConsumes` pattern), entangled with the cross-file union-interner landmine **OPEN #52**. Deferred deliberately.

### 55. 🔴 NEW 2026-07-18 (pre-existing, found by OPEN #54 Slice A review) — **`try <voidThrowingCall> otherwise <scalar>` in STATEMENT position PANICS in lowering (`valueTagToStdType: a 'void' tag names no value`).**

`try doIt(9) otherwise 0` where `doIt` returns void (throwing) panics `LowerMaxonToStd.maxon:101: valueTagToStdType: a 'void' tag names no value` — a void try-result is merged with a value fallback, minting a void phi block-arg that lowering cannot type. **Confirmed PRE-EXISTING on `main`** (identical panic; unrelated to Slice A's coercion-site checks). A void `try` whose `otherwise` supplies a value is ill-formed and should be a clean PARSER diagnostic (the `otherwise` value is dead — the success path has no value to bind), not a lowering panic. Same "turn a latent accept-and-crash into a clean reject" family as #50/#51. Its own small parser rung; no spec exercises it.

### 56. ✅ **FIXED 2026-07-19** (`f42e2c00c`..`c3dec820f`, 743→744) — **a VALID nested struct whose field type has a static `create` with a RANGED-INT-ALIAS param PANICKED the destructor walk (`SignatureIndex.maxon:822 managedNameDropCallee`). Root cause was NOT ranged-alias classification — it was an INTERNER MISMATCH.**

Confirmed on `main`: a `type BoxA { export var label as String; static create(x Integer) returns Self … }` nested as a field in `type WrapA { export var inner as BoxA; static create() … }` and built → **compile panic** at `SignatureIndex.maxon:822` (via `noteDestructorUsage` → `closeDestructorNeeds` → `addNestedStructDestructors` → `managedFieldDropCallee` → `managedNameDropCallee`).

**REAL root cause (the implementer's first hypothesis — "ranged alias mis-classified managed at :801" — was WRONG; `structFieldIsManaged` correctly returns false for a `named`-tagged non-struct/non-enum).** `ProgramSignatures.typeNames` is a PRIVATE interner ("an id may not leave this type", `SignatureIndex.maxon:866`). `addNestedStructDestructors` re-fetched the struct layout from `signatures` (field-type ids minted against `signatures.typeNames`) but resolved that id against **`project.typeNames`**. Declaring the `Integer` ranged alias adds a name to the project interner, SHIFTING its ids relative to the signatures interner's — so the same id names `BoxA` in signatures but `Integer` in project. The guard `signatures.structFieldIsManaged` (self.typeNames) saw a managed `BoxA` field and proceeded; `managedFieldDropCallee` (project.typeNames) misread the id as `Integer` → panic. In an unluckier arrangement the misread name would be a DIFFERENT declared struct → a silent WRONG `__destruct_<T>` (a wild-free). A latent memory-safety hole; the panic was the lucky case.

**FIX:** `addNestedStructDestructors` now takes the PROJECT `StructLayout` its caller (`closeDestructorNeeds`) already holds, so the field-type id and the interner resolving it are the same — mirroring the already-correct twin `synthesizeStructDestructor` and the union walks (which use project layouts throughout). Review then SINGLE-HOMED the `(project layout, project.typeNames)` pairing into `managedStructFieldDropCallee` (`MmRuntime.maxon:559`), called by BOTH struct walks, so the drift that caused this is unrepresentable. RED-first spec `nested-managed-field-ranged-create-param`. Two clean gates (optimizer: pure reduction, delta 0; reviewer: correct + complete for the destructor subsystem). See [[project-open56-interner-mismatch]]. ⚠ Review found a SIBLING of this bug in the PARSER's union-payload paths — folded into #52 below.

### 57. ✅ FIXED 2026-07-19 (F2b, `c428191ac`, 762→763) — **a `float` passed as a NON-FIRST argument PANICS x64 codegen: `X64Backend.maxon:603 emitRegRegMove … rdx→xmm0 crosses register files`. Distinct from #50 (float RETURN) and #56.**

> **✅ F2b (float ARGS) DONE 2026-07-19 — `f32-function-param-return` GREEN → the FLOAT FEATURE ("do BOTH": nameable F1 → return F2a → args F2b) is COMPLETE.** A float param/arg travels in XMM (x64) / SIMD d-reg (arm64) by a SEPARATE int/FP counter (v1's convention, NOT positional). Backend: `argFpRegisters` + `lowerParam` captures a float param from its XMM slot (`computeParamSlotIndex` = count of preceding same-file params, ported from v1); an extracted `emitCallArgMoves` (shared by lowerCall+lowerTryCall) routes each arg to GPR/XMM by the callee's param class via independent running counters. New `buildCalleeParamClasses` (a u64 float bitmask per callee — the arg-side twin of F2a's return-class index; kept a SEPARATE dense map, optimizer+review agreed: independent facts, both from `stdTypeIsFloat`, can't drift; merging heap-boxes a struct per entry = worse; linear +132KB/0.017%). Front-end: 1 line — a float-alias param VALUE re-tagged to float (`resolveFloatAliasType`) so `x*2.0` doesn't spuriously siToFp. ⭐⭐ **REVIEW CAUGHT A REAL ARM64 COMPILER CRASH** (the SIGNATURE bug again): the SHARED liveness clobber census decoded float-arg physical regs through the **x64-only** `operandRegister` (X64Register.fromRawValue, RegNum 0-31), so an arm64 `d1` (RegNum 32) panicked the backend on ANY 2+-float-arg call — the exact per-ISA hazard the sibling `operandClass` was ALREADY hardened against (one fact fixed once, missed in the twin). x64 unaffected (XMM arg regs = RegNum 16-21) + **the arm64 suite can't run on this host (see #61)**, so the x64 suite was green over it. Fixed target-neutrally (`operandPhysicalRegNum` → unified RegNum, x64 byte-identical). ⏭ **Deferred (own slices): int→float ARG widening** (needs call-site coercion; `int-*-to-float-param` stay disabled) + **#60** (throwing-float RETURN throw-path).

Confirmed on `main` with a zero-struct minimal case: `function takeF(n Integer, f float) …; takeF(3, f: 1.5)` → **compile panic** — the allocator colored a call-arg move with a float source (belongs in an XMM register) and an integer-slot dest (or vice-versa), and `emitRegRegMove` refuses the cross-register-file `mov`. Float args are being assigned an integer arg register (`rdx`) rather than an XMM slot in the calling convention's float-arg lane. Same "float codegen is incomplete" family as #50 (float RETURN via R8 not XMM0) — both want the float register lanes wired through call/return together, plus an arm64 (d0..d7 / v-regs) parity check. **⏭ THIS IS F2b — the NEXT slice, now that #50 (F2a) is done.** F2b = `lowerParam` float (a float param arrives in XMM by a SEPARATE int/FP counter — v1's internal convention, both targets) + arg routing + the float-alias PARAM type resolution (reuse F2a's `namedIsFloatAlias`/`floatResolvedTag`). Acceptance (byte-identical, now nameable): `f32-function-param-return`, `implicit-type-conversion` `int-literal/var/expression-to-float-param`.

### 60. 🔴 NEW 2026-07-19 (deferred from F2a, confirmed by its review) — **a THROWING function returning float (`returns Float throws E`) compile-PANICS on the THROW edge: `LowerMaxonToStd` hands the throw path a GPR-typed zero default, routed to XMM0/d0 for a float return → `X64Backend.maxon:603` cross-register-file `rax→xmm0`.**

F2a wired float returns through XMM0/d0, and the OK edge of a throwing float function works (`try computePi() otherwise 0.0` → exit 3 — `errorReturn(<float>, 0)` moves the real float to XMM0). But `lowerThrowError` (in `LowerMaxonToStd.maxon`) synthesizes the throw edge's primary-value default via `emitZeroConst` → an **i64** zero (a GPR value the caller ignores once the error flag is set); `emitReturnValueMove` then routes it to XMM0 for a float return type → the cross-file panic. **NOT a regression** (base panicked on ANY float return; F2a strictly improved this) and **fail-loud** (a compile panic, never a silent miscompile) — the review confirmed no green spec regresses. **Fix (its own slice): type the throw-path default to the function's return type** (a float zero for a float function) upstream in `LowerMaxonToStd`, which is OUTSIDE F2a's exclusive files; it needs its own throwing-float fragments as the gate. Both `lowerErrorReturn` sites already carry a ⚠ note. Family: "a value's Maxon tag / Std type disagrees with the register file it must land in" (with #50/#57).

### 61. 🔴 NEW 2026-07-19 (found by F2b review — the reason the F2b arm64 crash slipped a green x64 suite) — **the arm64 spec suite CANNOT run to completion on this (Windows) host: it dies in the TEST HARNESS on `static-variables` with `mm_decref: refcount underflow (already zero)` in `__destruct_ProcessCapture` (`Testing.runStagedTest`).**

Confirmed PRE-EXISTING (reproduces identically on base F2b, stash+rebuild). A `ProcessCapture` DOUBLE-FREE in shv2's own harness runtime (bootstrap-compiled, independent of the shv2 emitter under test) — the harness spawns the cross-compiled arm64 test binary as a subprocess and its `ProcessCapture` destructor runs twice. ⚠⚠ **CONSEQUENCE, and it is load-bearing: arm64 codegen is NOT gated on this host** — the arm64 half of every rung is COMPILE-checked here and its goldens synced cross-host by a Mac CI, but the arm64 SUITE never runs to green locally. That is exactly why **F2b's arm64 multi-float-arg compiler CRASH (see #57) passed CI** — the x64 suite was green and nothing ran the arm64 path on this host; the review's cross-compile probe caught it. **Two implications:** (1) fix the `ProcessCapture` double-free so the arm64 suite can gate locally; (2) until then, arm64-affecting changes need a cross-compile probe (`--target=arm64-macos`) as an explicit review step, not just a `build`. Family: the harness-runtime bugs in [[project-bootstrap-bugs-found-by-shv2]].
