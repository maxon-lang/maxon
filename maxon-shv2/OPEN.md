...
### 39. 🔴 NEW 2026-07-16 (P1.2) — **reassigning a `var` owned binding does NOT drop the old box — a MASKED leak (exit 0 today). Drop-on-reassign, single-owner FORCES it.**

`var s = build(1); s = build(2); s = build(3); print(s)` → **exit 0** (`v3`), and the loop-carried `var s = build(0); while … s = build(i) …` → **exit 0** too. Both LEAK the overwritten boxes, but the leak-check counter is not tripped: `parseAssignment` (`Parser.maxon`) rebinds via `scope.setValue` without dropping the previous owned value OR re-tracking the new one, so the leaked old box and the stale scope-exit drop of the reassigned value **cancel in the live count**, and the bump allocator (reclaims nothing) hides the use-after-decref. **Found by the INDEPENDENT review by probing.** It is the LAST reachable owned-String leak category (binding ✓, return ✓, nested-block ✓ all fixed). **`drop-on-reassign` is FORCED by static single-owner** (reassigning overwrites the owner, so the old value must drop) — not a design ruling, but it needs care: the reassigned value's CURRENT SSA value at the assignment point (a loop-carried `var` is a phi), the interaction with the block-scope drops (no double-drop), and reassign-to-borrowed (drop old, un-track). ⚠ **Will become a REAL exit-101 leak + use-after-free the moment `__mm_free` gets a free list** (recycling is P1.2's own follow-on). NEXT after the ownership merge.
