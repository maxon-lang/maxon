...
### 40. 🔴 NEW 2026-07-16 (P1.2 → P1.4) — **borrowed→owned reassignment INSIDE a nested block is a use-after-free (MASKED, exit 0). The height-stack `ownedBindings` model cannot represent a binding that becomes owned SHALLOWER than the current block.**
...
### 42. 🔴 NEW 2026-07-16 (P1.2 Wave C follow-up) — **use-after-move is enforced ONLY at bare reads, NOT at field access / method calls / field STORES. A moved struct read/written through its source is an unchecked UAF / wrong answer.**
