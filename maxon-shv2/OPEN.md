...
### 56. 🔴 NEW 2026-07-18 (pre-existing, found by OPEN #54 Slice B probing) — **a VALID nested struct whose field type has a static `create` with a RANGED-INT-ALIAS param PANICS the destructor walk: `SignatureIndex.maxon:822 managedNameDropCallee: managed field 'Integer' is neither a declared struct nor a declared enum/union`. An accept-and-crash on VALID code — a self-hosting hazard, HIGH.**

Confirmed on `main`: a `type BoxA { export var label as String; static create(x Integer) returns Self … }` (managed field + a static `create` taking the ranged-int alias `Integer`) nested as a field in `type WrapA { export var inner as BoxA; static create() … }` and built → **compile panic** at `SignatureIndex.maxon:822` (via `noteDestructorUsage` → `managedNameDropCallee`). Mechanism (per the Slice B implementer): a `named`-tagged entry whose name resolves to a ranged-int alias reaches `managedNameDropCallee` classified MANAGED (via `managedFieldDropCallee`'s `tag == ValueTypeTag.named` arm at `SignatureIndex.maxon:801`) — but a ranged alias is SCALAR and must never be classified managed. The existing `nested-struct-string-field-drop` test dodges it only because its `create()` takes no parameters. **NOT a Slice B regression** (Slice B touches SemanticCheck name-checking + the IR name column, never the `SignatureIndex` destructor-name walk); Slice B's check correctly rejects the wrong-struct program BEFORE this panic and correctly ACCEPTS the valid nested program (which then hits it). Fix: the destructor-name walk must exclude ranged/scalar aliases from the managed set (the same "ranged alias is not an aggregate" distinction Slice A/B encode via `containsStruct`/`containsEnum`). Its own rung.
