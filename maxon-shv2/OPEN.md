...
### 54. 🔴 (pre-existing since P1.1, CONFIRMED by P1.4b Wave 2b review + coordinator probes) — **shv2 does NOT name-check aggregate TYPE IDENTITY → a mismatched struct/union is dropped with the WRONG destructor = a wild free / SEGFAULT. A type-SOUNDNESS hole.** ✅ **Slice A CLOSED 2026-07-18** (return / reassign / `otherwise`-value); Slice B (consuming call-arg) + Slice C (value-position `match … gives` merge = **OPEN #45**) remain.
