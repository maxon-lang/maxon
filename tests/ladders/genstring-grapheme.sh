...
#
# --- WHAT IT READ ON THE DAY IT WAS BUILT (2026-07-28), so a later run has a
...
#   `data-count-ascii` — the same `__gr_end` scan minting nothing — is FLAT at 3.7 MB across a 4× data range.
#
...
# REPS (env, default 5) repeats the operation inside the timed region; wall nanos are used because the
# emitted program has no thread-CPU intrinsic, so measure on an idle box and take the MINIMUM of several
...
emit_data_program() {
	...
	data-iter-*)
		...
		;;
	data-count-*)
		# `String.count()` — `__gr_count`, which is `__gr_end` in a loop. O(n) PER CALL by design, so this
		# ladder should read ×2 per doubling and a ×4 would mean the scan restarts.
		echo -e "\tvar units = 0"
		echo -e "\tlet t0 = __Builtins.currentTimeNanos()"
		echo -e "\tvar r = 0"
		echo -e "\twhile r < $REPS 'rep'"
	...
	data-countloop)
		...
		# what `String.count`'s own doc warns about. It is here so a future O(1)-on-ASCII `count()` has a
