...
| `genstringviews.sh <n> <sites-*\|data-*\|loop-*> <out>` | ⚠ **THREE families, `gentrim.sh`'s twin in shape**: `sites-*` is a COMPILE ladder (`<n>` = CALL SITES); `data-*` is a RUN ladder on the STRING LENGTH; `loop-*` is a RUN ladder on the TRIP COUNT at a fixed tiny subject | `toByteArray` / `clone` / `codepoints` / `utf16` / `isEmpty` / `replaceFirst` / `String.from(bytes)` and the nine stdlib `utf16*` free functions (P1.8 slice E). ⛔ **`ScaleCorpus` emits ZERO of each — the SEVENTH consecutive rung with that property** — and `.clone()` is THE TRAP IN REVERSE: the corpus has 256 of them and **every receiver is an `Array`**, so a moved number cannot be about this rung. ⭐ **`data-*` settles a real O(n)-vs-O(n²) question**: all three views are PUSH-ONLY loops into an array the parser created with NO reserve, so the whole cost rests on `__managed_grown_cap` being geometric — if a push ever reallocated to the exact required length every one would read ×4 per doubling, exactly as `__str_append` used to. ⚠ **`SEED` is a REAL KNOB for `codepoints`/`utf16` and INERT for `toByteArray`/`clone`**, and the asymmetry is the point: a byte copy cannot see an encoding, while `SEED=supp` is the ONLY seed that reaches `utf16`'s SURROGATE-PAIR arm (two units per codepoint) — a ladder without it has not measured that arm. `loop-*` prices the four new per-trip owned allocations against `loop-control`; unlike slice D's `loop-trim-shared` there is nothing to HOIST, because a materialized view IS the per-call allocation. |
| `genstdlibload.sh <n> <synth\|real> <outdir>` (`MODULES=`, `DECLS_PER_MODULE=`, `FUNCS_PER_FILE=` env) | `n` corpus functions spread over files, plus `MODULES` extra modules shaped like `stdlib/helpers/string/utf16.maxon` and NEVER CALLED — `synth` generates them, `real` copies the actual stdlib file with every name suffixed | does `phase:parse` cost O(user corpus × modules loaded), or O(user corpus) + O(modules)? ⚠ **The shared ladder cannot ask it, and not for the usual reason**: the corpus DOES exercise the cost (every rung loads the same stdlib) but it can only ever move ONE axis, and a product term and a sum term are indistinguishable when one factor never varies. ⭐ **It matters far more than today's number** — the compiler loads ALL of `stdlib/` on every compile (every `.maxon` file there, less the one named exclusion `Internals.maxon`), so a product term would turn a 0.005% tax into a superlinearity every user pays. ⚠ **Written 2026-07-28 as `genwhitelist.sh`**, when the loader still chose its modules from a whitelist and the question was whether that list's intended end state — naming every file — was safe to reach; the whitelist was DELETED on 2026-08-31 having reached it, and **the ladder's subject was never the filter, only the count of modules LOADED**, so it survives unchanged. ⭐⭐ **VERDICT: A SUM, and the reading is EXACT rather than approximate** — extra `phase:parse` allocations vs `MODULES=0` came out IDENTICAL TO THE ALLOCATION at every corpus size: +1,914 / +7,656 / +30,632 / +122,512 at `MODULES` 1/4/16/64 (counts LARGER than `stdlib/` itself, so the end state was priced directly), the same at C=128, 256 and 512 AND on the real shared corpus at rung 0 and rung 5 (a 32× span). One real stdlib module measured three-binary behaves identically (`head − nowl` = **+1,654 parse allocations at every one of the six rungs**, FLAT). ⚠ The documented `O(files × types)` hazard was tested separately with modules carrying array literals and is also absent — the only cross term found has the WRONG SIGN (an extra module gets marginally cheaper as the program grows). |
...
| `genmanagedsites.sh <n> <onefunc\|spread\|nosites> <outdir>` | `onefunc`: <n> `__ManagedMemory` `set`+`get` pairs straight-line in ONE function; `spread`: the same 2n sites one pair per function (the CONTROL); `nosites`: <n> straight-line statements and no managed primitive at all | the two axes `inlineManagedPrimitives` has, neither of which `ScaleCorpus` can move: it wraps every array/buffer group in an `ownedSiteBlock` (`if acc > 0 '…'`), so it can double how many sites EXIST and can never raise how many share ONE BLOCK. `onefunc`/`spread` is the sites×block-length PRODUCT term the splitter would pay if a carve site were ever not the last op in its block — measured LINEAR 2026-08-25 (×1.99 ×1.99 ×2.00 allocations), and the script's header says WHY, because `splitBlockInPlace`'s own header gives a reason that does not follow. `nosites` is the SCAN axis, and it found something: two payload-bearing unions on the per-op path cost **2 allocations and 64 bytes per Std op in the program** (594 / 1,106 / 2,130 / 4,178 across a doubling program with ZERO expandable sites, now 6 / 6 / 6 / 6). A flat row is the answer; a row that grows means a union is back on that path. |
| `genleafsites.sh <n> <onefunc\|spread\|nosites\|guardone\|guardspread\|pressure> <outdir>` | `onefunc`: <n> calls to a 4-op pure leaf straight-line in ONE block; `spread`: the SAME <n> sites one per `if` body (the CONTROL); `nosites`: the same <n> calls with the callee made `throws`, so every site is a `tryCall` the pass never looks at, plus ONE seeded inlinable site; `guardone`/`guardspread`: those two shapes with a `regMaskContains`-shaped `int(0 to 63)` parameter, so each splice also mints a SLOW ARM; `pressure`: `genwidelive.sh sum`'s Θ(N)×Θ(N) splitter shape with the opaque made structurally NON-inlinable | the two axes `inlineLeaves` has — plus, in `pressure`, the A/B control the rung's own thesis needs. `ScaleCorpus` moves neither: its only inlinable leaves are called once each (`scaleOpaqueCore` from `scaleOpaque`, `callChain` link 0 from link 1), so it can double how many sites exist and can never raise how many share ONE BLOCK. **`onefunc` is the product term `inlineSitesIn` promises away** by splitting a block's K sites in REVERSE order — forward, each continuation takes the whole remaining tail and the total is Σ(B−pₖ) ≈ K·B/2, which with K ∝ B reads ×4 on a doubling ladder. MEASURED 2026-08-26 at K = 32…512, `phase:inlineLeaves` allocations minus the flat `nosites` baseline of 971: `onefunc` ×2.04 ×2.02 ×2.01 ×2.00, `spread` ×2.03 ×2.02 ×2.01 ×2.00, `guardone` ×2.01 ×2.01 ×2.00 ×2.00, `guardspread` ×2.01 ×2.01 ×2.00 ×2.00 (the guarded shapes cost ~2.1× per site — one block, one call op and one edge more). CPU min-of-3 at the top, floor still in it: ×1.68 (`onefunc` 256→512) and ×1.83 (`guardone`), both under 2. **`nosites` is the SCAN axis and it is the regression guard `genmanagedsites.sh` earned**: a doubling program the pass inlines nothing in reads **971 allocations FLAT at every rung** — zero per Std op walked and zero per block, because `LeafOpRole`/`LeafVerdict` are payload-free enums. A row that GROWS with <n> means a payload-bearing union is back on the per-op path. ⚠ It seeds ONE inlinable site on purpose: `inlineWithNameIndex` RETURNS after `buildLeafPlan` when nothing is eligible, so a ladder with zero of them never reaches `collectSites` and measures half the pass. **`pressure` is not a ladder for this pass at all** — it is how you ask whether the ALLOCATOR'S OWN emitted code got faster, which a self-compile A/B cannot answer because there the pass changes regalloc's input and regalloc's code at once. Zero splices anywhere ⇒ two compilers differing by this pass hand regalloc a bit-identical program (assert it: their per-phase ALLOCATION counts must match to the digit). MEASURED 2026-08-26, n=400, three interleaved pairs: every phase's allocations identical, `regalloc` ×0.877 and `regalloc:splitting` ×0.874 in CPU with every non-regalloc phase at ×0.96–1.01 — the 12.6% that inlining `regMaskContains` is actually worth. Every mode's program must print the same answer built with and without the inliner (checked: 2128 / 203 / 2130 / 3 / 3 / 7). |
| `gentestproject.sh <files\|onefile\|broken> <n> <m> <outdir>` | ⚠ **the odd one out: it writes a PROJECT DIRECTORY, not one `.maxon` file** — `n` `*.test.maxon` files of `m` tests each; `onefile` is the same source and test count with ONE test file; `broken` plants an E3005 per file | `maxon test` ITSELF — discovery, `groupTestsByFile`, the staging, the executor and both report faces. ⛔⛔ **`ScaleCorpus` compiles ordinary programs, so a default run measures this command's cost WHEN ABSENT, and that is a measured exact zero** (entry defaults to `main`, walk selection to `productionOnly`, compiler-written-source list empty): every one of those routines is reached by NOTHING on the standing ladder, and no number of rungs will change it. **The two knobs are independent** — test FILES (a group, a dispatcher, a process launch) against TESTS (an index, a guard, a report line) — which is what a ladder doubling both can never separate. ✅ **FOUND A QUADRATIC AND CLOSED IT**: `groupTestsByFile` collected the distinct files and then re-walked EVERY test for each, Θ(files × tests). `files 512 8` read **11,357 ms** against `onefile 512 4096`'s **684 ms** for the same sources, the same 4096 tests and the same tokens — 16.6×, and the only difference is the GROUP count. Holding T at 2048 and doubling F read the product straight off the rungs (552 / 1073 / 3062 / 6099 ms at F = 16/64/256/512); one pass with the previous group tried first reads 380 / 414 / 596 / 1070. ⭐ **ACCEPTANCE WAS BYTE-IDENTITY of `--list` at all four F**, not a green case — an optimization has no red. ◑ Two Θ(sources × test files) terms MEASURED AND LEFT (`groupOfFile`, `Queries.compilerMintedNamesIn`): 4509 ms at F=512 against 1795 at F=1 for the identical program, crossing over only in the hundreds of test files. ⚠ A third, `Diagnostics.authoredPathFor`, is Θ(diagnostics × sources) and is INSIDE THE NOISE — sweeping the diagnostic count 512 → 1 at fixed S read 4509 → 4533 ms. |
| `genunswitch.sh <loops> <pad> <versioned\|refused> <outfile>` | ONE function holding `loops` loops of the shape `unswitchInvariantGuards` versions — an element store and a `get` on a parameter array, an accumulator that escapes every loop — followed by `pad` straight-line calls; `refused` adds a `push` to every loop so each is refused at its first call | the two axes of the loop-versioning pass, neither of which `ScaleCorpus` can move: **no rung holds an element store inside a loop**, so the pass versions ZERO loops at rung 0 and rung 5 alike and its `phase:unswitchInvariantGuards` row measures only the per-function gather. `loops` is the doubling knob; `pad` is the PRODUCT axis (hold `loops`, double `pad`: a per-loop cost proportional to the function doubles here); `refused` is the CONTROL (the gather and the early refusal, no analysis past the calls, no rewrite). MEASURED 2026-09-09: the first cut read ×3.70 ×3.84 ×3.91 in allocations across 50→400 loops and +400,000 allocations for +4,000 pad ops at 100 loops — three per-loop walks of the function (the exit-phi substitution rebuilt every op of every block per loop; the hoist numbering's canonical map and the clone's substitution columns were function-sized and built per loop). After the rewrite ×2.01 ×1.97 ×2.00, and flat for the same pad (75,399 / 74,720 / 74,758). `versioned` with `pad` 0 checks its own sum and exits 1 on a wrong one. |
| `genclonegate.sh <sites> <fields> <gate\|control> <outfile>` | ONE `main` holding `sites` consumed-argument sink sites over a struct whose field graph is `fields` `Array with Integer` fields; `control` appends ONE trailing `String` field, which classifies `notImageable` so `everyStructSlotIsImageable` answers false and the `and` short-circuits BEFORE the graph walk | the DEEP-CLONE GATE's full type-graph walk (`ProgramSignatures.aggregateSupportsDeepClone`, reached per emitted VALUE through `Parser.coOwnConcreteRecordForSink` -> `valueIsAnImageableDeepBox` -> `structBoxIsImageableWithSlots` -> `structBoxHasADeepCloner`), whose cost is O(sites x fields) and which `ScaleCorpus` structurally cannot see: it doubles the FUNCTION count at a fixed body size, and every term here is per-function-body. The population is not `.clone()` tokens — the gate is asked at every borrowed struct value reaching a durable sink. ⭐ **`control` is the CONTROL and the point of the ladder**: identical site count through the identical door over the identical field graph, so the DIFFERENCE between the two modes' `phase:parse` columns is the walk plus ONE per-site emission term — an admitted gate takes `emitOwnDeepBoxCopy` where the control takes `emitOwnBoxForSink`. ⚠ **That term is flat in `fields` exactly as the walk's per-ask overhead is, so flatness attributes a residual to neither**; what subtracts it out is a BEFORE/AFTER of gate−control across two compilers, which is how this ladder is read. ⚠ **Every site lands in ONE `main`, so `regalloc` dominates the wall clock and bends on its own known term** (`genopaquefields.sh` carries the same warning) — read `phase:parse`. Both modes return **7** at every rung, so a miscompiled rung shows as a wrong exit code rather than merely as a time. |
| `genexistentialclone.sh <holders> <conformers> <types> <gate\|control> <outfile>` | `holders` DISTINCT record types, each holding one field at the interface `Shape` and each `.clone()`d once in `main`; `conformers` types implement `Shape`; `types` unrelated `Pad` structs pad the declaration registry the conformer roster scans; `control` is the same program with the interface-typed field replaced by a `String` one, so `typeSupportsDeepClone`'s `interfaceRef` arm is never reached and the roster is never listed | the CLONE GATE'S PER-CONFORMER TERM: `typeSupportsDeepClone`'s `interfaceRef` arm asks `interfaceConformersSupportDeepClone` -> `cloneConformerRosterOf` (a full pass over every struct name AND every enum) plus one aggregate graph walk PER CONFORMER, per interface-typed field per gate ask. `ScaleCorpus` structurally cannot see it: no corpus program holds a field at an interface type, so every column reads a delta of 0 that means UNREACHED. ⭐ The knobs are the product: `holders` is how often the gate is asked, `conformers` is how long the roster is, `types` is how much registry the roster scan walks. Doubling `holders` and `conformers` together is the reading — a term that is per (holder x conformer) shows here and on neither single axis. Both modes return **7** at every rung. |
...
arity ≥ 14 panics the register allocator (`RegisterAllocator.maxon:1166`, `chooseRegister: no free
register`). Measured on `main` as well as on the rung under test — arity 13 compiles and 14
...
⚠ **Build this one as an ordinary program** (`maxon build <out>`). Its `File.delete` call goes through
this same `stdlib/File.maxon` and this same emitted runtime, so it runs the identical machine code
