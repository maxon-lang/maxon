...
| `genstdlibload.sh <n> <synth\|real> <outdir>` (`MODULES=`, `DECLS_PER_MODULE=`, `FUNCS_PER_FILE=` env) | `n` corpus functions spread over files, plus `MODULES` extra modules shaped like `stdlib/helpers/string/utf16.maxon` and NEVER CALLED — `synth` generates them, `real` copies the actual stdlib file with every name suffixed | does `phase:parse` cost O(user corpus × modules loaded), or O(user corpus) + O(modules)? ⚠ **The shared ladder cannot ask it, and not for the usual reason**: the corpus DOES exercise the cost (every rung loads the same stdlib) but it can only ever move ONE axis, and a product term and a sum term are indistinguishable when one factor never varies. ⭐ **It matters far more than today's number** — the compiler loads ALL of `stdlib/` on every compile (every `.maxon` file there, with no exclusion at all), so a product term would turn a 0.005% tax into a superlinearity every user pays. ⚠ **Written 2026-07-28 as `genwhitelist.sh`**, when the loader still chose its modules from a whitelist and the question was whether that list's intended end state — naming every file — was safe to reach; the whitelist was DELETED on 2026-08-31 having reached it, and **the ladder's subject was never the filter, only the count of modules LOADED**, so it survives unchanged. ⭐⭐ **VERDICT: A SUM, and the reading is EXACT rather than approximate** — extra `phase:parse` allocations vs `MODULES=0` came out IDENTICAL TO THE ALLOCATION at every corpus size: +1,914 / +7,656 / +30,632 / +122,512 at `MODULES` 1/4/16/64 (counts LARGER than `stdlib/` itself, so the end state was priced directly), the same at C=128, 256 and 512 AND on the real shared corpus at rung 0 and rung 5 (a 32× span). One real stdlib module measured three-binary behaves identically (`head − nowl` = **+1,654 parse allocations at every one of the six rungs**, FLAT). ⚠ The documented `O(files × types)` hazard was tested separately with modules carrying array literals and is also absent — the only cross term found has the WRONG SIGN (an extra module gets marginally cheaper as the program grows). |
