...
#   ⇒ The confirmation that the generalization holds is a REAL loader A/B — rebuild the compiler with a
#     stdlib module withheld and diff the two builds over one corpus. The two routes agree to FOUR
#     ALLOCATIONS OUT OF 1,650: the real `helpers/string/utf16.maxon` dropped in as an ordinary USER
#     file (this ladder's `real` mode) costs **+1,650** `phase:parse` allocations, flat at corpus rung 0
#     AND rung 5; the same file reached through the LOADER costs **+1,654**, flat at all six rungs. The
#     four are the loader's own — one more `File.exists`, one more set insert, one more `join`. Do the
#     rebuild when a reading here is surprising; do not do it FIRST, because it costs a compiler build
#     where this costs a shell loop.
#
...
#
# ⚠ **THE CORPUS BODIES MUST NOT FOLD AWAY.** The compiler runs a real `foldConstOperands`, so a corpus
# of constant arithmetic compiles to nothing and every curve is a beautiful straight line through zero.
...
#   parent commit) were built by an IDENTICAL compiler and driven interleaved from equal-length paths.
...
emit_corpus() {
	...
	while [ "$f" -lt "$FILES" ]; do
		{
			echo "// stdlib-load ladder corpus file $f of $FILES — see tests/ladders/genstdlibload.sh"
			...
			while [ "$i" -lt "$FUNCS_PER_FILE" ]; do
				...
				echo -e "\tvar acc = a"
				# Two `if`s rather than an if/else: the compiler does not accept `else` (E2015), and
				# the corpus this ladder sits beside does not emit one either.
				echo -e "\tif b > a 'gt'"
				echo -e "\t\tacc = acc + b"
				echo -e "\tend 'gt'"
				echo -e "\tif b <= a 'le'"
	...
	{
		echo "// stdlib-load ladder entry point — see tests/ladders/genstdlibload.sh"
		...
		# exists to measure. (`main` takes no parameters, so an argv-derived seed is not available.)
