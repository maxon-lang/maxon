...
#              THE O(n)-vs-O(n²) QUESTION.** `__str_trim` scans FORWARD ONLY (a backward
#              `findGraphemeStart` has no GB12/GB13 rule and would split a flag the forward segmenter
#              joins), so it walks the WHOLE string on every call. A forward scan that re-derived the
#              cluster start per candidate would read ×4 here.
#
...
# the timed region; wall nanos are used because the emitted program has no thread-CPU intrinsic, so
# measure on an idle box and take the MINIMUM of several runs. Against the ×2-vs-×4 question that is
# ample; against a few percent it is worth nothing.
#
...
#   ×1.96. Nothing bends.
#
#   ⭐ **WHAT THE RUNG COSTS A PROGRAM THAT NEVER USES IT — path-clean INTERLEAVED A/B against the tree
#   WITHOUT the rung, both built by an IDENTICAL compiler (md5-identical) and driven from EQUAL-LENGTH
...
#   1,352 → 17,224, and IDENTICAL growth ratios across the A/B (×1.80 ×1.89 ×1.94 ×1.97 ×1.98).
#
...
emit_loop_program() {
	...
	loop-trim-shared)
		...
		;;
	loop-charset)
		# The set construction ALONE — no trim, no owned result String. This isolates the per-call cost
		# of building a preset rather than reusing one.
