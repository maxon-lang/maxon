#!/usr/bin/env python3
"""Regenerate every fmt fixture's expectations BY RUNNING THE COMPILER.

Every `expected-*` file and every `expected-tree/` in this corpus is the formatter's
real answer, never a hand-written one, so the corpus pins what `maxon fmt` DOES and
a later run that differs is a regression to read. It cannot tell you an answer is
WRONG -- nothing outside the formatter votes on that -- so read the diff when one of
these moves.
...
def main():
    slot = os.path.join(ROOT, "maxon-bin", ".maxon")
    exe = os.path.join(slot, "maxon.exe")
    if not os.path.exists(exe):
        exe = os.path.join(slot, "maxon")
    if not os.path.exists(exe):
