...
`runs-alone.test.maxon`, beside this directory, runs `spec-test` over a copy of it. This spec's one case is
an ordinary SHARED job: it marks itself busy, holds that mark for a second, then replaces it with a done
