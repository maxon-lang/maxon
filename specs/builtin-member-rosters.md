...
`String`'s half is pinned in `specs/stdlib-only-string-methods.md` — the defect the derivation was
...
no longer the COMPILER's — `stdlib/String.maxon` declares it and W49 wave 3 struck the compiler's second layout —
but its two accessors keep their arms under the roster-wins rule, so the roster keeps its obligation.

`Character`'s is here, because it had none at all and its own spec file (`specs/character-type.md`)
is ported byte-identical from `/specs` and may not gain an compiler-authored case.
