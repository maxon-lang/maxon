...
⚠ **THE NOUN IS THE POINT.** A payload is a positional slot of ONE CASE, not a
member of the union. `unknown parameter name` (E3003) is the code for a name with no binding in
scope, and calling it a FIELD points the fix at the
wrong declaration. The compiler names the case, and the code (`E3144`) says which of the two mistakes this is.
```maxon
...
says which way it disagrees, naming both counts.
```maxon
