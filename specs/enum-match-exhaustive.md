...
Enum match expressions require exhaustive case coverage, just like enum matches. Every enum case must be named by some arm. Plain `default` is not allowed — use `default throws` if you want a catch-all that throws an error.

An arm covering several cases names each of them with `or`, one alternative per line:
...
Every alternative counts toward coverage. A RANGE of cases (`low to medium`) is E3146.
