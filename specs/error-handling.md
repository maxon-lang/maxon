...
// corpus writes with a labeled union-case argument the compiler does not yet parse.) A leak or double-free
...
function's error flag exactly as a `throw` statement does, so it owes the declared clause the same debt.
Before this rule reached it, the arm quietly accepted `default throws ErrB.bZero` inside `throws ErrA`.
The compiler has one emission site, so this arm inherits the rule. Pinned so a fourth spelling cannot reopen it.
```maxon
typealias Code = int(0 to u32.max)
