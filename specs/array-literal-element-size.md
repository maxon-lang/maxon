...
**Why the width cannot be settled at PARSE time.** The first element's VALUE
KIND is not enough: a non-constant narrow element (e.g. a `ByteArray.get` result)
is integer-storage there, so the width comes out 8 and every slot store lands 8
bytes apart. Sizing it correctly requires the RESOLVED element type, which only a
post-resolution pass has.
