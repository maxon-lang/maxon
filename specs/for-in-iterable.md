...
`current()` + a throwing `advance()`, and the compiler has walked that directly since conditional extensions.
...
flow and OWNERSHIP are two answers, and the compiler's ownership is static — one owner, dropped at one point,
