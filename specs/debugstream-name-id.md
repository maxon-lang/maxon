...
(`specs/debugstream-log-events.md`). A member that is neither still earns the reserved-callee
...
its own — a hole is exactly the thing that cannot be known while the call is parsed. It is the same
answer `__Builtins.ucdByteAt`'s label one door over gives.
