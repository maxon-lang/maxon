...
**Registers and spills are two different carriers, and a function has only one of them.** the compiler's
...
⚠ **The band must be live ACROSS the loop, never inside it.** The compiler REFUSES a loop whose working set
...
**A generic body is DICTIONARY-PASSED, not monomorphized.** The compiler compiles one body and passes a witness
...
declaration. The compiler dictionary-passes instead of monomorphizing, so the body carrying these names is
...
type Accum uses T
	...
	function fold(bump Tick) returns Tick
		var total = bump
		var step = 1
		var rounds = 0
		while rounds < 3 'roll'
