...
no leak, no double free. (A never-reassigned `var` is linted E3077, so the reassignment
is WRITTEN and then skipped at run time behind a `false` argument — which is exactly what this case
is about. The value 5 is the else-path's twin, where it is likewise skipped.)
...
The retain makes `q` a genuine OWNER, so `let r = q` MOVES `q` into `r` (The compiler is move-only for
