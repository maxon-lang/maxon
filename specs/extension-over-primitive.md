...
registry holds — an `extension float` block declaring its `__self` as a *named* Float rather than the
float primitive breaks `"{self}"` interpolation. `Self` here is the primitive's
...
`trunc` rather than `as Integer`: a float→int CAST is `E3009 Cannot cast from float to int` in the compiler with no
...
⭐ `returns Self` on a primitive extension. `Self` is the primitive, so the returned value is an
ordinary `int`.
```maxon
extension int
...
⭐⭐ **THE DIFFERING-DECLARATIONS CONTROL for the conformance half.** the compiler's synthesized `int.hash`
