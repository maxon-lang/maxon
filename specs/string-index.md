...
```
```stderr
panic at String.maxon:380: String.sliceBytes: caller guarantees 0 <= start <= end <= byteLength()
Stack trace:
...
```
```maxoncstderr
error E2015: <fragment>:2:6: Unsupported: a declaration of the type name 'StringIndex', which the compiler owns — its one meaning comes from the compiler itself or from the stdlib module that declares it, and the compiler has no namespace to tell a user declaration of the name apart from that one
```
...
```
```maxoncstderr
error E2015: <fragment>:5:12: Unsupported: `StringIndex` member 'offset' — the compiler provides charIndex/bytePos; that list IS the surface, so nothing else is served here
```
