...
A zero low bound with a 64-bit top is the *only* shape on which the two readings can be told
apart: a value of `int(0 to 100)` is provably non-negative, so an arithmetic and a logical shift of it
agree bit for bit. Any case that means to pin this rule has to use it — which is why `8 shr 2` cannot.
...
⚠ Go: "the shift operators implement arithmetic shifts if the left operand is a signed
integer". A bare Maxon `int` IS signed, so `shr` sign-propagates on one: `sar` is emitted and
`(0-8) shr 60` is -1, not 15.

⚠ And "here" was the whole of it. The clause the rule turns on is *"if the left operand is a signed
integer"*, and the compiler read it as though every left operand were — see `shr-unsigned-operand-zero-fills`
below, which is the same sentence's OTHER half and which the compiler got wrong for exactly as long as this
one went unpinned.
```maxon
function main() returns ExitCode
	let neg = -8
...
This is easy to get wrong. A shift that took its optimal type from `lhs ?? rhs` would read a count
declared `int(0 to 63)` — an *unsigned* optimal type, and the most natural way there is to declare a
...
under the arithmetic one, so it is the one shift that can tell the two readings apart — and until
it was written down, the compiler answered **-1** for this program.
...
spec that cannot fail is what let this through, and only an operand whose top bit is set fixes that —
which is what `bits(64)` is for.
...
operand is read in**. It is easy to lose: reading a variable outside the block that assigned
it mints a fresh SSA value, and resolving a ranged type by *searching for a variable
currently bound to that value* finds nothing, concludes "not a ranged type", and therefore
"signed" — `u64.max shr 60` then reads **15** in the entry block and **-1** after an `if`.
...
value** with two different declared types. Resolving a shift's ranged type by searching for a variable
bound to that value returns whichever the search reaches first — `p`'s — so `w shr 60` would sign-fill
a type declared unsigned and answer **-1**, where the same shift on a parameter declared `Wide`
answers **15**.
...
The case above pins that the CAST decides the fill; these two pin what that costs, and the compiler got both
...
leaves the unsigned→signed direction reading the SOURCE, and the compiler answered 15 where it must
answer -1.
```maxon
typealias Wide = bits(64)
typealias Num = int(i64.min to i64.max)
...
`bits(64)` parameters produces a value the program never named a type for, so every later
...
BE outside `0..63`, so the whole of it computes a mask that is always zero, and the compiler hands the
...
Masked, `1 shl 100` would be `1 shl 36` = **68719476736**. It is 0 here.
```maxon
var wide = 90
typealias Bits = int(0 to 63)
