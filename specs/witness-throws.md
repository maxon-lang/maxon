...
An implementation may not throw a DIFFERENT error type than the interface method declares (E3016).
A MONOMORPHIZING compiler could accept it: a `try` there is typed off the callee's own clause,
so the caught error is whatever the impl actually threw. The compiler dictionary-passes, so the `try` at the dispatch
