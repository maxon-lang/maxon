...
`String` member on arity. On that shape (`g(a Num)` beside `g(a Num, b Num = 5)`, called `g(1)`) the
ONE-parameter member is the correct selection, so a fill-then-resolve that quietly took the defaulted
member would be a wrong ANSWER rather than a missing refusal. The sibling that defaults nothing is
