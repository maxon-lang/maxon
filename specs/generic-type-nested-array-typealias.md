...
`fill`, so it never becomes a gainer and the fixpoint never republishes it. It passes, and that is what
makes it the control — the two programs differ only in which method spells the push.
