...
The compiler answers it with a whole-program summary (`SemanticCheck.buildEffectFreeSummary`) whose contract is
...
### ⚠ ALL THREE SPELLINGS OF ONE FIELD WRITE ARE ACCEPTED, DELIBERATELY

Bare `n = n + 1` inside a method, `self.n = self.n + 1` inside a method, and `c.n = 7` through a
`c Counter` parameter are one write in three spellings, so they are one effect. A classification that
refused any of them would refuse code that should compile.
