...
the compiler's parser does not INFER an omitted parameter type; one call position hands it down. A container's
...
parameter has no second position to be typed from — and nothing to infer it by, since the compiler has no
...
with the same sentence; the reference bootstrap refuses it too (its anchor is the `)`, the compiler's is the
parameter name).
```maxon
typealias Integer = int(i64.min to i64.max)
...
```
```maxoncstderr
error E2015: <fragment>:4:27: Unsupported: parameter 'b' with no type — the compiler infers an omitted parameter type in exactly one position, a closure passed to a container's `map`, whose transform takes the container's own element; every other parameter declares its type
```
...
```
```maxoncstderr
error E2015: <fragment>:19:24: Unsupported: parameter 'x' with no type — the compiler infers an omitted parameter type in exactly one position, a closure passed to a container's `map`, whose transform takes the container's own element; every other parameter declares its type
```
