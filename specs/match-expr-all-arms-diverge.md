...
⚠ **THIS SPEC PINS A BOUNDED GAP IN the compiler, NOT A RULE OF THE LANGUAGE.** The programs below are legal
Maxon — the value binding is simply never reached, because control leaves through the arm that ran.
The compiler refuses them. The refusal is the
...
**The compiler has no `never`-typed expression result to stand in for the missing value.** `parseExpression`
