...
`where Key is Hashable and Equatable`, and `NotHashable` meets neither — so the compiler reports both.
Every unmet constraint is a separate thing the author has to supply.
