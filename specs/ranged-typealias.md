...
- A type-qualified bound may be paired with a literal — `0 to u32.max` and `0 to i64.max` are both valid ranges
...
value passes rather than tripping the naive signed lower check.
...
The half a literal cannot reach, at the position that has no callee entry guard to fall back on. ⚠ A
runtime guard keyed on the parameter's DECLARED type — the opaque `T` — would admit `b.put(grow(5))`
and exit **505**; the case is written against the language rather than against that reading.
...
`main$closure_0` is on the stack when it fires.
```maxon
typealias Small = int(0 to 10)
typealias Wide = int(0 to 100000)
typealias Fn1 = function(Wide) returns Wide
...
⭐ **THE SECOND CAST DOOR.** A top-level `let`'s initializer never reaches the expression path — it is
folded by the const evaluator, a different walk over different code. A body cast that refused the bare
keyword while the const walk accepted it as an "identity cast" would compile this exact program and
refuse the identical cast inside a function: one mistake with two verdicts, decided by nothing but where
the `let` was written. Both doors ask the same question and give the same sentence at the same anchor.
```maxon
