...
the compiler selects one FIELD of the backing at a time and does not materialize the record, so `.rawValue` with no
field after it names a value this compiler does not build. The refusal says that, rather than implying
the value cannot exist.
