...
ran — one operation, two spellings, disagreeing. The compiler has never had that split; the case is here so
...
`specs/lazy-static.md` does not yet carry, and it needs a guard word per scalar static rather than
