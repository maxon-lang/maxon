...
answered **0**, so the `fromRawValue` on the next line THREW. ⚠ The rule is to *"mirror the parent case's
ordinal so `.rawValue` agrees with the union's tag value (the natural pairing for de/serialization)"*.
