...
type Item
	...
	static function clone(src Self) returns Self
...
type GenModule uses Op implements Mergeable with Item
	...
	function getFunc(index Count) returns Item
		return try self.functions.get(index) otherwise 'unreachable'
			panic("getFunc: out of bounds")
		end 'unreachable'
	end 'getFunc'

	function pushFunc(func Item)
		self.functions.push(func)
	end 'pushFunc'

	function cloneFunc(src Item) returns Item
