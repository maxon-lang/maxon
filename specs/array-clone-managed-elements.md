...
the CORPUS rule and not a compiler choice: `specs/memory-safety.md` says `.clone()` yields "a new, independent
copy", and an element reached through `get` is a mutable heap record — so an element carried into the copy by
INCREF makes a write through the copy a write to the original. When the source array is later freed,
the clone's elements survive because they are separate allocations.
...
The shape the compiler's own worker pool is built on (`Array with SpecTestResultArray`): the thunk's inner
