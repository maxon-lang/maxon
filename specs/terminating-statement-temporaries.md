...
`specs/panic.md` is a byte-identical port of `/specs/panic.md` and may not gain
...
- **arm64-macos REFUSED to compile it.** `panic at RegisterAllocator.maxon:1032:
...
written out rather than deleted because a `unsupported-targets:` line that names everything is falsifiable: add
