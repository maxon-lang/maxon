...
### Union Comparison Restriction

Union values cannot be compared using `==` or `!=` operators. The only way to inspect a union value is through `match` statements or expressions. This prevents a class of bugs where a new case is added to a union but existing code silently falls through or uses a wrong default value — the compiler forces every match site to handle all cases explicitly.

```text
// This is a compile error:
if dir == Direction.north 'check'  // ERROR: cannot compare union values with '=='
  ...
end 'check'
