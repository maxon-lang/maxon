...
`stdlib/Array.maxon:115-115` declares the static every `Array` has:
...
### In the compiler it is an IDENTITY on the record, not a copy
...
```
```maxoncstderr
error E2015: <fragment>:6:20: Unsupported: `Array` static method 'nosuch' — the compiler provides two: `create()` and `init(managed)`
```
