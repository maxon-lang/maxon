...
```
```maxoncstderr
error E2015: <fragment>:4:13: Unsupported: `__ManagedMemory` member 'frobnicate' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
right answer. `specs/stdlib-array.md`'s `push-self-assignment` documents this exact program as
...
siblings in `managed-file.md` this case needs no `<!-- unsupported-targets: -->` marker and no file to exist.
...
pinned byte-for-byte by `specs/error-handling.md:634`. The outer pair is E3059's own
`type mismatch: '…'` frame; the inner pair quotes the noun inside it. Do not "fix" either half.
```maxon
enum MyError
	bad
end 'MyError'
