...
Callers convert via `mm.toCString()`, which hands the buffer straight back
...
function main() returns ExitCode
	...
	_ = __Builtins.subprocessResolveOnPath(s.toByteArray().managed)
	return 0
end 'main'
```
```maxoncstderr
...
The wording divergence is `error-subprocess-resolve-on-path-managed`'s, above.
```maxon
function main() returns ExitCode
	_ = __Builtins.subprocessResolveOnPath(42)
	return 0
end 'main'
```
```maxoncstderr
error E3005: <fragment>:3:17: '__Builtins.subprocessResolveOnPath' requires a cstring, but its argument is int
```
...
function main() returns ExitCode
	let s = "abc"
	let cs = s.cstr()
	_ = __Builtins.subprocessGetPid(cs)
	return 0
end 'main'
```
```maxoncstderr
error E3005: <fragment>:5:17: '__Builtins.subprocessGetPid' requires a int, but its argument is cstring
```
