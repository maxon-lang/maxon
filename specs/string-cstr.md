...
`s.cstr()` therefore left the process leaking and exiting **101**. The cure is COW: grow to
`length + 1`, terminate in place, hand back the record's own buffer (through `__managed_reserve`).
...
- **The out-of-bounds READ half is caught only by an accident of the allocator, and this spec says so
  ...
  bootstrap's half has one (`Directory.exists`, pinned by `specs/directory.md`) and the compiler's does not.
