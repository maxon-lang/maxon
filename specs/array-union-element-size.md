---
feature: array-union-element-size
status: stable
keywords: [array, union, element-size, push, grow, memory-management]
category: memory
---
# Array of Union: Element Size and Push Correctness

## Documentation

When an array holds elements of a union type (with associated values), the backing `__ManagedMemory` must have `element_size = 8` (heap pointer size). If `element_size` is incorrectly set to 0, every push computes `buffer + index * 0 = buffer`, always overwriting slot 0, and grow computes `newCap * 0 = 0` bytes, never actually growing the buffer. The array appears to have the right `count()` but only the last-pushed element survives; all earlier elements were decref'd and freed. Cascading cleanup then crashes on the stale pointers.

## Tests

<!-- test: union-array-push-count -->
### Push to array of union preserves count
Basic verification that pushing multiple union values gives the correct count.
```maxon
...
<!-- test: union-array-push-get-last -->
### Get last element from union array
```maxon
typealias Integer = int(i64.min to i64.max)
...
### Nested union (union wrapping union) in array
...
### Struct with union array field: cascade free must not crash
When a struct holding a union array is freed, the cascade must correctly
