...
rather than materializing a boolean. See `specs/if-statements.md`.
...
instead, the `-1` could not be returned at all: that alias is a quantity, and its return door refuses
a signed negative.
