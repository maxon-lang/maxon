...
`array-hashable.md` is ported byte-identical from `/specs` and may not gain an compiler-authored case — the
...
`clear()`) used to be refused too, because the refusal fires where `stdlib/Array.maxon:147`'s
...
NOTE** — the arrangement the remaining opaque-gate cases print. It is RAISED at `stdlib/Array.maxon:166:32`,
...
note: stdlib/Array.maxon:166:32: raised inside the library, on behalf of the construct above
```
...
String, Val)` — a key type that satisfies `Hashable` and `Equatable` perfectly well — takes the SAME
refusal … so the gate refuses valid ones."* Its instantiation IS
