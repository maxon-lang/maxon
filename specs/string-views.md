...
declared in `stdlib/Character.maxon`, which the compiler cannot load (`Character` is a name the compiler owns),
...
the element type of the slot receiving it, exactly as a bare `3` is built at a `Count` parameter
(`nominal-generic-alias/a-literal-decays-into-a-narrow-ranged-brand`). So `String.from([1, 2, 3])` lays
down three BYTES and is the case below; what this one refuses is a value that already IS an
eight-byte-strided array and says so by its brand.
```maxon
...
```
```maxoncstderr
error E3005: <fragment>:8:17: argument type mismatch for 'bytes': expected 'ByteArray', got 'IntArray'
```

<!-- test: from-takes-a-bare-literal -->
...
guard fired. Unrestricted, for the reason stated at the head of this group.
...
reaches a runtime check because it never builds"). The subject
...
`Codepoint` is `int(0 to 1114111)` in `stdlib/Character.maxon`, a module the compiler cannot load — so the
