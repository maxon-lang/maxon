...
type Value implements Parsable
	...
	static function fromString(input String) returns Self throws ParseError
		return Value{n: input.byteLength() as Integer}
	end 'fromString'
end 'Value'
...
type Money implements Parsable
	...
	static function fromString(input String) returns Self throws MoneyParseError
		...
		return Money{cents: input.byteLength() as Integer}
	end 'fromString'
end 'Money'
...
cases already propagate. The compiler SYNTHESIZES that enum rather than reading it, because `stdlib/Array.maxon` is
