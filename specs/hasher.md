...
running value through its own structures — the compiler query spine does exactly this — needs no second
implementation of the fold.

The result type is `HashDigest`, `bits(64)` — a raw PATTERN, because the FNV offset basis
`14695981039346656037` is above `i64.max` before anything is hashed and the mixing multiply is defined
to wrap. It is **not** `HashValue`: that alias is 32
...
combine each part's length itself. `maxon-bin/Compiler/ContentHash.maxon` is that call site, and
...
`mixBytes` as `maxon-bin/Compiler/ContentHash.maxon` actually writes it: one expression, the inner
