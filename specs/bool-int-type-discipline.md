...
- `Cannot return 'bool' from function declared to return 'int'` (E3005)

Three cases have no diagnostic of that family to borrow: an `int` condition, `-flag`, and `true +
false`. An internal error leaking to a user describes no defect in the program, so each of the
three is reported as a positioned diagnostic of the same family the rest of the rule uses.
