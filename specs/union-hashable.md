---
feature: union-hashable
status: experimental
keywords: [union, hashable, equatable, map, hash, equals]
category: type-system
---

# Union Hashable & Equatable

## Documentation

Unions without associated values automatically conform to `Hashable` and `Equatable`. This allows them to be used as map keys, call `.hash()` and `.equals()`, and satisfy where-clause constraints.
...
For simple unions, the raw value is the ordinal (an `int`). For backed unions (int, float, String, Character), the raw value is the explicit backing value.

Unions with associated values do **not** automatically conform — they have no single raw value to hash.
