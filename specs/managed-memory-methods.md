...
Out and back through a raw pointer. `specs/string-from-cstring.md` owns the conversion's own cases —
...
```
```maxoncstderr
error E2015: <fragment>:3:37: Unsupported: `__ManagedMemory.create`'s `elementSize` is 4, and the compiler has no element TYPE of that width — a buffer must be byte-packed (1) or machine-word (8). Typing it as a word anyway would check `get`/`set` against a range the buffer's stride cannot hold, and store the value truncated.
```
...
restage, publish again — which is what pins the destroy walk's per-slot NULL GUARD rather than an
...
```
```maxoncstderr
error E3005: <fragment>:7:9: Value -1 is outside the range of 'ElementIndex' (int(0 to 18446744073709551615))
```
...
⚠ **THE `try a.append(b) otherwise …` IN `specs/managed-memory-builtin.md` IS NOT EVIDENCE AGAINST IT.** A
redundant `try` on a non-throwing builtin is refused here, so that line records a permissiveness this
compiler does not have rather than the callee's contract.
```maxon
function main() returns ExitCode
...
```
```maxoncstderr
error E2015: <fragment>:4:5: Unsupported: `__ManagedMemory` member 'push' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:13: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
declared on `Array` (`stdlib/Array.maxon:272`) and NOWHERE else: not in the corpus, and no corpus
caller writes `mm.appendMemory(…)`. The envelope collapse makes the
...
```
```maxoncstderr
error E2015: <fragment>:5:9: Unsupported: `__ManagedMemory` member 'appendMemory' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:9: Unsupported: `__ManagedMemory` member 'managed' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:17: Unsupported: `__ManagedMemory` member 'pop' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:17: Unsupported: `__ManagedMemory` member 'first' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:17: Unsupported: `__ManagedMemory` member 'last' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:9: Unsupported: `__ManagedMemory` member 'insert' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:5: Unsupported: `__ManagedMemory` member 'reserve' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:5: Unsupported: `__ManagedMemory` member 'resize' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:13: Unsupported: `__ManagedMemory` member 'isEmpty' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:4:13: Unsupported: `__ManagedMemory` member 'clone' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:6:15: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:3:12: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:8:11: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:12:15: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:10:13: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:11:18: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
...
```
```maxoncstderr
error E2015: <fragment>:15:15: Unsupported: `__ManagedMemory` member 'count' — the compiler provides length/capacity/get/set/setLength/setByte/byteAt/elementSize/grow/fill/toCString/makeCharFromBytes/append/slice/clear/remove/swap/shiftRight/shiftLeft/createCursor; that list IS the surface, so nothing else is served here
```
