 ...
 * fixture's `.maxproj` and reads `collectMaxonSources` from there, which never consults a marker
...
/**
 * A fixture project of its own, staged on disk and rooted by an empty `.maxproj`.
 *
 ...
 *
 * The `.maxproj` marker is what stops the server's project-file climb at the fixture, so its
 * project is the fixture's own files rather than the whole workspace.
 */
