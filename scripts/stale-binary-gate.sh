...
# CORPUS pinned inside a gate that is not about the corpus. the compiler's `--filter` is a SUBSTRING match on
...
if grep -q 'STALE' "$WORK/out.log" \
	...
	&& grep -q 'maxon-bin\.exe\|maxon-bin$\|\.maxon[/\\]maxon-bin' "$WORK/out.log" \
	&& grep -q 'build\.sh' "$WORK/out.log"; then
