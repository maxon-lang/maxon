#!/usr/bin/env bash
# Regenerate `maxon-bin/Compiler/Runtime/SlabClasses.maxon` from Go's VENDORED size-class table.
#
# ⭐ WHY A GENERATOR AND NOT A HAND-TYPED TABLE. the compiler's allocator is Go's design, so its size classes
...
out="$root/maxon-bin/Compiler/Runtime/SlabClasses.maxon"
