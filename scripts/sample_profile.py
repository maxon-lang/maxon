...
So a binary the compiler emitted — the stage-2 compiler of a self-host chain, which
...
code the compiler's own codegen produced.
...
def load_symbols(path):
    ...
    # A bootstrap-emitted binary carries the table in `.symtab`; an compiler-emitted
...
def main():
    ...
    # the symbols would load and the launch die. An absolute path is unambiguous to both.
