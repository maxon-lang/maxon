...
strips the prose and leaves every directive byte-identical.
    ...
    python scripts/strip-spec-prose-comments.py            # rewrite specs/
...
def strip(lines, path, problems):
    ...
    and a line ENDING in a fence closes too — `specs/arrays.md` writes one
...
def main():
    ...
    parser.add_argument("paths", nargs="*", default=None,
                        help="files or directories (default: specs)")
    ...
    roots = [pathlib.Path(p) for p in args.paths] if args.paths else [pathlib.Path("specs")]
    files = []
    for root in roots:
        if root.is_dir():
