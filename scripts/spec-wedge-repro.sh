#!/usr/bin/env bash
# scripts/spec-wedge-repro.sh — loop the compiler spec suite until it WEDGES, and on a wedge
...
exe="$repoRoot/maxon-bin/.maxon/maxon"
outDir="$repoRoot/temp/wedge-repro"

if [ ! -x "$exe" ]; then
	echo "spec-wedge-repro: no the compiler binary at $exe — build it first" >&2
	exit 2
fi
...
killRecorded() {
	...
	for p in $recorded; do
		...
		case "$command" in
			*maxon-bin*|*wasmtime*) kill "-$signal" "$p" 2>/dev/null ;;
			*) : ;;
		esac
	done
	return 0
