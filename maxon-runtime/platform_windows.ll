...
; ============================================================================
; LibXml2 stubs for LLVM WindowsManifest
; These are never actually called, but LLVM requires them at link time
; ============================================================================

define void @xmlSetGenericErrorFunc(ptr %ctx, ptr %handler) {
entry:
  ret void
}

define ptr @xmlReadMemory(ptr %buffer, i32 %size, ptr %URL, ptr %encoding, i32 %options) {
entry:
  ret ptr null
}

define ptr @xmlDocGetRootElement(ptr %doc) {
entry:
  ret ptr null
}

define void @xmlFreeDoc(ptr %doc) {
entry:
  ret void
}

define void @xmlUnlinkNode(ptr %node) {
entry:
  ret void
}

define void @xmlFreeNode(ptr %node) {
entry:
  ret void
}

define ptr @xmlNewProp(ptr %node, ptr %name, ptr %value) {
entry:
  ret ptr null
}

define ptr @xmlStrdup(ptr %str) {
entry:
  ret ptr null
}

define ptr @xmlCopyNamespace(ptr %ns) {
entry:
  ret ptr null
}

define void @xmlFree(ptr %ptr) {
entry:
  ret void
}

define ptr @xmlAddChild(ptr %parent, ptr %cur) {
entry:
  ret ptr null
}

define ptr @xmlNewDoc(ptr %version) {
entry:
  ret ptr null
}

define void @xmlDocSetRootElement(ptr %doc, ptr %root) {
entry:
  ret void
}

define void @xmlDocDumpFormatMemoryEnc(ptr %doc, ptr %mem, ptr %size, ptr %encoding, i32 %format) {
entry:
  ret void
}

define void @xmlFreeNs(ptr %ns) {
entry:
  ret void
}

define ptr @xmlNewNs(ptr %node, ptr %href, ptr %prefix) {
entry:
  ret ptr null
}
