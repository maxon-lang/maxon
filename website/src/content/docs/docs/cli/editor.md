...
| `textDocument/completion` | Members after `.` (the trigger character): fields, methods, static functions and enum cases. The receiver may be a type name, `self` or `Self`, a field of the enclosing type, a `for … in` loop variable, or a local whose type is evident — including one bound by a call or by a `try … otherwise`. A type the buffer declares is answered from the buffer, including the members an `extension` over it adds. There is no completion of bare identifiers. |
