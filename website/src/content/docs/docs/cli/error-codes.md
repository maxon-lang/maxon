...
A type was required here and the token stream had something else. The compiler emits it for the two shapes where a name IS present but is not a usable type: a bare sized type as a typealias RHS (`typealias I = i64`), and an `as` cast target that names a typealias the reading file cannot see. A cast target no declaration binds ANYWHERE is E3011 instead, which is sharper than a blanket E2003 -- see specs/cast-target-type-resolution.md. It also covers the shape where NO name is present and none can be inferred: a closure parameter past the first in a container `map`'s transform (`nums.map(function(a, b) gives a + b)`). That position types ONE parameter, by the arity the transform is declared at, so a later untyped one has nothing to take a type from -- see specs/closure-param-type-inference.md. An untyped parameter in a position that offers nothing at all is E2015 instead, which names the one construct that does infer.
...
A bare function name has more than one reachable definition; qualify it as 'dir.name()'. a bare-name function call has more than one reachable definition under directory-as-module rules. The user must write the directory-qualified form (`dir.name()`) to disambiguate. Emitted by the cross-file fallback in `qualifyCalleeForContextWithType` when `methodNameIndex` returns multiple distinct visible candidates.
...
### E3122 — `transformArityMismatch`

A function value bound as a container `map`'s transform declares a different number of parameters than the position calls it with. `map`'s transform is `function(Element) returns Element` (`stdlib/Interfaces.maxon:189`), so it takes exactly ONE parameter -- the container's own element -- and the runtime walk calls it with exactly one argument plus the uniform `__env` slot. A transform declaring a further parameter reads a value nobody passed: `nums.map(function(a Integer, b Integer) gives b)` prints `sum=0` on arm64-macOS, `b` being whatever the second argument slot held, while on `wasm32-wasi` the program does not run at all: `call_indirect` type-checks the callee's signature and `__managed_map` traps with "indirect call type mismatch". One target of four could see it, which is what makes the shape undefined rather than unspecified. NOT E3036, which states that "a call passes a different number of arguments than the callee declares parameters": there is NO call at this span. The author bound a function VALUE to a position, and the call that would go wrong is one the compiler makes inside `__managed_map`, over a source line the author never wrote -- so E3036's sentence would be false about the program in front of them. NOT E3005 either, which is what the four DECLARED function-typed places (a cast, a `return`, an assignment, an ordinary function-typed argument) earn when their whole SHAPE disagrees. This position deliberately requires less: the transform's parameter TYPES are not checked here, so "the two function types must agree" would be false as a statement of what this position demands.
...
- the SAME NAME bound at two DIFFERENT slots (`walk(dir) or run(_, dir)`), which passes every slot test and is then decided by `Scope.install`, keyed by name and last-wins: the arm read slot 1 on the `walk` path, printing 0 where 7 was stored. The same hole existed one construct over for a single case's own list (`walk(x, x)`), so ONE check over the arm's finished binding list serves both doors. It is the `and fallthrough` rule one construct over (`Parser.rejectFallthroughIntoBindingArm`), which refuses a related hazard for a related reason: an arm body reached from a case that does not hold what its bindings destructure. The cure is to give the disagreeing case its own arm, or to rename the colliding binding.
...
A message of a service declares a parameter whose value cannot cross to another green thread. A send moves an argument or lends it, and either way the receiving green thread must reach, count and release what it is handed through the request union's payload cascade. Four shapes cannot be reached that way. A `Promise` is a green-thread handle its awaiter owns. A function value reaches a captured environment block, which is a box with a second referent by construction. A value held at an interface type is a fat pointer whose witness half is read-only data and whose value half is released through `__drop_existential`, a pair the request union's payload cascade cannot carry. An opaque type parameter has no layout at the send, so the sender cannot know what it is handing over. The diagnostic fires at the `spawn` that made the type a service, which may be in a different file from the method it names: whether a type is a service is a whole-program property, and the site that decided it is the one worth naming. The cure is to send a `.clone()`, to send the scalar the value is derived from, or to drop the parameter from the message and reach the value another way.
