---
title: Error Codes
description: Every diagnostic code the Maxon compiler reports, grouped by compilation stage.
sidebar:
  order: 7
---
...
### E2008 — `parserMismatchedEndLabel`
...
### E2050 — `otherwiseBlockMissingBinding`
...
### E2058 — `parserTestOutsideTestFile`

A `test` declaration appears in a file whose name does not end in `.maxtest`. Tests belong only in test files, so which declarations a build carries is answerable from the file list alone -- no flag, no attribute, nothing to read the body for. The message names the fix: rename the file to its `.maxtest` name, or move the declaration into a `.maxtest` file.

### E2059 — `parserTestEmptyName`
...
### E2064 — `directiveUnknownPredicate`
...
### E2065 — `directiveMalformedCondition`
...
### E2066 — `conformanceWithArgSurplus`
...
### E2070 — `ucdTableLabelUnknown`

A '\_\_Builtins.ucd\*' load names a compiler-owned Unicode table its intrinsic does not read. Each intrinsic reads exactly ONE table, because the label and the STRIDE are one fact: 'ucdByteAt' reads '\_\_ucd\_bmp', one byte per BMP codepoint, and 'ucdI64At' reads '\_\_ucd\_supp', an array of 8-byte packed range entries. Reading either with the other's stride cannot produce an answer -- only a wrong one -- so the pairing is refused rather than assumed, and the message names the table the intrinsic DOES read. ⚠ IT IS A SECURITY GUARD AND NOT A TYPO CHECK, which is why it is a refusal at the label token rather than a link failure later. The label becomes a FILE NAME under the stdlib directory, so an unchecked one is a PATH TRAVERSAL. A '\_\_ucd\_' PREFIX check does not close it: '\_\_ucd\_../../x' carries the prefix and still escapes. A ROSTER of the tables the compiler actually owns is what closes it. NOT E2010: a label that is not a string LITERAL at all is refused at the token by the ordinary 'expected a string literal' rule, because that is a shape the parser can state without knowing which intrinsic it is reading for.

### E2073 — `unknownStaticMember`

A qualified read names a member the type does not declare -- `Config.MIN_SIZE` where `Config` declares only `MAX_SIZE`. The base is a type the AUTHOR declared, so the type has a static roster to answer from and the honest sentence is about that roster. Blamed at the MEMBER token, which is the name that is missing. Without it the form falls to its last-resort reading as a SIZED NUMERIC BOUND (`u64.max`) and is refused E2010 -- "Expected 'min or max' but got 'MIN\_SIZE'", a demand the author never made, in words for a base this program does not have; and for `Box.min`, whose member spells a bound, on the BASE token, blaming a name that resolves perfectly well. THE GATE IS "the base names a declared STRUCT", deliberately not "the base is not a sized type name". A ranged ALIAS base keeps the bound reading -- specs/type-name-collision.md pins `Status.big` against an alias as E2010 -- and an UNDECLARED base has no roster, so the noun "type" would be false of it. Both still report as a malformed bound, which is a true statement about each; narrowing them further needs a base-kind-aware noun. The runnable oracle answers the same programs E3018 "Type 'Config' has no static member 'MIN\_SIZE'" -- one subject, each compiler in its own house spelling.

### E2074 — `nestedProjectFile`

A `.maxproj` file sits below another project's directory: a directory above it holds a `.maxproj` of its own. Projects do not nest, so a build refuses the pair from either end -- building the outer project, or building the inner one -- and the editor reports it on the inner `.maxproj`. Move the inner project out of the outer project's directory.

## Semantic Analysis (E3xxx)

Type checking, borrowing, visibility, declaration lookup.

### E3001 — `noMainFunction`

The program declares no 'main' function.

### E3002 — `mainWrongReturnType`
...
### E3015 — `semanticUnknownInterface`
...
### E3036 — `callArgCountMismatch`

A call passes a different number of arguments than the callee declares parameters.

### E3037 — `callUnknownArgLabel`

A call's 'name:' label names no parameter of the callee.

### E3038 — `callDuplicateArg`

Two arguments of one call target the same parameter.

### E3054 — `mainCannotThrow`

'main' is declared 'throws'. There is no caller left to handle the error.

### E3055 — `tryRequiresThrowingFunction`

'try' is applied to a call that cannot throw.

### E3057 — `throwingFunctionRequiresTry`

A throwing call, a call to a throwing interface method, or an `await` of a throwing promise is made without 'try' where nothing catches its error. Inside a block-form `try` or a `test` body no `try` is needed: the block's handler or the test takes the error. A closure written inside a test is not the test's body, so it still needs one. Write `try f() otherwise …`, propagate with a bare `try` from a function that `throws` the same error type, or move the call into a `try` block.

### E3058 — `otherwiseRequiresTry`

'otherwise' is written without a 'try'.

### E3059 — `errorTypeMismatch`
...
### E3061 — `semanticDuplicateTypeAlias`
...
A typealias is declared and never used. "Used" means the NAME appears in a type position. For a GENERIC-INSTANCE alias (`typealias Small = Array with Byte`) a spelling in another file counts, but only while one declaration in the whole program holds that name and both the file spelling it and the file declaring it mean the generic-instance form by it; every other alias form is answered from its OWN declaring file alone. An exported alias is exempt (one file cannot see another's uses), and being implicitly inferable from a bare `[...]` literal is not a use.
...
### E3066 — `semanticEnumNotComparable`
...
### E3068 — `refIdentityOnPrimitive`

The 'is'/'is not' reference-identity operator is applied to a value, which has no object identity. Identity has meaning only for a reference: a heap record is a pointer, and two names can share one. A primitive (`int`, `float`, `bool`, `byte`, ...) is refused, and so is an inline packed record — a `type` whose fields are all `let` and pack into one machine word, so its value is that word. An enum case and an opaque type parameter are values with no object anywhere. A `union` box and a function value are one-word addresses, and are refused because the language defines no identity reading for them. An existential is a fat pointer, so a one-word compare would ignore its witness half. Widening the admitted set yields a pointer compare, never a type error, so a tag admitted without a defined reading is a wrong answer rather than a missing one. `TypeRules.tagHasReferenceIdentity` owns the tag half and `Parser.valueMayBeAReference` the layout half.
...
### E3072 — `semanticBuiltinTypeConstruction`
...
### E3076 — `semanticConstructorRestriction`
...
### E3078 — `semanticVarFromImmutable`
...
### E3084 — `tryBlockBindingNotMatched`
...
### E3085 — `unionMatchPatternAmbiguous`
...
### E3086 — `semanticFieldNotInitialized`
...
An 'export' symbol is never referenced outside its declaring file. Drop the modifier, make it 'module', or say 'public' if it is API surface. an `export` (global) decl is never referenced from any file outside its declaring file. The decl could be file-private (drop the modifier) or `module`-visible. Applies to functions, types, enums, typealiases, top-level constants, top-level vars, and exported struct fields. Emitted by `checkUnusedExports` after all files have been parsed and type-resolved. A `public` declaration is EXEMPT. `export` says "other files may see this, and I expect this program to use it"; `public` says "this is API surface", so "no caller in this compilation" is not a finding about it. That distinction is what makes this diagnostic answerable at all: see `Visibility` in maxon-bin/Compiler/Project.maxon. A one-file program is audited like any other. The entry point and every target or task a `.maxproj` or `.maxtasks` file declares are exempt: nothing in the source calls them, because the runtime, `maxon build` or `maxon run` enters them by name. A type a SIGNATURE names is credited at the naming function's tier while that function is itself referenced from outside the type's declaring file, because dropping the modifier would then earn E3167. So "drop the modifier" is not the remedy for such a type: narrow the demanding function to file-private, or raise the type. Narrowing that function to `module` keeps a demand, which excuses this and E3094 but not E3093.

### E3093 — `semanticExportableAsModule`
...
### E3094 — `semanticUnusedModuleSymbol`
...
### E3095 — `semanticAmbiguousCrossFileCall`

A bare function name has more than one reachable definition; qualify it as 'dir.name'. Two or more directories declare a free function of the name and this file may name more than one of those declarations, each in a directory other than its own or the root. It is refused at each door a bare function name is written at — a call, a function value (`let f = helper`) and a function-backed enum case (`op = helper`) — and each accepts the directory-qualified form (`beta.helper`) as the remedy. A call is refused in every form it takes: as a statement or with its result used, under `try` or `if let … = try`, and spawned with `async`, awaited or stored. An enum case refused here is reported once, in the file that declares the enum, whichever file calls through it. Decided by `ProgramSignatures.resolveFunctionName`; a call is reported by `SemanticCheck.validateCall`, the other two doors by the parser.

### E3097 — `semanticEnumAccessorComparison`
...
### E3098 — `promiseErrorTypeMismatch`
...
- an associated-value error's payload has no static type to mm\_decref, so it LEAKS (only a runtime `errorIsHeapPtr` bit in the box approximates the answer);
...
### E3104 — `targetUnsupportedConstruct`
...
### E3112 — `stdlibShadowedNameIsAlsoAMember`
...
### E3113 — `throwsTypeNotAnErrorEnum`
...
### E3117 — `rawByteFillNarrowElement`
...
### E3119 — `associatedTypeAbiDisagreement`
...
### E3121 — `floatWidenedToExistential`
...
### E3129 — `matchArmPayloadBindingConflict`
...
### E3134 — `semanticServiceNotASpawnTarget`
...
### E3136 — `semanticServiceNotAMessage`
...
### E3141 — `semanticPromiseCannotBeBorrowed`

A promise owns a green thread, and a green thread has exactly ONE owner at every instant -- awaiting it consumes it, so under the ownership model it is a mutable value and cannot be borrowed. Reading a promise out of a container is therefore a MOVE, not a copy: the slot it came from must be emptied when the read is consumed, or the container's element walk reclaims a thread the consume already reclaimed. This refuses the reads that CANNOT NAME the slot they came from, because a move with nowhere to record its source cannot be finished: a list's `value()` or its iterator's `current()`, `last()`, a library member handing back a pair that carries a promise the container still holds, and a consume of a promise no frame owns (read twice from one slot, or merged from two branches). A struct FIELD is a slot on the same terms as a container element, and every door that spends a promise asks this -- `await`, `.cancel()`, a store into a container, a store into a field or a union case, an argument at a parameter the callee takes, an `otherwise` fallback merged into a `try`'s result, and a `return` -- so a read whose slot another read already consumed is refused wherever it is carried, naming the word the source wrote for that door. A `return` is refused for a promise no frame owns: a merge of two promises (`p if c else q`) or a second name for a parameter. Return one this frame owns -- a spawn, a `pop`/`remove`, or the parameter itself. An array cursor's `current()`/`peek(n)` and a `for (it, p) in a.withIterator()` destructure are NOT refused: each names the array slot its cursor stands on at the read, so an `await` empties that slot. The cure is a read that names its slot -- `get(i)`, `first()`, `for ... in`, or an array cursor -- or a move-out (`pop`, `remove`), whose result the caller owns outright. It is NOT about the promise being un-awaitable: every one of these reads yields a perfectly good promise. It is about there being no way to say which slot stopped owning it.

### E3142 — `semanticPromiseAlreadyConsumed`
...
### E3147 — `matchOrChainNeedsOwnLine`
...
### E3152 — `rawIntrinsicOutsideRuntimeTier`
...
### E3153 — `managedValueInRuntimeSource`
...
### E3154 — `wideVisibilityInRuntimeSource`
...
### E3155 — `reservedNameAsFunctionValue`
...
### E3157 — `contradictoryFrameDirectives`
...
### E3158 — `alwaysSplicedBodyIsStillCalled`
...
### E3160 — `semanticLentGraphEscapes`
...
### E3161 — `errorMatchNotExactlyOnce`
...
### E3162 — `bareGenericSubjectUnbound`
...
A `spawn` runs while the module-level globals are still being initialized. A global initializer runs before `main`, and the `let`s every message may read are marked shared only once the last initializer has returned, so a service started earlier could step a plain count concurrently with the mark -- or read a global not yet built. Found by closing the call graph from the initializers, dispatches included, and reported at the `spawn`; the note is the initializer that reaches it. Start the service in `main`, or in a function `main` calls.

### E3165 — `semanticLetInitializerReachesAVar`

A module-level `let`'s initializer reaches a module-level `var` whose value holds a record. A `let` is fixed at startup and may not hold what a `var` owns: the `var` may write the record after `main` starts, and in a program that spawns a service the `let`'s graph could not be marked shared with an owner outside the `let`s. An initializer cannot name another global, so the `var` is reached through what it calls -- the call graph once overload resolution has chosen each callee, dispatches included. Refused whether or not the program spawns, and even when only a scalar is read out of the `var`, since what the initializer keeps is not followed. A scalar `var` owns no record and is not refused. Reported at the `var`'s use; the note is its declaration. Make the binding a `var`, or build its value without reading the `var`.
...
### E3167 — `semanticSignatureTypeLessVisibleThanFunction`
...
### E4016 — `builtinErrorEnumMismatch`
...
## Code Generation (E5xxx)

Native machine code emission.

### E5001 — `registerPressure`
...
## Binary Writer / Linker (E6xxx)

Executable formatting and linking.

### E6002 — `binaryOutputNotReplaceable`
...
### E9002 — `internalDroppedSignature`
...
### E9003 — `internalDeclarationTheSweepMissed`

A function declaration is missing from the whole-program declaration index. This is a compiler bug. Before any file is parsed in full, the compiler sweeps every file for the functions it declares, so that a call can be typed from a declaration in a file not yet parsed. The full parse of the declaration at this position found a function the sweep did not record, and a call to it would be compiled as a call to a function nobody declared. The fault is the compiler's: report it to the Maxon project with the source that produces it.
