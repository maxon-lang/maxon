...
type Tok
	export var line as int

	static function make(l int) returns Tok
		return Self{line: l}
	end 'make'
end 'Tok'
...
type Lex
	...
	function consumeExpecting(tag int) returns Tok throws IterErr
		counter = counter + tag
		counter = counter + tag
		counter = counter + tag
		counter = counter + tag
...
function run() returns int throws IterErr
