...
⛔ **THE RECIPROCAL IS NOT AN INSTRUMENT IN THIS LANGUAGE, so there is no case here.** `1.0 / -0.0`
being `-inf` while `1.0 / 0.0` is `+inf` is the textbook way to observe a zero's sign, and division
by a possibly-zero divisor THROWS (E3057) — measured on both compilers — so the infinity it looks at
does not exist to be compared. No ranged type restores it: the reciprocal must accept a ZERO to have
a sign to reveal, and a range containing zero is exactly what makes the divide fallible.

⚠ **The FILE's subject is untouched.** `-0.0`'s sign bit is pinned by the negated-literal,
field-default, range-bound and interpolation cases below, and observing the sign of a zero DIRECTLY
is `__Builtins.floatToBits` — see `specs-shv2/atan2.md`'s `atan2.signed-zero`, which pins seven
signed-zero argument combinations, the three with a zero RESULT through that builtin.
