...
<!-- ⭐ A CHARACTER LITERAL AT A `Character` PARAMETER, AND IT NEEDS NO ADOPTION AT ALL — re-measured 2026-08-05 (A5m-ab). This note used to file the case as a gap: the other Character-expecting positions (comparison, assignment, `return`) adopted the literal's type and a call ARGUMENT could not, because the parse-time whole-program index records parameter NAMES but no parameter TYPES (`mixDeclaration` hashes a declaration's name and RETURN type only), so at the moment an argument is parsed there was nothing to ask what the parameter expects — measured then as `E3005: argument type mismatch for 'target': expected 'Character', got 'int'`, with only a multi-byte literal (`target: 'ö'`) compiling. That `got 'int'` can no longer happen: EVERY character literal materializes as a `Character` (`Parser.parseCharLiteral`), so `target: 'a'` arrives already being what the parameter declares and the missing per-parameter fact was never needed for this direction. What the case pins is the model end to end — a `Character` minted per trip by `for c in "banana"`, handed across a call beside a literal one, compared by `Character` equality in the callee (`character-parameter-and-equality` pins the multi-byte spelling of the same thing). ⚠ **The MIRROR image is where the absent parameter type still shows, and there shv2 is RIGHT**: a character literal at an INTEGER-typed parameter is refused `E3005 … expected 'Integer', got 'Character'`, because `integerizedOperand` coerces against a DECLARED slot and the argument door has none to read. MEASURED 2026-08-05 — the oracle refuses it in the same words, so that refusal is agreement with the references and not a gap -->
