...
```
```maxoncstderr
error E2015: <fragment>:6:13: Unsupported: `Character` member 'codepoint' — shv2 provides bytes/byteLength; `asciiValue` is throwing and its `if try …` call form does not exist yet, and `codepoint`/`codepoints`/`compare`/`hash`/`clone` and the static `fromCodepoint` have no consumer in the corpus
```
