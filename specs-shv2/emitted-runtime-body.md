...
onto `stdlib/String.maxon:297`, whose own body is `try byteAt(index) otherwise panic(…)` — ordinary
...
`stdlib/String.maxon:282` declares. The corpus's `hashString` is what installs it here, and this block is
its only gate.
