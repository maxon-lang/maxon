...
⚠ **PORT NOTE (BATCH29/A3a).** `status:` reads `stable` here and `selfhosted` in `/specs`: that frontmatter names the runner that owns the file, and the owner here is shv2. The `/specs` original carries no `RequiredIR` block and is byte-identical below the frontmatter. Its twin stays `status: selfhosted` for a different reason, stated there: the bootstrap MISCOMPILES this program — `MM leak: 1 allocation(s) remain`, exit 101, which is the very defect the case was written to catch. shv2 runs it green.
