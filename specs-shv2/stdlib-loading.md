...
subdirectories alike, exactly as v1 and the C# bootstrap do — and loads all of them. Exactly ONE file
...
- FUNCTION-name-vs-BARE-BUILTIN collisions are SILENT, so the builtin is RETIRED FIRST — which is
  ...
  stdlib declaration with a user one needs namespaces, which shv2 does not have"*. Both are FALSE and
  nothing ever ran them: N1's namespaces landed, a user `function sleep` compiles clean, and the
  USER's body is what its own call sites reach — which is also what the reference compiler does. The
...
A module's real content is its CALL SITES and not its own parse: `maxon-shv2 build <module>` answering
...
same), and which made the module unusable by this file's own rule: a call to the name could never
...
answers identically on the identical source.
