...
| 6 | General inlining with a cost model | beyond today's leaf-only single round; needs inline-frame records in `.mxdbg` and a symbolizer that walks them so a panic inside an inlined body still names its function, and a pressure check so the merged loop's working set never manufactures an E5001 | `flipCount`/`advancePermutation` into `main`: no call in the permutation loop means no confinement, no argument moves and no callee prologue (five pushes and pops per permutation) | med-high | open — **recommended next**: it removes the call-crossing cost rows 13, 17 and 18 only redistribute or shrink, and the remaining `main` cost (the `mod 2` sequence, row 16) with it |
...
| 17 | Interprocedural clobber analysis (`-fipa-ra`) | a direct call to a function allocated already clobbers only the registers that function writes; callees are allocated before callers over the call graph, and recursion, indirect calls and runtime callees keep the full ABI set | `main`'s eight call-crossing values would survive in caller-saved registers `flipCount` and `advancePermutation` never touch, with no spill at all — the cost row 13 only redistributed | med-high | open — the general answer to the five-register shortage; measure `flipCount`'s actual clobber set first (it uses most of the pool in its own loops, so the win may be small here and large elsewhere) |
| 18 | A callee-saved XMM as the spill home | a value confined at a call takes an idle callee-saved XMM (`xmm6`–`xmm15` here; `d8`–`d15` on arm64) through `movq` instead of a frame slot; the register is reserved out of the float pool and saved by the prologue | the reloads round 13 left in `main` (`temp`, `count`, `n`) and every spill in a float-free loop; a `movq` round trip is 2–3 cycles against 4–5 through store forwarding | med | open — only pays on a spill that is on the serial chain, which row 13 measured `main`'s are not; below 17 and 6 |
