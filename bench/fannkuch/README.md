...
| 6 | Called-once inlining with inline frame records | every function with exactly one direct call site is spliced into its caller regardless of size (post-order waves over the call graph; refused when recursive, throwing, by-reference, over a register-pressure budget, referenced by anything but the call, or on a target without inline frame records); every block carries its inline site, the backends emit an `__inlframes` table behind `__symtable`, and the panic/fault printers and the profiler walk it so a trace names the spliced callee at every nesting level; the leaf inliner's re-run redirect and its store+panic and div/mod refusals are gone | `flipCount`, `advancePermutation` and `initialPermutation` into `main`: no call in the permutation loop, no prologue/epilogue, no argument moves, no call-crossing spill | med-high | **closed, round 11** — the largest single win of the loop; a general size-driven cost model for multi-site callees stays open |
...
| 13 | Spill weight by hot use count in the live-range splitter | a per-value hot weight (`10^depth` per def and use record, 0 off the hot path) ranks the two spill tiers as one tournament, lightest first, so a loop invariant with one hot use is spilled before a loop-carried value with three | `main`'s `index` and `checksum` live in frame slots across the two calls per permutation while `temp`, `count` and `limit` (1, 1, 2 hot uses) hold three of the FIVE callee-saved registers — `rsi`/`rdi` are caller-saved in Maxon's ABI, so eight call-crossing values compete for five homes; ~190 of 2,376 samples sat on the slot moves | med | **DECLINED, round 10** — the census moved as predicted and the medians did not (the second row of the benefit table): `main`'s loop 13 → 5 slot ops per permutation, ops 2085 → 2076, n=11 medians 2,327 → 2,327 ms, self-compile −1.3%. The moves sit off the serial chain behind two calls and the core hides them; the profile's samples on them were skid from the call returns. The diff also needed a breadth-first fallback beneath a full-pool overflow (a false E5001 on `SignatureIndex.foldFile`) — a warning that the victim order reaches further than one peak |
| 17 | Interprocedural clobber analysis (`-fipa-ra`) | a direct call to a function allocated already clobbers only the registers that function writes; callees are allocated before callers over the call graph, and recursion, indirect calls and runtime callees keep the full ABI set | `main`'s eight call-crossing values would survive in caller-saved registers `flipCount` and `advancePermutation` never touch, with no spill at all — the cost row 13 only redistributed | med-high | open — no longer a fannkuch shape: after round 11 the permutation loop holds no call; a general win elsewhere |
| 18 | A callee-saved XMM as the spill home | a value confined at a call takes an idle callee-saved XMM (`xmm6`–`xmm15` here; `d8`–`d15` on arm64) through `movq` instead of a frame slot; the register is reserved out of the float pool and saved by the prologue | the reloads round 13 left in `main` (`temp`, `count`, `n`) and every spill in a float-free loop; a `movq` round trip is 2–3 cycles against 4–5 through store forwarding | med | open — no longer a fannkuch shape after round 11 (no call in the permutation loop) |
| 19 | A wider callee-saved half in Maxon's ABI | `rsi`/`rdi` (and more) preserved by the callee, as the Windows convention does | every non-leaf prologue pushes more so every call-crossing loop spills less | low | open — a trade to measure on the self-compile, not a fannkuch win: row 13 says the spills it would remove cost nothing here |
...
|---|---|---|---|---|
| 11 | called-once inlining + inline frame records (row 6): `InlineLeaves` splices every function with one direct call site into its caller in post-order waves, sound for traces through per-block inline sites emitted as an `__inlframes` table the x64/arm64 printers and the profiler walk (`specs/inline-called-once.md`, 12 cases; the leaf rule's redirect removed). Landed beside it, entangled in `emitFunctionChunk`: JCC-erratum padding — a branch unit that crosses or ends on a 32-byte boundary is re-emitted behind NOP padding and every chunk starts on 32 (`specs/jcc-erratum-padding.md`; it was `tests/profile`'s pre-existing red, and fannkuch's reverse-loop back edge ended exactly on a boundary). The optimize pass removed phantom loops from terminator-less dead blocks (LICM/unswitching were superlinear in function size) and a quadratic reach cache in the splitter. `main` holds the three bodies; the permutation loop has no call | 2,335 → 1,810 ms (**−22.5%**); n=12 30,930 → 24,054 ms, ratio **1.17** | ops 2085 → 1984, mov 226 → 206, call-direct 142 → 144 (the spliced bodies' cold slow arms), exe 45,334 → 48,134 B (padding + inline tables) | 40,194 → 43,377 ms (**+7.9%**, over the 5% band — landed on the user's ruling; +1.25 s is the inliner pass itself, +1.0 s regalloc on the bigger functions, +0.85 s the rescheduled cleanups and constant folds; the compiler's own code runs slightly faster) |
| 10 | **DECLINED** — spill weight in the live-range splitter (row 13): `UseIndex` carried a per-value hot weight in place of `touchesHotLoop`, and `chooseVictim`'s tiers 2 and 4 became one tournament ranked lightest first, then tier, then Belady; beneath a full-pool overflow it had to rank by next use first or a call-free block of the same loop stayed over the pool (a false E5001 compiling the tree). `main`'s permutation loop went from 13 to 5 slot ops per permutation, exactly the predicted shape, and the time did not move. Not landed; the round's one landed change is the miscompile the self-compile exposed (a spill reload coloured into a register a later op of the block reads by name — `sweepEstablishedRegisters` now forbids the registers physically live after a def) | 2,327 → 2,327 ms (**0.0%**) | ops 2085 → 2076, mov 226 → 220 | 37,383 → 36,887 ms (−1.3%), both arms at their own fixed point |
