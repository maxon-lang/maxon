...
| 11 | Redundant-load elimination under a dominating load | a load whose address was loaded on a dominating path with no store or call between is replaced by that value | `flipCount`'s second `temp.get(firstValue)` reloads `[buf + fv*8]` one block after the first, per flip | low | open — **next**: round 8's profile is `flipCount` 67.7%, `advancePermutation` 19.4%, `main` 12.8%, and the flip loop's per-flip cost is now the `firstValue >= 0` range check, the bound check, this reload and the store |
