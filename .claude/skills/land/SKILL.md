...
| **Every touched golden staged — MINTED *and* MODIFIED** | `git status --short specs/fragments/ specs/` → `git add` **every line**, `??` and ` M` alike. See the box below |
...
> **modified** one is the dangerous half — it looks like churn, and `git checkout -- specs/fragments/`
...
> ⚠ **`git add -A specs/fragments/` picks up deletions too** — a golden the runner *deleted* is
