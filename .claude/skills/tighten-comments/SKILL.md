...
4. ⛔ **NEVER RECORD WHERE CODE CAME FROM. Provenance is history.** No "ported from", no "the same
   reason X does it", no "inherited from", no citation into a file that is not in this tree. A
   comment describes THIS code.
   ...
   reads better anyway, because it no longer needs the reader to know where the code came from:
   ...
   - ❌ `ported from the code that keys its stdlib cache per target`
...
The register to aim at — real comments from `maxon-bin/Compiler/Lsp/`:
