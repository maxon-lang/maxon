...
| Fragments | ⚖ **THE GOLDENS ARE REFERENCE, NOT A GATE** (user, 2026-08-02) — **nothing here is red or green, and none of it reaches an exit code.** `git status --short specs-shv2/fragments/` — **additions only**. An **`M`** is a codegen change: justify or fix. Empty diff after a spec run **proves byte-identical codegen**. ⚙ **A run REPORTS what moved and what is owed, and fails on neither:** it prints `N golden fragment(s) no longer match…` for drift, and asks git whether any golden under `specs-shv2/fragments/` is untracked, naming each one (`maxon-shv2/Testing/GoldenTracking.maxon`). `git add` pays the second; the **`M`** column is still yours to justify, because "this codegen change is intended" is not a question a machine can answer. ⛔ **These reports used to FAIL the run, and that is why they no longer do:** an x64-linux red read as *"10 stale golden mismatches + 9 others"* and the 9 were nine float programs exiting 1 (row `X5`), unlooked-at for a day. **A red suite now means a program did the wrong thing — read it, there is nothing in it to regenerate away** |
