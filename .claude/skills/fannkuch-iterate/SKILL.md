...
>
> The loop runs on `main` like any other change: `/land`'s rebase on `origin/main`, its battery ONCE,
> one commit, one push — and no waiting for CI after it. Start each round in a fresh session — a round's context is the README ranking, the log and
...
- Build the compiler from the tree: `./maxon-bin/.maxon/maxon.exe run build` at the repository root
  (`scripts/build-from-seed.sh` if the slot is empty). One self-compile is enough for the PROGRAMS it
  emits. The compiler's OWN code is emitted by the compiler that built it, so §5's `--self-compile`
  arm needs a second build with the first build's output — without it the experiment's self-compile
  shows only the new pass's cost, never its codegen win.
...
- Run §5 and §6 below between `/land`'s battery and its commit, so the round is ONE commit carrying
  ...
  by a newer `origin/main` is `/land`'s: rebase and re-run §8.

## 5. A/B

`PRE` is the commit the change sits on, taken AFTER `/land`'s rebase — the change is still uncommitted,
so that is `PRE=$(git rev-parse --short HEAD)`. A `PRE` taken before the rebase is a control missing
every commit the rebase brought in.

The harness builds the control once, from the seed, in its own git worktree under
`bench/fannkuch/out/ref-$PRE/tree/`, and reuses it by sha. That seed build is a C1 carrying the seed's
runtime, so bring it to its own fixed point before the A/B: run the harness once so it exists, then in
that tree `./maxon-bin/.maxon/maxon.exe run build`.

```
python scripts/bench-fannkuch.py --n 11 --runs 5 --ref $PRE --self-compile --note "<candidate>: <mechanism>; census <col> a->b; predicted <shape>; both arms at their own fixed point"
```

Control and experiment are two compilers in ONE session, runs interleaved, the control first whatever
the argument order. A before/after across a source change is not an A/B (roadmap, EC19). The note
carries the WHY; the tool cannot.

> ## ⛔ HALT ON A SLOWER SELF-COMPILE (user ruling 2026-09-08)
>
> `--self-compile` times each arm's compiler compiling `maxon-bin`. **A `HALT` line means the change made the compiler compile
...
> record — the README row marked DECLINED with the two medians, the census movement and the mechanism,
> the log rows §5's `--note` already wrote, and the roadmap's DECLINED row — so the next session does
> not rebuild it. A decline is a measurement, not a failure; the roadmap's
> DECLINED rows are its most-cited ones.

## 6. Headline

Whenever n=11 moved by more than the arms' spread. One run is enough while the arms are far apart;
take 5 only when they are within a few percent of each other or of C:
...
- **Attribute the shape before predicting the win.** The eight-row EC workstream was ×1.90 on fannkuch
  and −5.8% on nbody.
