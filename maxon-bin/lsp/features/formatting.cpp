...
std::string FormattingProvider::formatArrayLiteral(const ArrayLiteralExprAST* expr, const FormattingConfig& config) {
    ...
    if (!expr->values.empty()) {
        ...
        out << "]";
    } else if (expr->hasVariableSize()) {
        // Variable-sized array: [expr]type
        out << "[" << formatExpression(expr->sizeExpr.get(), config) << "]";
        out << formatType(expr->elementType);
    } else {
        // Constant-sized array: [10]int
        out << "[" << expr->size << "]" << formatType(expr->elementType);
    }
...
std::string FormattingProvider::getOperatorString(char op) {
    switch (op) {
        ...
        case '=': return "==";
        case '!': return "!=";
        case 'l': return "<=";  // Less than or equal
        case 'g': return ">=";  // Greater than or equal
        case '&': return "and";
        case '|': return "or";
