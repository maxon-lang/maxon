...
void CodeGenerator::initProfiling()
{
    ...
    // Determine target platform from compile-time macros
#ifdef _WIN32
    bool isWindows = true;
#else
    ...
    // Declare platform-specific I/O functions for profiling output
    if (isWindows)
    {
    ...
    {
        // POSIX write function: i64 write(i32 fd, ptr buf, i64 count)
        llvm::FunctionType *writeType = llvm::FunctionType::get(
            llvm::Type::getInt64Ty(context),
            {
                llvm::Type::getInt32Ty(context),    // fd
                llvm::PointerType::get(context, 0), // buf
                llvm::Type::getInt64Ty(context)     // count
            },
            false);
        llvm::Function::Create(writeType, llvm::Function::ExternalLinkage, "write", module.get());
    }
    ...
    if (isWindows)
    {
        ...
        printBuilder.CreateCall(memcpyFunc, {buffer,
                                             prefixGlobal,
        ...
        printBuilder.CreateCall(memcpyFunc, {countPtr,
                                             countAlloca,
        ...
        printBuilder.CreateCall(writeFileFunc, {stdoutHandle,
                                                buffer,
    ...
    else
    {
        ...
        // Write the buffer to stdout (fd=1)
        llvm::Function *writeFunc = module->getFunction("write");
        printBuilder.CreateCall(writeFunc, {llvm::ConstantInt::get(llvm::Type::getInt32Ty(context), 1), // stdout fd
                                            buffer,
                                            llvm::ConstantInt::get(llvm::Type::getInt64Ty(context), 22)});
    }
...
{
    if (!enableProfiling || !bbCountGlobal)
        return;
...
{
    if (!enableProfiling || !printBBCountFunc || !mainFunc)
        return;
