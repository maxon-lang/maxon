...
std::vector<std::string> extractStructNames(const std::string &filePath) {
	...
	try {
		...
		for (size_t i = 0; i < tokens.size(); ++i) {
			if (tokens[i].type == TokenType::KEYWORD && tokens[i].value == "type" && i + 1 < tokens.size()) {
				// Accept both IDENTIFIER and KEYWORD as type names (e.g., "array" is a keyword but valid type name)
