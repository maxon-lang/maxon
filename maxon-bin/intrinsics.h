...
struct IntrinsicParam {
	...
	bool isAnyType;						   // True if any type is accepted (for generic intrinsics)
	std::vector<std::string> allowedTypes; // For multiple allowed types or array element types

	// Single type parameter
	IntrinsicParam(const std::string &t) : type(t), isArrayType(false), isAnyType(false) {}
