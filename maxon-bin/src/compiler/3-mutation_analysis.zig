...
pub const MutationAnalyzer = struct {
    ...
    fn checkExpressionForParamMutation(
        ...
        switch (expr) {
            ...
            // integer, float_lit, nil_lit, unary, binary, compare, logical, call, struct_init, array_literal, array_type:
            // These cannot be mutation targets (only identifier, field_access, index can)
            .integer, .float_lit, .bool_lit, .nil_lit, .unary, .binary, .compare, .logical, .call, .struct_init, .array_literal, .array_type => {},
        }
    }
