...
/// Used internally when we know the index is valid
fn intrinsicManagedArrayGetUnchecked(self: *AstToIr, call: ast.CallExpr) ConvertError!TypedValue {
    ...
    // Load buffer pointer from managed array (offset 0)
    const buf_ptr = try self.func().emitLoad(managed.value, .ptr);

    // Calculate element pointer: buffer + index * 8
    const elem_ptr = try self.func().emitGetElemPtr(buf_ptr, index.value, 8);

    // Load and return the element value
    const elem_val = try self.func().emitLoad(elem_ptr, .i64);

    return .{ .value = elem_val, .ty = .{ .primitive = "int" } };
}
