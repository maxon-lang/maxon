...
bool testDeadCodeElimination(bool verbose) {
    if (verbose) {
    ...
    if (!writeTestFile(testFile, testCode)) {
        if (verbose) std::cout << "  FAILED: Could not write test file" << std::endl;
        return false;
    }
    
    if (!compileTestFile(testFile, exeFile, false)) {
        if (verbose) std::cout << "  FAILED: Compilation error" << std::endl;
...
bool testLinkerSelectiveLinking(bool verbose) {
    if (verbose) {
    ...
    if (!writeTestFile(arrayFile, arrayCode) || !writeTestFile(minimalFile, minimalCode)) {
        if (verbose) std::cout << "  FAILED: Could not write test files" << std::endl;
        return false;
    }
    ...
    if (arrayFuncCount < 0 || minimalFuncCount < 0) {
        if (verbose) std::cout << "  FAILED: Could not analyze binaries" << std::endl;
        return false;
    }
...
bool testBasicCompilation(bool verbose) {
    if (verbose) {
    ...
    if (!compiled || !exeExists) {
        if (verbose) std::cout << "  FAILED: Could not compile basic program" << std::endl;
        return false;
    }
...
bool testFragmentTestRunner(bool verbose) {
    if (verbose) {
...
int runSelfTest(bool verbose) {
    ...
    if (testBasicCompilation(verbose)) {
        passed++;
        if (!verbose) std::cout << "  ✓ Basic compilation" << std::endl;
    } else {
        failed++;
    ...
    if (testDeadCodeElimination(verbose)) {
        passed++;
        if (!verbose) std::cout << "  ✓ Dead code elimination" << std::endl;
    } else {
        failed++;
    ...
    if (testLinkerSelectiveLinking(verbose)) {
        passed++;
        if (!verbose) std::cout << "  ✓ Linker selective linking" << std::endl;
    } else {
        failed++;
    ...
    if (testFragmentTestRunner(verbose)) {
        passed++;
        if (!verbose) std::cout << "  ✓ Fragment test runner validation" << std::endl;
    } else {
        failed++;
