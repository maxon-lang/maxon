...
int countFunctionsInBinary(const std::string &exePath) {
	// Find llvm-objdump in the LLVM installation directory
	std::string execDir = getExecutableDirectory();
	std::filesystem::path llvmBinDir = std::filesystem::path(execDir).parent_path() / "llvm-project" / "bin";
	std::string llvmObjdump = (llvmBinDir / "llvm-objdump").string();
...
#ifdef _WIN32
	std::string command = "\"\"" + llvmObjdump + "\" -d \"" + exePath + "\"\"";
#else
	std::string command = "\"" + llvmObjdump + "\" -d \"" + exePath + "\"";
#endif
