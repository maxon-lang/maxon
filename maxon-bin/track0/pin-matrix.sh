...
# scheduler counters; this one drives the torture programs across processor counts
# and asserts what the SCHEDULER does with them.
#
...
if [ ! -x "$MAXON" ]; then
	echo "FAIL: no the compiler binary at $MAXON (build it, or set MAXON=)"
	exit 1
fi
