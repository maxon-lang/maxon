...
# the monitor run), then runs four checks that together prove the emitted per-P
# sharded lock-free allocator + green-thread scheduler are correct under more
# than one live P. See README.md for what each check means.
#
# ⚠ CHECK 4 READS `[slab-stats]`, WHICH THE EMITTED RUNTIME DOES NOT PRINT.
# `MAXON_SLAB_STATS` and `MAXON_SLAB_GLOBAL_LOCK` are unimplemented: a program
# reads the same cross-P event for itself through
# `__Builtins.slabRemoteFreeCount()` (asserted by `specs/slab-sharding.md`), and
# there is no serialising knob to A/B against. So check 4 SKIPS on a run that
# dumps no stats line rather than failing over a counter nobody printed.
#
# Knobs exercised: MAXON_MAX_PROCS (scheduler P clamp; =1 forces single-thread),
# MAXON_SLAB_STATS / MAXON_SLAB_GLOBAL_LOCK (see the warning above).
#
# Exits 0 iff every check that can run passes; non-zero (and prints FAIL) otherwise.
...
. "$REPO/scripts/lib/host-binaries.sh"
MAXON="${MAXON:-$(maxon_compiler_path "$REPO")}"
...
run_torture "" 1 0; UNCLAMP_STATS="$STATS"; UNCLAMP_RF="$RF"; UNCLAMP_AGG="$AGG"; UNCLAMP_RC="$RC"
run_torture 1 1 0;  SERIAL_RF="$RF"
: "${UNCLAMP_RF:=0}"; : "${SERIAL_RF:=0}"

# Both halves below read counters that only a `[slab-stats]` dump carries, and the
# emitted runtime prints none: the cross-P count is a LANGUAGE surface
# (`__Builtins.slabRemoteFreeCount()`, asserted by `specs/slab-sharding.md`) and no
# knob serialises the allocator to A/B against. Absent the line, `$RF`/`$LW` are
# empty and every threshold below would be compared against a default — a verdict
# about nothing, which is worse than no verdict.
if [ -z "$UNCLAMP_STATS" ]; then
	skip "no [slab-stats] line: MAXON_SLAB_STATS and MAXON_SLAB_GLOBAL_LOCK are unimplemented."
	echo "        A program reads the cross-P count itself via \`__Builtins.slabRemoteFreeCount()\`;"
	echo "        \`specs/slab-sharding.md\` asserts on it. Nothing here can be measured from outside."
else
