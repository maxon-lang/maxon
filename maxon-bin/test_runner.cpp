	...
	try {
		...
		for (unsigned int i = 0; i < numWorkers; ++i) {
			...
			if (pid == -1) {
				...
				// Child process - execute test-fragments-subset
				std::vector<char *> args;
				args.push_back(const_cast<char *>("maxon"));
				args.push_back(const_cast<char *>("test-fragments-subset"));

				std::string outputFileArg = outputFiles[i];
				args.push_back(const_cast<char *>(outputFileArg.c_str()));
				...
				// Add test files
				std::vector<std::string> testFileArgs = testGroups[i];
				for (auto &testFile : testFileArgs) {
				...
				std::cerr << "Error: Failed to exec worker process " << i << std::endl;
				exit(1);
			} else {
		...
		// If no tests were actually run (worker processes didn't execute), this is a failure
		if (passedTests == 0 && failedTests == 0 && totalTests > 0) {
			std::cerr << "Error: No tests were executed. This may indicate worker processes failed to run." << std::endl;
			return 1;
		}
